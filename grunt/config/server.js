...
module.exports = function(grunt) {
  ...
  function consoleLoggerMiddleware(req, res, next) {
    if (!(req.method === 'POST' && req._parsedUrl.pathname.replace(/\//g, '')
        === 'console' && Array.isArray(req.body))) {
      return next();
    }
    ...
    req.body.forEach(function(log) {
      ...
      if (log.type === 'error') {
        ...
        if (!coverageWriteStream) {
          coverageWriteStream = fs.createWriteStream(path.join(__dirname,
            '/../../coverage.log'));
        }
  ...
  function testResultLoggerMiddleware(req, res, next) {
    ...
    grunt.log[logType]('[%s][%s]', req.headers['user-agent'],
      Date.now(), message);
  }
      ...
      options: {
        ...
        middleware: function(connect, options) {
          ...
          return [
            ...
            connect.logger({format:'[:user-agent][:timestamp] :method :url',
