        ...
        && (source.path.len() == inferred.path.len()
            || (inferred.path.len() >= source.path.len()
                && !inferred.path.iter().any(|t| {
                    t.property
                        == react_compiler_hir::PropertyLiteral::String(JsString::from("current"))
                })))
    {
        CompareDependencyResult::Ok
    } else if is_subpath {
        if source.path.iter().any(|t| {
            t.property == react_compiler_hir::PropertyLiteral::String(JsString::from("current"))
        }) || inferred.path.iter().any(|t| {
            t.property == react_compiler_hir::PropertyLiteral::String(JsString::from("current"))
        }) {
    ...
    let path_str: String = dep_path
        ...
        .map(|entry| {
            let prop = match &entry.property {
                react_compiler_hir::PropertyLiteral::String(s) => s.to_string(),
