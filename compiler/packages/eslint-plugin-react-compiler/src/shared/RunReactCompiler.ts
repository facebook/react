  ...
  for (const [key, entry] of Object.entries(userOpts)) {
    if (key === 'environment' && COMPILER_OPTIONS.environment != null) {
      for (const envKey of Object.keys(entry as Record<string, any>)) {
        if (
          COMPILER_OPTIONS.environment.hasOwnProperty(envKey) &&
          isDeepStrictEqual(
    ...
    } else if (COMPILER_OPTIONS.hasOwnProperty(key)) {
...
class LRUCache<K, T> {
  ...
  #headIdx: number = 0;
  // #store: (key: K) => T | Error;
  // #isValue: null | ((key: T | Error) => key is T);

  constructor(
    size: number,
    // store: (key: K) => T | Error,
    // isValue?: (key: T | Error) => key is T,
  ) {
    this.#values = new Array(size).fill(SENTINEL);
    // this.#store = store;
    // this.#isValue = isValue ?? null;
  }
  ...
  get(key: K): T | null {
    ...
    if (idx === this.#headIdx) {
      ...
      //   if (this.#isValue && !this.#isValue(value)) {
      //     return value; // Return error directly
      //   }
      //   this.#headIdx =
