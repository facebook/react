...
import {HIRFunction, Type, typeEquals} from '../HIR';

/**
 * Temporary workaround for InferTypes not propagating the types of phis.
 * Previously, LeaveSSA would replace identifiers with a canonical identifier,
 * which would have the side-effect of choosing a particular operand (with a type)
 * to use for the phi.
 *
 ...
 * Once we fix InferTypes to infer phi types, then the InferMutableRanges bug is
 * exposed.
 *
 * As a temporary workaround, this pass propagates the type of phis and can be called
 * safely *after* InferMutableRanges.
 */
