...
export function validateNoDerivedComputationsInEffects(fn: HIRFunction): void {
  ...
  for (const block of fn.body.blocks.values()) {
    let needsFixPointItr = recordPhiDerivations(block, context);
    for (const instr of block.instructions) {
      ...
      if (needsFixPointItr && modifiedContext) {
         ...
         */
        needsFixPointItr = recordPhiDerivations(block, context);
      }
    }
  }
...
function recordPhiDerivations(
  ...
  let needsFixPointItr = false;
  for (const phi of block.phis) {
    let typeOfValue: TypeOfValue = 'ignored';
    let sourcesIds: Set<IdentifierId> = new Set();
    for (const operand of phi.operands.values()) {
      const operandMetadata = context.derivationCache.get(
    ...
    if (typeOfValue !== 'ignored') {
      addDerivationEntry(
        ...
        sourcesIds,
        typeOfValue,
        context.derivationCache,
      );
    }
  }
  return needsFixPointItr;
}
...
function recordInstructionDerivations(
  ...
  if (value.kind === 'FunctionExpression') {
    ...
    for (const [, block] of value.loweredFunc.func.body.blocks) {
      for (const instr of block.instructions) {
        modifiedContext = recordInstructionDerivations(instr, context);
      }
    }
  ...
  for (const lvalue of eachInstructionLValue(instr)) {
    addDerivationEntry(lvalue, sources, typeOfValue, context.derivationCache);
  ...
  for (const block of effectFunction.body.blocks.values()) {
    ...
    for (const instr of block.instructions) {
      ...
      if (
        ...
        const argMetadata = context.derivationCache.get(
        ...
        const calleeMetadata = context.derivationCache.get(
