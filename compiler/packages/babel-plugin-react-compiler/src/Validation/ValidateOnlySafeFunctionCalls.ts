  ...
  for (const [_, block] of fn.body.blocks) {
    for (const instr of block.instructions) {
      function testCallee(callee: Place) {
        if (callee.identifier.type.kind === 'Function' && callee.identifier.type.shapeId == null && Array(...eachInstructionOperand(instr)).some((x) => (x.effect === Effect.ConditionallyMutate || x.effect === Effect.Mutate) && x.identifier.type.kind !== "Primitive")) {
