...
> 5 |     x.value = e.target.value;
    |     ^ InvalidReact: Mutating a value returned from 'useState() or useReducer()', which should not be mutated. Use the setter or dispatcher function to update instead. Found mutation of `x` (5:5)
  6 |     setX(x);
  7 |   };
