        ...
        if (lambda != null) {
          ...
          for (const [
            ...
            if (!context.hasCalleeWithInsertedFire(fireCalleePlace)) {
              ...
              const {loadUseFireInstr, useFirePlace} =
                makeLoadUseFireInstruction(fn.env);
              const {loadFireCalleeInstr, loadedFireCallee} =
                makeLoadFireCalleeInstruction(
                  fn.env,
                  fireCalleeInfo.capturedCalleeIdentifier,
                );
              const {callUseFireInstr, useFireCallResultPlace} =
        ...
        if (value.args.length === 1 && value.args[0].kind === 'Identifier') {
          ...
          if (callExpr != null) {
            ...
            context.pushError({
              loc: value.loc,
              description:
                '`fire()` can only receive a `callExpression()`. Method calls and other expressions are not allowed',
...
function makeLoadUseFireInstruction(env: Environment): {
