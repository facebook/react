...
import { collectMutableOperands, mergeLocation } from "./InferReactiveScopeVariables";
...
type CurrentScope = null | { kind: "pending", id: number } | { kind: "available", scope: ReactiveScope, id: number };

function visitBlock(fn: HIRFunction, block: BasicBlock, scope: CurrentScope, seen: Map<number, CurrentScope>) {
  const visited = seen.get(block.id);
  if (visited === undefined) {
    seen.set(block.id, scope);
  } else {
    CompilerError.invariant(visited === null ? scope === null : visited.id === scope?.id, {
      reason:
  ...
  function extend(currentScope: ReactiveScope, operands: Iterable<Identifier>) {
    ...
    if (instruction.value.kind === "StartMemoize") {
      ...
      if (operands.length > 0) {
        if (currentScope.kind === "pending") {
          currentScope = { kind: "available", id: currentScope.id, scope: {
