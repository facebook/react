...
export function flattenScopesWithHooksOrUseHIR(fn: HIRFunction): void {
  ...
  for (const id of prune) {
    ...
    if (
      ...
      // Note that scope blocks are weird in that they represent two nested goto-labels
      // (see createReactiveFunction)
      // TO simplify flattening / label rewrite logic, we entirely remove empty dependency arrays here
