...
/**
 * How deeply does the macro require its arguments to be inlined?
 * Shallow means that only the top-level expressions must be inlined:
 *
 * ```
 * macro(foo(bar()))
 * => shallow inline =>
 ...
 * ```
 *
 * While transitive means that everything below must be inlined:
 *
 * ```
 * <fbt>
 *   <outer>
 *     <inner>
 *       <innerinner>
 *         Text // inline all the way down to this
 *       </innerinner>
 *     </inner>
 *   </outer>
 * </fbt>
 * ```
 *
 * However, a shallow inlined macro acts as a stopping point for
 * inlining of a parent transitive one. So for example we have
 * to inline from <fbt> "downward" but switch to shallow mode
 * for any nested <fbt:param>
 */
enum InlineLevel {
    ...
    for (let i = block.instructions.length - 1; i >= 0; i--) {
      ...
      switch (value.kind) {
        ...
        case 'JsxExpression': {
          ...
          let macroDefinition = macroTags.get(lvalue.identifier.id);
          if (macroDefinition == null) {
