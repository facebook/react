...
export function buildDependencyInstructions(
  ...
  if (dep.path.every(path => !path.optional)) {
    ...
    const last = writeOptionalDependency(dep, builder, null);
    result = {
      kind: 'Identifier',
      identifier: last,
      effect: Effect.Freeze,
...
/**
 * Write a dependency into optional blocks if there is an `optional` anywhere
 * along its path.
 *
...
function writeOptionalDependency(
  ...
  CompilerError.invariant(
    dep.path.some(path => path.optional),
    {
      reason:
