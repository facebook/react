...
export function insertGatedFunctionDeclaration(
  ...
  if (referencedBeforeDeclaration) {
    const name =
      fnPath.node.type === 'FunctionDeclaration' ? fnPath.node.id?.name : null;
    CompilerError.throwTodo({
      reason: `Encountered a function used before its declaration, which breaks React Compiler's gating codegen due to hoisting`,
      description: `Rewrite the reference to ${name ?? 'this function'} to not rely on hoisting to fix this issue`,
