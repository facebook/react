...
function processFn(
  ...
  if (fn.node.body.type !== 'BlockStatement') {
    ...
    if (optIn.isErr()) {
       ...
       * was not tested or rolled out on this function. In that case, we should
       * fall back to the safest option which is to not optimize the function.
       */
  ...
  if (compileResult.kind === 'error') {
    ...
    return retryCompileFunction(fn, fnType, programContext);
  }
  ...
  const compiledFn = compileResult.compiledFn;
  programContext.logEvent({
    kind: 'CompileSuccess',
...
function getReactFunctionType(
  ...
  if (fn.node.body.type === 'BlockStatement') {
    ...
    if (optInDirectives.isOk() && optInDirectives.unwrap() !== null) {
