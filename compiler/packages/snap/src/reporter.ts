...
export function normalizeCodeBlankLines(snapshot: string): string {
  ...
  const filtered = lines.filter(line => {
    ...
    // Strip unused var declarations from babel-plugin-idx (e.g., `var _ref2;`)
    // These are generated by generateUidIdentifier and scope.push() in
    // babel-plugin-idx, but the actual references may use a different _refN.
    // The TS and Rust compilers interact differently with Babel's scope UID
    // counter, producing different _refN numbering for unused declarations.
    const match = line.match(/^\s*var (_ref\d*);$/);
    if (match) {
      const varName = match[1];
      // Check if this identifier is used anywhere else in the code
      const regex = new RegExp('\\b' + varName + '\\b');
      const otherLines = lines.filter(l => l !== line);
      const isUsed = otherLines.some(l => regex.test(l));
