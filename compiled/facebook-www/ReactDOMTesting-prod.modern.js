...
function use(usable) {
  if (null !== usable && "object" === typeof usable) {
    if ("function" === typeof usable.then) {
      var index$45 = thenableIndexCounter;
      thenableIndexCounter += 1;
      null === thenableState && (thenableState = []);
      usable = trackUsedThenable(thenableState, usable, index$45);
  ...
  for (var provider = fiber.return; null !== provider; ) {
    switch (provider.tag) {
      ...
      case 3:
        ...
        var root$51 = enqueueUpdate(provider, fiber, lane);
        if (null !== root$51) {
          var eventTime = requestEventTime();
          scheduleUpdateOnFiber(root$51, provider, lane, eventTime);
          entangleTransitions(root$51, provider, lane);
        }
...
var HooksDispatcherOnMount = {
  ...
  useSyncExternalStore: function (subscribe, getSnapshot, getServerSnapshot) {
    ...
    if (isHydrating) {
      ...
      var root$47 = workInProgressRoot;
      if (null === root$47) throw Error(formatProdErrorMessage(349));
      includesBlockingLane(root$47, renderLanes$1) ||
        pushStoreConsistencyCheck(fiber, getSnapshot, getServerSnapshot);
    }
    hook.memoizedState = getServerSnapshot;
    root$47 = { value: getServerSnapshot, getSnapshot: getSnapshot };
    hook.queue = root$47;
    mountEffect(subscribeToStore.bind(null, fiber, root$47, subscribe), [
      subscribe
    ]);
    fiber.flags |= 2048;
    pushEffect(
  ...
  if (didBailout)
    for (var child$100 = completedWork.child; null !== child$100; )
      (newChildLanes |= child$100.lanes | child$100.childLanes),
        (subtreeFlags |= child$100.subtreeFlags & 14680064),
        (subtreeFlags |= child$100.flags & 14680064),
    ...
    if (6 === didTimeout) markRootSuspended(root, lanes);
      ...
      if (
        ...
        if (2 === didTimeout) {
          ...
          var errorRetryLanes$169 = getLanesToRetrySynchronouslyOnError(
            root,
            errorRetryLanes
          );
          0 !== errorRetryLanes$169 &&
            ((lanes = errorRetryLanes$169),
            (didTimeout = recoverFromConcurrentError(
              root,
              errorRetryLanes,
              errorRetryLanes$169
      ...
      switch (didTimeout) {
        ...
        case 4:
          ...
          for (originallyAttemptedLanes = -1; 0 < lanes; )
            (errorRetryLanes$169 = 31 - clz32(lanes)),
              (errorRetryLanes = 1 << errorRetryLanes$169),
              (errorRetryLanes$169 = didTimeout[errorRetryLanes$169]),
              errorRetryLanes$169 > originallyAttemptedLanes &&
                (originallyAttemptedLanes = errorRetryLanes$169),
              (lanes &= ~errorRetryLanes);
              ...
              case 22:
                if (suspenseBoundary.mode & 1) {
                  ...
                  if (null === offscreenQueue) {
                    ...
                    var wakeables$56 = offscreenQueue.wakeables;
                    null === wakeables$56
                      ? (offscreenQueue.wakeables = new Set([wakeable]))
                      : wakeables$56.add(wakeable);
                  }
                  break;
                }
              default:
...
function commitRootImpl(
  ...
  if (0 !== (finishedWork.subtreeFlags & 15990) || transitions) {
    ...
    executionContext |= 4;
    ReactCurrentOwner.current = null;
    var shouldFireAfterActiveInstanceBlur$177 = commitBeforeMutationEffects(
      root,
      finishedWork
    );
    commitMutationEffectsOnFiber(finishedWork, root);
    shouldFireAfterActiveInstanceBlur$177 &&
      ((_enabled = !0),
...
function attemptSynchronousHydration(fiber) {
  switch (fiber.tag) {
    case 3:
      var root$180 = fiber.stateNode;
      if (root$180.current.memoizedState.isDehydrated) {
        var lanes = getHighestPriorityLanes(root$180.pendingLanes);
        0 !== lanes &&
...
if (canUseDOM) {
  ...
  if (canUseDOM) {
    var isSupported$jscomp$inline_1523 = "oninput" in document;
    if (!isSupported$jscomp$inline_1523) {
      var element$jscomp$inline_1524 = document.createElement("div");
      element$jscomp$inline_1524.setAttribute("oninput", "return;");
      isSupported$jscomp$inline_1523 =
        "function" === typeof element$jscomp$inline_1524.oninput;
    }
    JSCompiler_inline_result$jscomp$297 = isSupported$jscomp$inline_1523;
    ...
    if (resourceRoot)
      switch (as) {
        case "style":
          ...
          var key$197 = getStyleKey(href),
            precedence = options.precedence || "default",
            resource = as.get(key$197);
          if (resource) break;
          resource = resourceRoot.querySelector(
            getStylesheetSelectorFromKey(key$197)
          );
          resource ||
            ((href = {
              rel: "stylesheet",
            ...
            (options = preloadPropsMap.get(key$197)) &&
          ...
          as.set(key$197, resource);
          break;
        case "script":
          (as = getResourcesFromRoot(resourceRoot).hoistableScripts),
            (key$197 = getScriptKey(href)),
            (precedence = as.get(key$197)),
            precedence ||
              ((precedence = resourceRoot.querySelector(
                "script[async]" + key$197
              )),
              precedence ||
                ((href = {
                  src: href,
              ...
              as.set(key$197, precedence));
      }
    else if ("style" === as || "script" === as)
      if ((resourceRoot = getDocumentForPreloads())) {
        key$197 = escapeSelectorAttributeValueInsideDoubleQuotes(href);
        precedence = key$197 =
          'link[rel="preload"][as="' + as + '"][href="' + key$197 + '"]';
        switch (as) {
          case "style":
        ...
        preloadPropsMap.has(precedence) ||
          ...
          null === resourceRoot.querySelector(key$197) &&
  ...
  switch (type) {
    ...
    case "link":
      if (
        ...
        var key$204 = getStyleKey(pendingProps.href),
          styles$205 = getResourcesFromRoot(currentProps).hoistableStyles;
        type = styles$205.get(key$204);
        type ||
          ((currentProps = currentProps.ownerDocument || currentProps),
          (type = { type: "stylesheet", instance: null, count: 0 }),
          styles$205.set(key$204, type),
          preloadPropsMap.has(key$204) ||
            ((pendingProps = {
              rel: "preload",
              as: "style",
            ...
            preloadPropsMap.set(key$204, pendingProps),
            currentProps.querySelector(getStylesheetSelectorFromKey(key$204)) ||
              null !==
                currentProps.querySelector(
                  'link[rel="preload"][as="style"][' + key$204 + "]"
                ) ||
              ((key$204 = currentProps.createElement("link")),
              setInitialProperties(key$204, "link", pendingProps),
              markNodeAsHoistable(key$204),
              currentProps.head.appendChild(key$204))));
        return type;
      }
      return null;
    case "script":
  ...
  if (null === resource.instance)
    switch (resource.type) {
      ...
      case "stylesheet":
        ...
        var instance$211 = hoistableRoot.querySelector(
          getStylesheetSelectorFromKey(styleProps)
        );
        if (instance$211)
          return (
            (resource.instance = instance$211),
            markNodeAsHoistable(instance$211),
            instance$211
          );
        key = assign({}, props, {
          "data-precedence": props.precedence,
        ...
        setInitialProperties(instance$211, "link", key);
        insertStylesheet(instance$211, props.precedence, hoistableRoot);
        return (resource.instance = instance$211);
      case "script":
        instance$211 = getScriptKey(props.src);
        if (
          (styleProps = hoistableRoot.querySelector(
            "script[async]" + instance$211
          ))
        )
          return (
        ...
        key = props;
        if ((styleProps = preloadPropsMap.get(instance$211)))
          (key = assign({}, props)),
...
var devToolsConfig$jscomp$inline_1743 = {
  ...
  bundleType: 0,
  version: "18.3.0-www-modern-619d6df2",
...
var internals$jscomp$inline_2147 = {
  ...
  reconcilerVersion: "18.3.0-www-modern-619d6df2"
};
...
exports.version = "18.3.0-www-modern-619d6df2";
