...
function getLaneLabelMap() {
  if (enableSchedulingProfiler) {
    for (var map = new Map(), lane = 1, index$9 = 0; 31 > index$9; index$9++) {
...
function movePendingFibersToMemoized(root, lanes) {
  if (isDevToolsPresent)
    for (
      ...
      root = 1 << index$17;
      index$17 = pendingUpdatersLaneMap[index$17];
      0 < index$17.size &&
        (index$17.forEach(function(fiber) {
          var alternate = fiber.alternate;
...
function getTransitionsForLanes(root, lanes) {
  ...
  for (var transitionsForLanes = []; 0 < lanes; ) {
    var index$19 = 31 - clz32(lanes),
      lane = 1 << index$19;
    index$19 = root.transitionLanes[index$19];
    null !== index$19 &&
      index$19.forEach(function(transition) {
  ...
  if (enableTransitionTracing)
    for (; 0 < lanes; ) {
      var index$20 = 31 - clz32(lanes),
        lane = 1 << index$20;
      null !== root.transitionLanes[index$20] &&
        (root.transitionLanes[index$20] = null);
      lanes &= ~lane;
    }
}
...
function acquireResource(resource) {
  switch (resource.type) {
    ...
    case "meta":
      a: {
        ...
        if (!instance) {
          var props = resource.props,
            root$58 = resource.root,
            type = resource.type;
          switch (type) {
            case "title":
              var titles = root$58.querySelectorAll("title");
            ...
            case "meta":
              ...
              } else if ((instance = root$58.querySelector(matcher))) {
              ...
              insertResourceInstanceBefore(root$58, instance, titles);
              break;
            case "link":
              ...
              if ((titles = root$58.querySelector(titles))) {
              ...
              insertResourceInstanceBefore(root$58, instance, null);
              break;
            case "base":
              (titles = root$58.querySelector(resource.matcher))
                ? ((instance = resource.instance = titles),
                  (instance[internalResourceMarker] = !0))
                : ((instance = resource.instance = createResourceInstance(
                    type,
      ...
      return resource;
    case "style":
      root$58 = resource.instance;
      if (!root$58)
        if (
          ((props = resource.root),
          (type = resource.precedence),
          (root$58 = escapeSelectorAttributeValueInsideDoubleQuotes(
            resource.props.href
          )),
          (titles = props.querySelector(
            'link[rel="stylesheet"][data-precedence][href="' + root$58 + '"]'
          )))
        )
          if (
            ((root$58 = resource.instance = titles),
            (root$58[internalResourceMarker] = !0),
          ...
          root$58 = resource.instance = createResourceInstance(
            "link",
            resource.props,
            getDocumentFromRoot(props)
          );
          attachLoadListeners(root$58, resource);
          titles = root$58;
...
function isHostResourceType(type, props, hostContext) {
  switch (type) {
    ...
    case "script":
      ...
      var onError$66 = props.onError;
      return (
        props.async && "string" === typeof type && !hostContext && !onError$66
      );
  }
  return !1;
}
...
function use(usable) {
  if (null !== usable && "object" === typeof usable) {
    if ("function" === typeof usable.then) {
      var index$95 = thenableIndexCounter;
      thenableIndexCounter += 1;
      null === thenableState && (thenableState = []);
      return trackUsedThenable(thenableState, usable, index$95);
    }
    if (
  ...
  for (var provider = fiber.return; null !== provider; ) {
    switch (provider.tag) {
      ...
      case 3:
        ...
        var root$101 = enqueueUpdate$1(provider, fiber, lane);
        null !== root$101 &&
          (scheduleUpdateOnFiber(root$101, provider, lane, eventTime),
          entangleTransitions(root$101, provider, lane));
...
var HooksDispatcherOnMount = {
  ...
  useSyncExternalStore: function(subscribe, getSnapshot, getServerSnapshot) {
    ...
    if (isHydrating) {
      ...
      var root$97 = workInProgressRoot;
      if (null === root$97) throw Error(formatProdErrorMessage(349));
      includesBlockingLane(root$97, renderLanes) ||
        pushStoreConsistencyCheck(fiber, getSnapshot, getServerSnapshot);
    }
    hook.memoizedState = getServerSnapshot;
    root$97 = { value: getServerSnapshot, getSnapshot: getSnapshot };
    hook.queue = root$97;
    mountEffect(subscribeToStore.bind(null, fiber, root$97, subscribe), [
      subscribe
    ]);
    fiber.flags |= 1024;
    pushEffect(
...
function pushRootMarkerInstance(workInProgress) {
  if (enableTransitionTracing) {
    var transitions = workInProgressTransitions,
      root$114 = workInProgress.stateNode;
    null !== transitions &&
      transitions.forEach(function(transition) {
        if (!root$114.incompleteTransitions.has(transition)) {
          var markerInstance = {
            tag: 0,
          ...
          root$114.incompleteTransitions.set(transition, markerInstance);
        }
      });
    var markerInstances = [];
    root$114.incompleteTransitions.forEach(function(markerInstance) {
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (null !== JSCompiler_temp) {
    var dehydrated$121 = JSCompiler_temp.dehydrated;
    if (null !== dehydrated$121)
  ...
  if (showFallback) {
    ...
    dehydrated$121 = JSCompiler_temp.sibling;
    ...
    null !== dehydrated$121
      ? (showFallback = createWorkInProgress(dehydrated$121, showFallback))
    ...
    null === didSuspend
      ...
      : ((JSCompiler_temp = didSuspend.cachePool),
        null !== JSCompiler_temp
          ? ((dehydrated$121 = CacheContext._currentValue),
            (JSCompiler_temp =
              JSCompiler_temp.parent !== dehydrated$121
                ? { parent: dehydrated$121, pool: dehydrated$121 }
                : JSCompiler_temp))
    ...
    enableTransitionTracing &&
      ...
      null !== didSuspend &&
        ((JSCompiler_temp = enableTransitionTracing
          ? markerInstanceStack.current
          : null),
        (dehydrated$121 = showFallback.updateQueue),
        ...
        null === dehydrated$121
          ...
          : dehydrated$121 === primaryChildProps
          ? (showFallback.updateQueue = {
              transitions: didSuspend,
              markerInstances: JSCompiler_temp,
              wakeables:
          ...
          : ((dehydrated$121.transitions = didSuspend),
            (dehydrated$121.markerInstances = JSCompiler_temp))));
    ...
    if (0 !== (completedWork.mode & 2)) {
      for (
        ...
        (newChildLanes |= child$155.lanes | child$155.childLanes),
          (subtreeFlags |= child$155.subtreeFlags & 7340032),
          (subtreeFlags |= child$155.flags & 7340032),
      ...
      for (
        ...
        (newChildLanes |=
          ...
          (subtreeFlags |= treeBaseDuration$154.subtreeFlags & 7340032),
          (subtreeFlags |= treeBaseDuration$154.flags & 7340032),
              ...
              case 22:
                if (suspenseBoundary.mode & 1) {
                  ...
                  if (null === offscreenQueue) {
                    ...
                    var wakeables$109 = offscreenQueue.wakeables;
                    null === wakeables$109
                      ? (offscreenQueue.wakeables = new Set([wakeable]))
                      : wakeables$109.add(wakeable);
                  }
                  break;
                }
              default:
...
var devToolsConfig$jscomp$inline_1815 = {
  ...
  bundleType: 0,
  version: "18.3.0-www-modern-62e2b1374-20221121",
