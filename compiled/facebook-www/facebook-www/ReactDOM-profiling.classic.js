...
function getLaneLabelMap() {
  if (enableSchedulingProfiler) {
    for (
      var map = new Map(), lane = 1, index$10 = 0;
      31 > index$10;
      index$10++
    ) {
...
function markStarvedLanesAsExpired(root, currentTime) {
  for (
    ...
    var index$13 = 31 - clz32(lanes),
      lane = 1 << index$13,
      expirationTime = expirationTimes[index$13];
    if (-1 === expirationTime) {
      if (0 === (lane & suspendedLanes) || 0 !== (lane & pingedLanes))
        expirationTimes[index$13] = computeExpirationTime(lane, currentTime);
...
function markRootFinished(root, remainingLanes) {
  ...
  for (root = root.hiddenUpdates; 0 < noLongerPendingLanes; ) {
    var index$15 = 31 - clz32(noLongerPendingLanes),
      lane = 1 << index$15;
    remainingLanes[index$15] = 0;
    eventTimes[index$15] = -1;
    expirationTimes[index$15] = -1;
    var hiddenUpdatesForLane = root[index$15];
    if (null !== hiddenUpdatesForLane)
      for (
        root[index$15] = null, index$15 = 0;
        index$15 < hiddenUpdatesForLane.length;
        index$15++
      ) {
        var update = hiddenUpdatesForLane[index$15];
...
function markRootEntangled(root, entangledLanes) {
  ...
  for (root = root.entanglements; rootEntangledLanes; ) {
    var index$16 = 31 - clz32(rootEntangledLanes),
      lane = 1 << index$16;
    (lane & entangledLanes) | (root[index$16] & entangledLanes) &&
      (root[index$16] |= entangledLanes);
...
function movePendingFibersToMemoized(root, lanes) {
  if (isDevToolsPresent)
    for (
      ...
      root = 1 << index$18;
      index$18 = pendingUpdatersLaneMap[index$18];
      0 < index$18.size &&
        (index$18.forEach(function(fiber) {
          var alternate = fiber.alternate;
...
function getTransitionsForLanes(root, lanes) {
  ...
  for (var transitionsForLanes = []; 0 < lanes; ) {
    var index$20 = 31 - clz32(lanes),
      lane = 1 << index$20;
    index$20 = root.transitionLanes[index$20];
    null !== index$20 &&
      index$20.forEach(function(transition) {
...
function clearTransitionsForLanes(root, lanes) {
  if (enableTransitionTracing)
    for (; 0 < lanes; ) {
      var index$21 = 31 - clz32(lanes),
        lane = 1 << index$21;
      null !== root.transitionLanes[index$21] &&
        (root.transitionLanes[index$21] = null);
      lanes &= ~lane;
    }
}
...
function acquireResource(resource) {
  switch (resource.type) {
    ...
    case "meta":
      a: {
        ...
        if (!instance) {
          var props = resource.props,
            root$43 = resource.root,
            type = resource.type;
          switch (type) {
            case "title":
              var titles = root$43.querySelectorAll("title");
            ...
            case "meta":
              ...
              } else if ((instance = root$43.querySelector(matcher))) {
              ...
              insertResourceInstanceBefore(root$43, instance, titles);
              break;
            case "link":
              ...
              if ((titles = root$43.querySelector(titles))) {
              ...
              insertResourceInstanceBefore(root$43, instance, null);
              break;
            case "base":
              (titles = root$43.querySelector(resource.matcher))
                ? ((instance = resource.instance = titles),
                  (instance[internalResourceMarker] = !0))
                : ((instance = resource.instance = createResourceInstance(
                    type,
      ...
      return resource;
    case "style":
      root$43 = resource.instance;
      if (!root$43)
        if (
          ((props = resource.root),
          (type = resource.precedence),
          (root$43 = escapeSelectorAttributeValueInsideDoubleQuotes(
            resource.props.href
          )),
          (titles = props.querySelector(
            'link[rel="stylesheet"][data-precedence][href="' + root$43 + '"]'
          )))
        )
          if (
            ((root$43 = resource.instance = titles),
            (root$43[internalResourceMarker] = !0),
          ...
          root$43 = resource.instance = createResourceInstance(
            "link",
            resource.props,
            getDocumentFromRoot(props)
          );
          attachLoadListeners(root$43, resource);
          titles = root$43;
...
function isHostResourceType(type, props, hostContext) {
  switch (type) {
    ...
    case "script":
      ...
      var onError$51 = props.onError;
      return (
        props.async && "string" === typeof type && !hostContext && !onError$51
      );
  }
  return !1;
}
...
function use(usable) {
  if (null !== usable && "object" === typeof usable) {
    if ("function" === typeof usable.then) {
      var index$94 = thenableIndexCounter;
      thenableIndexCounter += 1;
      null === thenableState && (thenableState = []);
      return trackUsedThenable(thenableState, usable, index$94);
    }
    if (
  ...
  for (var provider = fiber.return; null !== provider; ) {
    switch (provider.tag) {
      ...
      case 3:
        ...
        var root$100 = enqueueUpdate$1(provider, fiber, lane);
        null !== root$100 &&
          (scheduleUpdateOnFiber(root$100, provider, lane, eventTime),
          entangleTransitions(root$100, provider, lane));
...
var HooksDispatcherOnMount = {
  ...
  useSyncExternalStore: function(subscribe, getSnapshot, getServerSnapshot) {
    ...
    if (isHydrating) {
      ...
      var root$96 = workInProgressRoot;
      if (null === root$96) throw Error(formatProdErrorMessage(349));
      includesBlockingLane(root$96, renderLanes) ||
        pushStoreConsistencyCheck(fiber, getSnapshot, getServerSnapshot);
    }
    hook.memoizedState = getServerSnapshot;
    root$96 = { value: getServerSnapshot, getSnapshot: getSnapshot };
    hook.queue = root$96;
    mountEffect(subscribeToStore.bind(null, fiber, root$96, subscribe), [
      subscribe
    ]);
    fiber.flags |= 1024;
    pushEffect(
...
function pushRootMarkerInstance(workInProgress) {
  if (enableTransitionTracing) {
    var transitions = workInProgressTransitions,
      root$113 = workInProgress.stateNode;
    null !== transitions &&
      transitions.forEach(function(transition) {
        if (!root$113.incompleteTransitions.has(transition)) {
          var markerInstance = {
            tag: 0,
          ...
          root$113.incompleteTransitions.set(transition, markerInstance);
        }
      });
    var markerInstances = [];
    root$113.incompleteTransitions.forEach(function(markerInstance) {
...
function updateSuspenseComponent(current, workInProgress, renderLanes) {
  ...
  if (null !== JSCompiler_temp) {
    var dehydrated$120 = JSCompiler_temp.dehydrated;
    if (null !== dehydrated$120)
  ...
  if (showFallback) {
    ...
    dehydrated$120 = JSCompiler_temp.sibling;
    ...
    null !== dehydrated$120
      ? (showFallback = createWorkInProgress(dehydrated$120, showFallback))
    ...
    null === didSuspend
      ...
      : ((JSCompiler_temp = didSuspend.cachePool),
        null !== JSCompiler_temp
          ? ((dehydrated$120 = CacheContext._currentValue),
            (JSCompiler_temp =
              JSCompiler_temp.parent !== dehydrated$120
                ? { parent: dehydrated$120, pool: dehydrated$120 }
                : JSCompiler_temp))
    ...
    enableTransitionTracing &&
      ...
      null !== didSuspend &&
        ((JSCompiler_temp = enableTransitionTracing
          ? markerInstanceStack.current
          : null),
        (dehydrated$120 = showFallback.updateQueue),
        ...
        null === dehydrated$120
          ...
          : dehydrated$120 === primaryChildProps
          ? (showFallback.updateQueue = {
              transitions: didSuspend,
              markerInstances: JSCompiler_temp,
              wakeables:
          ...
          : ((dehydrated$120.transitions = didSuspend),
            (dehydrated$120.markerInstances = JSCompiler_temp))));
  ...
  if (!isHydrating)
    switch (renderState.tailMode) {
      ...
      case "collapsed":
        ...
        for (var lastTailNode$151 = null; null !== lastTailNode; )
          null !== lastTailNode.alternate && (lastTailNode$151 = lastTailNode),
            (lastTailNode = lastTailNode.sibling);
        null === lastTailNode$151
          ? hasRenderedATailFallback || null === renderState.tail
            ? (renderState.tail = null)
            : (renderState.tail.sibling = null)
          : (lastTailNode$151.sibling = null);
    }
}
function bubbleProperties(completedWork) {
  ...
  if (didBailout)
    if (0 !== (completedWork.mode & 2)) {
      for (
        var treeBaseDuration$153 = completedWork.selfBaseDuration,
          child$154 = completedWork.child;
        null !== child$154;

      )
        (newChildLanes |= child$154.lanes | child$154.childLanes),
          (subtreeFlags |= child$154.subtreeFlags & 7340032),
          (subtreeFlags |= child$154.flags & 7340032),
          (treeBaseDuration$153 += child$154.treeBaseDuration),
          (child$154 = child$154.sibling);
      completedWork.treeBaseDuration = treeBaseDuration$153;
    } else
      for (
        treeBaseDuration$153 = completedWork.child;
        null !== treeBaseDuration$153;

      )
        (newChildLanes |=
          treeBaseDuration$153.lanes | treeBaseDuration$153.childLanes),
          (subtreeFlags |= treeBaseDuration$153.subtreeFlags & 7340032),
          (subtreeFlags |= treeBaseDuration$153.flags & 7340032),
          (treeBaseDuration$153.return = completedWork),
          (treeBaseDuration$153 = treeBaseDuration$153.sibling);
  else if (0 !== (completedWork.mode & 2)) {
    treeBaseDuration$153 = completedWork.actualDuration;
    child$154 = completedWork.selfBaseDuration;
    for (var child = completedWork.child; null !== child; )
      (newChildLanes |= child.lanes | child.childLanes),
        (subtreeFlags |= child.subtreeFlags),
        (subtreeFlags |= child.flags),
        (treeBaseDuration$153 += child.actualDuration),
        (child$154 += child.treeBaseDuration),
        (child = child.sibling);
    completedWork.actualDuration = treeBaseDuration$153;
    completedWork.treeBaseDuration = child$154;
  } else
    for (
      treeBaseDuration$153 = completedWork.child;
      null !== treeBaseDuration$153;
      ...
      (newChildLanes |=
        treeBaseDuration$153.lanes | treeBaseDuration$153.childLanes),
        (subtreeFlags |= treeBaseDuration$153.subtreeFlags),
        (subtreeFlags |= treeBaseDuration$153.flags),
        (treeBaseDuration$153.return = completedWork),
        (treeBaseDuration$153 = treeBaseDuration$153.sibling);
  ...
  switch (workInProgress.tag) {
    case 1:
      return (
        ...
        current & 32768
          ? ((workInProgress.flags = (current & -32769) | 64),
            0 !== (workInProgress.mode & 2) &&
              transferActualDuration(workInProgress),
            workInProgress)
          : null
...
function disconnectPassiveEffect(finishedWork) {
  switch (finishedWork.tag) {
    ...
    case 11:
    case 15:
      commitHookPassiveUnmountEffects(finishedWork, finishedWork.return, 8);
      recursivelyTraverseDisconnectPassiveEffects(finishedWork);
      break;
    case 22:
      var instance = finishedWork.stateNode;
...
function scheduleUpdateOnFiber(root, fiber, lane, eventTime) {
  ...
  if (0 === (executionContext & 2) || root !== workInProgressRoot) {
    ...
    if (
      enableTransitionTracing &&
      ((current = ReactCurrentBatchConfig$3.transition),
      null !== current &&
        null != current.name &&
        (-1 === current.startTime && (current.startTime = now()),
        enableTransitionTracing))
    ) {
...
function markRootSuspended$1(root, suspendedLanes) {
  ...
  for (root = root.expirationTimes; 0 < suspendedLanes; ) {
    var index$14 = 31 - clz32(suspendedLanes),
      lane = 1 << index$14;
    root[index$14] = -1;
    suspendedLanes &= ~lane;
  }
              ...
              case 22:
                if (suspenseBoundary.mode & 1) {
                  ...
                  if (null === offscreenQueue) {
                    ...
                    var wakeables$108 = offscreenQueue.wakeables;
                    null === wakeables$108
                      ? (offscreenQueue.wakeables = new Set([wakeable]))
                      : wakeables$108.add(wakeable);
                  }
                  break;
                }
              default:
          ...
          : (0 !== (current.lanes & renderLanes) &&
            ...
            context.parent !== Component
              ? ((context = { parent: Component, cache: Component }),
                ...
                0 === workInProgress.lanes &&
                  (workInProgress.memoizedState = workInProgress.updateQueue.baseState = context),
                pushProvider(workInProgress, CacheContext, Component))
              : ((Component = prevState.cache),
                pushProvider(workInProgress, CacheContext, Component),
...
function createHydrationContainer(
  ...
  enqueueUpdate$1(containerInfo, isStrictMode, hydrationCallbacks);
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  element = enqueueUpdate$1(current, container, lane);
  null !== element &&
  ...
  switch (fiber.tag) {
    case 3:
      ...
      if (root$247.current.memoizedState.isDehydrated) {
        ...
        0 !== lanes &&
          (markRootEntangled(root$247, lanes | 1),
...
var devToolsConfig$jscomp$inline_1847 = {
  ...
  bundleType: 0,
  version: "18.3.0-www-classic-62e2b1374-20221121",
...
(function(internals) {
  ...
  reconcilerVersion: "18.3.0-next-62e2b1374-20221121"
});
assign(Internals, {
...
exports.version = "18.3.0-next-62e2b1374-20221121";
