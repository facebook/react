...
var enableDebugTracing = dynamicFeatureFlags.enableDebugTracing,
    ...
    enableDeferRootSchedulingToMicrotask = dynamicFeatureFlags.enableDeferRootSchedulingToMicrotask,
    alwaysThrottleRetries = dynamicFeatureFlags.alwaysThrottleRetries,
    enableDO_NOT_USE_disableStrictPassiveEffect = dynamicFeatureFlags.enableDO_NOT_USE_disableStrictPassiveEffect,
...
function ensureRootIsScheduled(root) {
  ...
  if (!disableLegacyMode && ReactSharedInternals.isBatchingLegacy && root.tag === LegacyRoot) {
...
function flushSyncWorkAcrossRoots_impl(onlyLegacy) {
  ...
  do {
    ...
    while (root !== null) {
      if (onlyLegacy && (disableLegacyMode || root.tag !== LegacyRoot)) ; else {
...
function pushPrimaryTreeSuspenseHandler(handler) {
  ...
  if (props.unstable_avoidThisFallback === true && ( // If an avoided boundary is already visible, it behaves identically to
...
function updateMemoComponent(current, workInProgress, Component, nextProps, renderLanes) {
  if (current === null) {
    ...
    disableDefaultPropsExceptForClasses || Component.defaultProps === undefined)) {
  ...
  if (current === null) {
  ...
  } else {
    {
      if (current.memoizedProps.name !== workInProgress.pendingProps.name) {
        error('Changing the name of a tracing marker after mount is not supported. ' + 'To remount the tracing marker, pass it a new key.');
      }
    }
  }
...
function mountLazyComponent(_current, workInProgress, elementType, renderLanes) {
  ...
  if (typeof Component === 'function') {
    if (isFunctionClassComponent(Component)) {
      ...
      var _resolvedProps = disableDefaultPropsExceptForClasses ? props : resolveDefaultPropsOnNonClassComponent(Component, props);
    ...
    if ($$typeof === REACT_FORWARD_REF_TYPE) {
      var _resolvedProps2 = disableDefaultPropsExceptForClasses ? props : resolveDefaultPropsOnNonClassComponent(Component, props);
      ...
      var _resolvedProps3 = disableDefaultPropsExceptForClasses ? props : resolveDefaultPropsOnNonClassComponent(Component, props);

      workInProgress.tag = MemoComponent;
      return updateMemoComponent(null, workInProgress, Component, disableDefaultPropsExceptForClasses ? _resolvedProps3 : resolveDefaultPropsOnNonClassComponent(Component.type, _resolvedProps3), // The inner type can have defaults too
      renderLanes);
    }
  }
    ...
    case FunctionComponent:
      {
        ...
        var resolvedProps = disableDefaultPropsExceptForClasses || workInProgress.elementType === Component ? unresolvedProps : resolveDefaultPropsOnNonClassComponent(Component, unresolvedProps);
    ...
    case ForwardRef:
      {
        ...
        var _resolvedProps5 = disableDefaultPropsExceptForClasses || workInProgress.elementType === type ? _unresolvedProps2 : resolveDefaultPropsOnNonClassComponent(type, _unresolvedProps2);
    ...
    case MemoComponent:
      {
        ...
        var _resolvedProps6 = disableDefaultPropsExceptForClasses ? _unresolvedProps3 : resolveDefaultPropsOnNonClassComponent(_type, _unresolvedProps3);

        _resolvedProps6 = disableDefaultPropsExceptForClasses ? _resolvedProps6 : resolveDefaultPropsOnNonClassComponent(_type.type, _resolvedProps6);
    ...
    case HostComponent:
      {
        ...
        if (current !== null && workInProgress.stateNode != null) {
          ...
          getHostContext(); // TODO: Move createInstance to beginWork and keep it on a context
...
function commitLayoutEffectOnFiber(finishedRoot, current, finishedWork, committedLanes) {
  ...
  switch (finishedWork.tag) {
    ...
    case OffscreenComponent:
      {
        var isModernRoot = disableLegacyMode || (finishedWork.mode & ConcurrentMode) !== NoMode;
    ...
    while (fiber !== null) {
      switch (fiber.tag) {
        ...
        case HostRoot:
          ...
          abortRootTransitions(root, abort, deletedTransitions, deletedOffscreenInstance);
          break;
      }
    ...
    case TracingMarkerComponent:
      {
        if (enableTransitionTracing) {
          ...
          if (_transitions !== null) {
            ...
            abortParentMarkerTransitionsForDeletedFiber(current, _abortReason, _transitions, null, true);

            if (nearestMountedAncestor !== null) {
              abortParentMarkerTransitionsForDeletedFiber(nearestMountedAncestor, _abortReason, _transitions, null, false);
            }
          }
        }
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  var shouldTimeSlice = !includesBlockingLane(root, lanes) && !includesExpiredLane(root, lanes) && (disableSchedulerTimeoutInWorkLoop || !didTimeout);
    ...
    case FunctionComponent:
      {
        ...
        var resolvedProps = disableDefaultPropsExceptForClasses || unitOfWork.elementType === Component ? unresolvedProps : resolveDefaultPropsOnNonClassComponent(Component, unresolvedProps);
        var context;
    ...
    case ForwardRef:
      {
        ...
        var _resolvedProps = disableDefaultPropsExceptForClasses || unitOfWork.elementType === _Component ? _unresolvedProps : resolveDefaultPropsOnNonClassComponent(_Component, _unresolvedProps);
...
function commitRootImpl(root, recoverableErrors, transitions, didIncludeRenderPhaseUpdate, renderPriorityLevel, spawnedLane) {
  ...
  if (includesSyncLane(pendingPassiveEffectsLanes) && (disableLegacyMode || root.tag !== LegacyRoot)) {
    flushPassiveEffects();
...
function commitDoubleInvokeEffectsInDEV(root, hasPassiveEffects) {
  {
    if ((disableLegacyMode || root.tag !== LegacyRoot)) {
      ...
      if ((disableLegacyMode || root.tag === ConcurrentRoot) && !(root.current.mode & (StrictLegacyMode | StrictEffectsMode))) {
...
function warnIfSuspenseResolutionNotWrappedWithActDEV(root) {
  {
    if ((disableLegacyMode || root.tag !== LegacyRoot) && isConcurrentActEnvironment() && ReactSharedInternals.actQueue === null) {
...
var Surface = /*#__PURE__*/function (_React$Component) {
  ...
  _proto4.componentDidMount = function componentDidMount() {
    ...
    this._mountNode = createContainer(this._surface, disableLegacyMode ? ConcurrentRoot : LegacyRoot, null, false, false, ''); // We synchronously flush updates coming from above so that they commit together
