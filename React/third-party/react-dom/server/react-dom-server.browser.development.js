...
define('react-dom/server', ['exports', 'react'], function (exports, React) { 'use strict';
...

      {
        if ('contextType' in type) {
          ...
          if (!isValid && !didWarnAboutInvalidateContextType.has(type)) {
            ...
            console.error('%s defines an invalid contextType. ' + 'contextType should point to the Context object returned by React.createContext().%s', getComponentName(type) || 'Component', addendum);
          }
        }
      }
  ...
  function useLayoutEffect(create, inputs) {
    {
      ...
      console.error('useLayoutEffect does nothing on the server, because its effect cannot ' + "be encoded into the server renderer's output format. This will lead " + 'to a mismatch between the initial, non-hydrated UI and the intended ' + 'UI. To avoid this, useLayoutEffect should only be used in ' + 'components that render exclusively on the client. ' + 'See https://reactjs.org/link/uselayouteffect-ssr for common fixes.');
    }
  }
  ...
  function warnNoop(publicInstance, callerName) {
    {
      ...
      console.error('%s(...): Can only update a mounting component. ' + 'This usually means you called %s() outside componentWillMount() on the server. ' + 'This is a no-op.\n\nPlease check the code for the %s component.', callerName, callerName, componentName);
...

          {
            if (partialState === undefined) {
              ...
              if (!didWarnAboutUndefinedDerivedState[_componentName]) {
                console.error('%s.getDerivedStateFromProps(): A valid state object (or null) must be returned. ' + 'You have returned undefined.', _componentName);
      ...
      } else {
        {
          if (Component.prototype && typeof Component.prototype.render === 'function') {
            ...
            if (!didWarnAboutBadClass[_componentName2]) {
              console.error("The <%s /> component appears to have a render method, but doesn't extend React.Component. " + 'This is likely to cause errors. Change %s to extend React.Component instead.', _componentName2, _componentName2);
        ...
        if (typeof inst.componentWillMount === 'function') {
          {
            if ( inst.componentWillMount.__suppressDeprecationWarning !== true) {
              ...
              if (!didWarnAboutDeprecatedWillMount[_componentName4]) {
                console.warn( // keep this warning in sync with ReactStrictModeWarning.js
...

      {
        if (typeof inst.getChildContext === 'function') {
          ...
          if (typeof _childContextTypes === 'object') {
            ...
            {
              console.error('%s.getChildContext(): childContextTypes must be defined in order to ' + 'use getChildContext().', getComponentName(Component) || 'Unknown');
            }
          }
        }
            ...
            case REACT_MEMO_TYPE:
              {
                ...
                var _nextChildren6 = [React.createElement(elementType.type, Object.assign({
                  ref: _element.ref
                }, _element.props))];
                var _frame6 = {
                  type: null,
            ...
            case REACT_LAZY_TYPE:
              {
                ...
                var _nextChildren10 = [React.createElement(result, Object.assign({
                  ref: _element2.ref
                }, _element2.props))];
                var _frame10 = {
                  type: null,
    ...
    _proto.renderDOM = function renderDOM(element, context, parentNamespace) {
...

      {
        if (namespace === Namespaces.html) {
          ...
          // allow <SVG> or <mATH>.
          if (tag !== element.type) {
            console.error('<%s /> is using incorrect casing. ' + 'Use PascalCase for React components, ' + 'or lowercase for HTML elements.', element.type);
          }
        }
      }
