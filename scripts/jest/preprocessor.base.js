...
function createPreprocessor(isDev) {
  ...
  return {
    process: function (src, filePath) {
      ...
      if (!filePath.match(/\/third_party\//)) {
        ...
        if (isDev && !isInDevToolsPackages) {
          sourceOnlyPlugins.push(pathToBabelPluginReplaceConsoleCalls);
        }
        const plugins = (
          isTestFile ? testOnlyPlugins : sourceOnlyPlugins
    ...
    getCacheKey: createCacheKeyFunction(
      ...
      [(process.env.REACT_VERSION != null).toString(), isDev.toString()]
    ),
  };
}
