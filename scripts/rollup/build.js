  ...
  const rollupConfig = {
    ...
    external(id) {
      ...
      if (!shouldBundleDependencies && isProvidedByDependency) {
        if (id.indexOf('/src/') !== -1) {
          throw Error(
            `You are trying to import ${id} but ${externals.find(containsThisModule)} is one of npm dependencies, so it will not contain that source file.
            You probably want to create a new bundle entry point for it instead.`
          );
        }
        return true;
      }
...
function handleRollupWarning(warning) {
  if (warning.code === 'UNUSED_EXTERNAL_IMPORT') {
    ...
    if (typeof importSideEffects[externalModule] !== 'boolean') {
      throw new Error(
        `An external module "${externalModule}" is used in a DEV-only code path but we do not know if it is safe to omit an unused require() to it in production.
