...
2. The release is then [**published to the `next` channel**](#publishing-release) using the [`download-release`](#download-release) and [`publish`](#publish) scripts. (Currently this process is manual but might be automated in the future using [GitHub "actions"](https://github.com/features/actions).)
...
5. Run the [`download-release`](#download-release) script with the build ID <sup>2</sup> you found:
```sh
scripts/release/download-release.js --build=124756
```
...
```sh
scripts/release/download-release.js --build=124763
```
...
To prepare a stable release, choose a "next" version and run the [`promote-to-stable`](#promote-to-stable) script <sup>1</sup>:

```sh
scripts/release/promote-to-stable.js --version=0.0.0-241c4467e
```
