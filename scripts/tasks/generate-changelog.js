...
async function summarizePackageCommits({
  ...
  const targetList = allPackageSpecs.map(targetSpec =>
  ...
  log(
    `Summaries available for ${summaries.size} commit(s) for ${spec.name}.`
  );
...
async function main() {
  ...
  for (let i = 0; i < packageSpecs.length; i++) {
    ...
    commitsForPackage.forEach(commit => {
      ...
      const summary = summaryMap.get(commit.sha) || commit.subject;
