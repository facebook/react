(() => {
  ...
  }, abort_controller_getSignal = () => abortController.signal, browser = __webpack_require__.g.browser || __webpack_require__.g.chrome;
  ...
  const isWebWorkerContext = () => "function" == typeof importScripts && !getContextName().includes("background");
  isWebWorkerContext() ? self : window;
  var util, objectUtil;
  !function(util) {
  ...
  class CyberhavenDomEvents {
    ...
    async listenDomEvents() {
      ...
      const signal = abort_controller_getSignal(), eventOpts = {
        capture: !0,
        signal
      };
  ...
  function getAccountEmail() {
    try {
      const gOutlookAppNameLoginHintRegexp = new RegExp(".*login_hint=([^&]*)"), link = document.getElementById("O365_AppName").href, matches = gOutlookAppNameLoginHintRegexp.exec(link);
      return matches?.[1];
    } catch (error) {
      return;
    }
  }
  function getAccountEmailFromTenantLogo() {
    try {
      const gOutlookEmailHrefRegexp = new RegExp(".*username=([^&]*)"), link = document.getElementById("O365_MainLink_TenantLogo").href, matches = gOutlookEmailHrefRegexp.exec(link);
      return matches?.[1];
    } catch (error) {
      return;
    }
  }
  function xpath(expression, obj = document, root = document, resolver, type, result) {
  ...
  CyberhavenDomEvents.register({
    ...
    uploadWrapVersion: !1
  }), window.self === window.top && (async function({cssSelector, elementXPath, document = window.document, commonParentXPath}) {
    let nextPasteInfo;
    listenPageEvent("Cyberhaven_IsPasteAllowed", (({detail}) => {
      nextPasteInfo = detail;
    }), {
      wrapVersion: !0
    }), async function({cssSelector, elementXPath, document = window.document, commonParentXPath}, onClick) {
      const signal = abort_controller_getSignal();
      document.addEventListener("click", (event => {
        if (!event.isTrusted) return;
        const timestamp = Date.now();
        let isClickButtonDetected = !1;
        if (elementXPath && commonParentXPath) {
          const parentElement = event.target.closest(commonParentXPath);
          if (!parentElement) return;
          isClickButtonDetected = xpath(elementXPath, parentElement).length > 0;
        } else if (elementXPath) {
          const elements = xpath(elementXPath, document);
          elements.length && (elements[0] === event.target || elements[0].contains(event.target)) && (isClickButtonDetected = !0);
        } else cssSelector && (isClickButtonDetected = !!event.target?.closest?.(cssSelector));
        isClickButtonDetected && onClick(event, timestamp);
      }), {
        capture: !0,
        signal
      });
      ...
      const blockPaste = !nextPasteInfo.allowed;
      blockPaste && event.stopImmediatePropagation(), sendMessageToBackground("copy-paste", {
        action: "paste",
        isBlocked: blockPaste,
        pageData: nextPasteInfo.pageData,
        timestamp
      });
    }));
  }({
    elementXPath: './/i[@data-icon-name="ClipboardPasteRegular"]',
    commonParentXPath: "button"
  }), service_communication_onMessage("get-page-data", measureTimeExecution("com.office.outlook:get-page-data", (() => {
    const metadata = {
      email: decodeURIComponent(getAccountEmail() ?? getAccountEmailFromTenantLogo() ?? ""),
      account_type: window.location.hostname.includes("outlook.live.com") ? "onedrive" : "outlook"
    };
    return pageData = {
      metadata
    }, {
