(() => {
  ...
  async function trackPasteClicks({cssSelector, elementXPath, document = window.document, commonParentXPath}) {
    let nextPasteInfo;
  ...
  CyberhavenDomEvents.register({
    ...
    delayAddListeners: 1e3
  }), "github.dev" === location.hostname && function(className, elementFound) {
    const config = {
      childList: !0,
      subtree: !0
    }, observer = new MutationObserver((function(mutationsList, observer) {
      for (const mutation of mutationsList) if ("childList" === mutation.type && mutation.addedNodes.length) for (const addedNode of mutation.addedNodes) if (addedNode.nodeType === Node.ELEMENT_NODE && addedNode.classList.contains(className)) {
        observer.disconnect(), elementFound(addedNode);
        break;
      }
    }));
    ...
    shadowRoot && trackPasteClicks({
      cssSelector: '[aria-label="Paste"]',
      document: shadowRoot
    });
  })), window.self === window.top && (service_communication_onMessage("get-page-data", measureTimeExecution("com.github:get-page-data", (async () => {
    let metadata;
    if ("raw.githubusercontent.com" === location.hostname) {
      const storage = await page_communication_dispatchOnPageWithReply("Cyberhaven_WebAppStorage", {
        type: "get",
        key: "github",
        storage: "permanent"
      }) || {};
      metadata = {
        email: storage[storage.lastUsername],
        account_type: "github"
      };
    } else metadata = await page_communication_dispatchOnPageWithReply("Cyberhaven_GetAccountData");
    return pageData = {
      metadata
    }, {
      url: location.href,
    ...
    proxy("Cyberhaven_WebAppStorage", "webapp-storage"), proxy("Cyberhaven_WebAppCookies", "webapp-cookies");
  })));
})();