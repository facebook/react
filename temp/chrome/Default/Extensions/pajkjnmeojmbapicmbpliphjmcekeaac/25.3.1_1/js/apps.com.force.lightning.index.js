(() => {
  ...
  class CopyPasteController {
    ...
    watchIfPasteAllowed() {
      onMessage("set-paste-status", (event => {
  ...
  class DomUploads {
    ...
    constructor(getLocalPageData, uploadWrapVersion = !0, ignoreUploadErrors = !1) {
      ...
      }(uploadWrapVersion, ignoreUploadErrors), onMessage("upload-search-and-pop", (message => getUpload(message.data.attrs, message.data.timestamp)));
    }
  ...
  function registerGetPageData(config, extras, options = {}) {
    ...
    onMessage("notify-url-change", (() => {
      content_script_cache.pageData = void 0;
    })), onMessage("get-page-data", (async () => {
  ...
  new class extends CyberhavenDomEvents {
    cloudFileObjectType="cloud_file_salesforce";
    ...
    onTopFrame() {
      this.injectPageData(), this.injectXhrScript(), onTopFrameWindow("remove_record", (async event => {
        const beExtras = await this.getCloudFileBeExtras(), beEvent = new BackendEvent({
          event_type: "delete",
          sourceDest: {
            ...beExtras,
            ...event.record
        ...
        this.sendBeEvent(beEvent);
      })), onTopFrameWindow("create_record", (async event => {
        const beExtras = await this.getCloudFileBeExtras(), beEvent = new BackendEvent({
          event_type: "create",
          sourceDest: {
            ...beExtras,
          ...
          source: {
            upload_file_id: event.uploadId
          }
        });
        this.sendBeEvent(beEvent);
      })), onTopFrameWindow("attach_record", (async event => {
        const beExtras = await this.getCloudFileBeExtras(), beEvents = event.files.map((file => new BackendEvent({
          event_type: "create",
          sourceDest: {
            ...beExtras,
            file_id: file.id,
    ...
    async getCloudFileBeExtras() {
      const accountData = await page_communication_dispatchOnPageWithReply("Cyberhaven_GetAccountData");
      return invariant(accountData?.email, "Salesforce accountData.email is not available"), 
      {
        cloud_account: accountData.email,
    ...
    injectPageData() {
      registerGetPageData("src/apps/com.force.lightning/page-data.web.ts", {
        metadata: {
          account_type: this.cloudApp
        },
        pageData: {
          cloud_provider: this.cloudProvider,
    ...
    injectXhrScript() {
      onInjectScript({
        service: "salesforce",
        scriptId: "salesforce_xhr",
        scriptUrl: "src/apps/com.force.lightning/xhr.web.ts"
      }, (() => {
        var destEventName;
        proxy("Cyberhaven_UploadCloudData", "upload-cloud-data"), destEventName = "Cyberhaven_GetReportMetadata", 
        onMessage("get-report-metadata", (({data}) => page_communication_dispatchOnPageWithReply(destEventName, data)));
      }));
    }
  };
})();