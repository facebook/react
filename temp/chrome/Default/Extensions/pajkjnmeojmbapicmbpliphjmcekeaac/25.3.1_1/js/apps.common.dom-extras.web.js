(() => {
  ...
  detail);
  let nextPasteInfo;
  const shouldBlockNextPaste = () => nextPasteInfo && !nextPasteInfo.allowed && nextPasteInfo.validUntil > Date.now(), clipboardApiCopy = "true" === document.currentScript?.dataset.clipboardAPICopy, clipboardApiPaste = "true" === document.currentScript?.dataset.clipboardAPIPaste, interceptClipboardAPI = () => {
    if ((clipboardApiCopy || clipboardApiPaste) && listenPageEvent("Cyberhaven_IsPasteAllowed", (({detail}) => {
      nextPasteInfo = detail;
    }), {
      wrapVersion: !0
    }), clipboardApiPaste) {
      window.navigator.clipboard._read = window.navigator.clipboard.read, window.navigator.clipboard._readText = window.navigator.clipboard.readText;
      const notifyPasteEventThrottle = function(func, timeFrame) {
        let lastTime = 0;
        ...
        dispatchOnPage("Cyberhaven_NotifyPasteEvent", {
          isPasteBlocked: shouldBlockNextPaste(),
          timestamp,
      ...
      Object.defineProperty(window.navigator.clipboard, "read", {
        get: function() {
          try {
            const timestamp = Date.now();
            if (notifyPasteEventThrottle(timestamp), shouldBlockNextPaste()) return () => new Promise((resolve => resolve({})));
          } catch (e) {
            logError({
              message: "Error while reading clipboard data",
              error: e
            });
          }
          return this._read;
        ...
        get: function() {
          try {
            if (notifyPasteEventThrottle(Date.now()), shouldBlockNextPaste()) return () => new Promise((resolve => resolve("")));
          } catch (e) {
            logError({
              message: "Error while reading clipboard data",
              error: e
        ...
        set(value) {
          this._readText = value;
        }
      });
    }
    if (clipboardApiCopy) {
      if (window.navigator.clipboard._write && window.navigator.clipboard._writeText) return;
      window.navigator.clipboard._write = window.navigator.clipboard.write, window.navigator.clipboard._writeText = window.navigator.clipboard.writeText;
      const notifyCopyEvent = () => {
        dispatchOnPage("Cyberhaven_NotifyCopyEvent", {
          timestamp: Date.now(),
          pageData: nextPasteInfo?.pageData
        }, {
          wrapVersion: !1
      ...
      Object.defineProperty(window.navigator.clipboard, "writeText", {
        get: function() {
          return notifyCopyEvent(), this._writeText;
        },
        set(value) {
          this._readText = value;
        }
        ...
        get: function() {
          return notifyCopyEvent(), this._write;
        },
        set(value) {
          this._read = value;
        }
  ...
  "true" === document.currentScript?.dataset.openFilePickerApi && "showOpenFilePicker" in window && (() => {
    const windowShoowOpenFilePicker = window.showOpenFilePicker;
    window.showOpenFilePicker = new Proxy(window.showOpenFilePicker, {
      async apply(target, window, args) {
        const promise = Reflect.apply(target, window, args), result = await promise;
        try {
          const timestamp = Date.now();
          dispatchOnPage("Cyberhaven_FileUploadEvent", {
            files: await Promise.all(result.filter((item => "file" === item.kind)).map((item => item.getFile()))),
            timestamp
          }, {
            wrapVersion: !1
          });
    ...
    }), listenPageEvent("Cyberhaven_Unload", (() => {
      window.showOpenFilePicker = windowShoowOpenFilePicker;
    }));
  })();
  "true" === document.currentScript?.dataset.dynamicInputs && (() => {
    const HTMLInputElementDispatchEvent = HTMLInputElement.prototype.dispatchEvent, HTMLInputElementClick = HTMLInputElement.prototype.click;
    function unload() {
      HTMLInputElement.prototype.dispatchEvent = HTMLInputElementDispatchEvent, HTMLInputElement.prototype.click = HTMLInputElementClick;
    }
    function onChangeHandler(event) {
      try {
        ...
        }(event) || !target || !target.files?.length) return;
        const timestamp = Date.now();
        dispatchOnPage("Cyberhaven_FileUploadEvent", {
    ...
    try {
      HTMLInputElement.prototype.dispatchEvent = new Proxy(HTMLInputElement.prototype.dispatchEvent, {
        apply(target, input, args) {
          try {
            args[0] instanceof Event && "click" === args[0].type.toLowerCase() && !document.body.contains(input) && input.addEventListener("change", onChangeHandler, !0);
          } catch {}
          return Reflect.apply(target, input, args);
        }
      }), HTMLInputElement.prototype.click = new Proxy(HTMLInputElement.prototype.click, {
        apply(target, input, args) {
          try {
            document.body.contains(input) || input.addEventListener("change", onChangeHandler, !0);
          } catch (err) {}
          return Reflect.apply(target, input, args);
        }
      }), listenPageEvent("Cyberhaven_Unload", unload);
    } catch {
      unload();
    }
  })(), (clipboardApiCopy || clipboardApiPaste) && interceptClipboardAPI();
})();