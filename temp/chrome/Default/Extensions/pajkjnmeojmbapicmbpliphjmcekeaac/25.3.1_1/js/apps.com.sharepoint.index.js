(() => {
  ...
  (delay, reason, defaultResolve = void 0) => new Promise(((resolve, reject) => setTimeout((() => void 0 === reason ? resolve(defaultResolve) : reject(reason)), delay)))), abortController = new AbortController, abort_controller_unsubscribe = () => {
    abortController.abort();
  }, getSignal = () => abortController.signal, abort_controller_onCleanup = callback => {
  ...
  async function sendMessageToBackground(destination, data) {
    try {
      ...
      return abort_controller_unsubscribe(), Promise.reject(err);
    }
  }
  ...
  async function onInjectScript({service, scriptId, scriptUrl}, onInjectScript) {
    ...
    try {
      ...
      dispatchOnPage("Cyberhaven_Page_Unload", null, {
        ...
        errorMessage: get_error_message_getErrorMessage(error)
      }), console.error(error);
    }
  }
  ...
  class CyberhavenDomEvents {
    ...
    setUnloadListener() {
      listenPageEvent("Cyberhaven_Unload", (({detail}) => {
        this.randomScriptId !== detail.id && (this.loaded = !1, abort_controller_unsubscribe());
      }), {
        wrapVersion: !1
      }), abort_controller_onCleanup((() => {
        dispatchOnPage("Cyberhaven_Page_Unload", null, {
          wrapVersion: !1
        });
      }));
  ...
  function createProtectedMessaging() {
    return {
      sendMessage: (eventName, data) => {
        window.postMessage(JSON.stringify({
          event: "ch_" + String(eventName),
          payload: data
        }));
      },
      onMessage: (eventName, callback) => {
        const handleWindowMessage = message => {
          ...
          if ("string" != typeof message.data) return;
          const parsed = (json => {
            if (json) try {
          ...
          parsed?.event === "ch_" + String(eventName) && callback(parsed.payload);
        };
        window.addEventListener("message", handleWindowMessage), abort_controller_onCleanup((() => {
          window.removeEventListener("message", handleWindowMessage);
        }));
      }
    };
  }
  class ContentPageTyped extends CyberhavenDomEvents {
    messages=createProtectedMessaging();
    pageEvent(eventName, callback) {
      this.messages.onMessage(eventName, (async data => {
        try {
          await callback(data);
        } catch (error) {
          page_logging_logError({
            error: `Error occured in page event: ${String(eventName)}`,
            errorMessage: get_error_message_getErrorMessage(error)
          });
        }
      }));
    }
    async getCloudFileBeExtras() {
      return {
        cloud_provider: this.cloudProvider,
        cloud_app: this.cloudApp,
        object_type: this.cloudFileObjectType
      };
    }
  }
  ...
  function registerGetPageData(config, extras, options = {}) {
  ...
  new class extends ContentPageTyped {
    cloudFileObjectType="cloud_file_onedrive";
    cloudProvider="office_365";
    cloudApp="sharepoint";
    constructor() {
      super(void 0, {
        ignoreUploadErrors: !0
      }), this.init(), this.handleFileBrowserFrame();
    }
    onTopFrame() {
      this.injectCloudDataScript(), this.injectFileBrowserScript();
    }
    handleFileBrowserFrame() {
      new URL(window.location.href).pathname.toLowerCase().endsWith("/_layouts/15/filebrowser.aspx") && this.injectFileBrowserScript();
    }
    injectCloudDataScript() {
      registerGetPageData("src/apps/com.sharepoint/cloud_data.web.ts", {
        pageData: {
          cloud_provider: this.cloudProvider,
          default_object_type: this.cloudFileObjectType,
          page_app: this.getOfficePageAppType()
        }
      });
    }
    injectFileBrowserScript() {
      onInjectScript({
        service: "office_365",
        scriptId: "sharepoint_xhr",
        scriptUrl: "src/apps/com.sharepoint/filebrowser.web.ts"
      }, (() => {
        this.handleFileBrowserEvents();
      }));
    }
    handleFileBrowserEvents() {
      this.pageEvent("copy_move", (async ({events, isMoveMode, accountEmail, requestId, webAbsoluteUrl}) => {
        invariant(accountEmail, "account email is not available");
        const fileUrl = path => webAbsoluteUrl ? encodeURI(new URL(webAbsoluteUrl).origin + path) : "", beExtras = await this.getCloudFileBeExtras(), beEvents = events.map((({source, destination}) => new BackendEvent({
          event_type: isMoveMode ? "move" : "copy",
          sourceDest: {
            ...beExtras,
            cloud_account: accountEmail,
            metadata: requestId ? {
              request_id: requestId
            } : void 0,
            file_size: source.File_x0020_Size
          },
          source: {
            file_id: trimCurlyBrackets(source.UniqueId).toLowerCase(),
            file_name: source.FileLeafRef,
            file_path: source.FileRef,
            file_url: fileUrl(source.FileRef)
          },
          destination: {
            file_id: trimCurlyBrackets(destination.UniqueId).toLowerCase(),
            file_name: destination.FileLeafRef,
            file_path: destination.FileRef,
            file_url: fileUrl(destination.FileRef)
          },
          timestamp: (new Date).toISOString()
        })));
        this.sendBeEvent(beEvents);
      })), this.pageEvent("upload_create", (async ({uploadCreateResponse, uploadEvent, requestId, accountEmail, webAbsoluteUrl}) => {
        invariant(accountEmail, "account email is not available");
        const beExtras = await this.getCloudFileBeExtras(), fileUrl = this.replaceUrlPath(webAbsoluteUrl, uploadCreateResponse.ServerRelativeUrl), beEvent = new BackendEvent({
          event_type: "create",
          sourceDest: {
            ...beExtras,
            cloud_account: accountEmail,
            file_name: uploadCreateResponse.Name,
            file_path: uploadCreateResponse.ServerRelativeUrl,
            file_id: trimCurlyBrackets(uploadCreateResponse.UniqueId).toLowerCase(),
        ...
        await this.sendBeEvent(beEvent);
      })), this.pageEvent("rename", (async ({accountEmail, listItem, newName, webAbsoluteUrl, requestId}) => {
        invariant(accountEmail, "account email is not available");
        const beExtras = await this.getCloudFileBeExtras(), newFilePath = listItem.FileRef.replace(listItem.FileLeafRef, newName), beEvent = new BackendEvent({
          event_type: "rename",
          destination: {
            file_name: newName,
            file_path: newFilePath,
            file_url: this.replaceUrlPath(webAbsoluteUrl, newFilePath)
          },
          source: {
            file_name: listItem.FileLeafRef,
            file_path: listItem.FileRef,
            file_url: this.replaceUrlPath(webAbsoluteUrl, listItem.FileRef)
          },
          sourceDest: {
            ...beExtras,
            cloud_account: accountEmail,
            file_id: trimCurlyBrackets(listItem.UniqueId).toLowerCase(),
            file_size: listItem.File_x0020_Size,
            metadata: requestId ? {
              request_id: requestId
            } : void 0
          }
    ...
    replaceUrlPath(url, path) {
      return url ? encodeURI(new URL(url).origin + path) : void 0;
    }
    getOfficePageAppType() {
      const url = new URL(window.location.href);
      return url.pathname.endsWith("onedrive.aspx") ? "office_sharepoint" : url.pathname.endsWith("Doc.aspx") ? "office_document_app" : "unknown";
    }
  };
})();