(() => {
  ...
  const registerEnablePreviousHookListener = () => {
    isEnablePreviousHookListenerRegistered || globals_isWebWorkerContext() || (listenPageEvent("Cyberhaven_EnablePreviousHookIfExist", (({detail: {initiatedByScriptId}}) => {
  ...
  let currentUser;
  var fn;
  !function(options, {enablePreviousScriptOnReload = !1} = {}) {
    ...
    isXhrHooked = !0, enabled = !(getCurrentScriptId() && !globals_isWebWorkerContext() && optimisticallyInjectedScripts.includes(getCurrentScriptId())), 
    ...
    after({responseText}, xhr) {
      200 === xhr.status && (currentUser = JSON.parse(responseText).data);
    },
    matchUrlAfterConfig: [ {
      method: "GET",
      urlRegex: "/api/v0/users/current"
    }, {
      method: "GET",
      urlRegex: "/auth-api/v0/users/current"
    } ]
  } ]), fn = async () => ({
