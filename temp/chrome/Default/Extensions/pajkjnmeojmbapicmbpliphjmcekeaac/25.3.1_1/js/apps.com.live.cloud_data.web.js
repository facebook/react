(() => {
  ...
  detail);
  const cloud_data_web_globals = window;
  !function(eventName, callback, {ignoreSignal = !1, wrapVersion = addVersionToEventNameByDefault} = {}) {
    ...
    const email = cloud_data_web_globals.$Config?.email ?? cloud_data_web_globals.O365ShellContext?.BootHeaderState?.userEmail.toLowerCase() ?? function() {
      let configText;
      try {
    ...
    return {
      email,
      account_type: email && function() {
        try {
          const {item: {id}} = JSON.parse(localStorage.getItem("tmp.auth.v1.GLOBAL.User.User") || "{}"), tenant = JSON.parse(localStorage.getItem(`tmp.auth.v1.${id}.Tenants.Tenants`) || "{}").item.find((tenant => tenant.userId === id));
          return "organization" === tenant?.tenantType ? "sharepoint" : "onedrive";
        } catch {}
        return "onedrive";
      }()
    };
