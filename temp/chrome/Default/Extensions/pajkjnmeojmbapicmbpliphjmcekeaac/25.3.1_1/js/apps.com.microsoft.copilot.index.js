(() => {
  ...
  CyberhavenDomEvents.register();
  const getEmail = location.hostname.includes("copilot.microsoft.com") ? () => {
    const activeSessionKey = Object.keys(localStorage).find((key => key.endsWith(".active-account")));
    if (!activeSessionKey) return null;
    const activeSessionValue = localStorage.getItem(activeSessionKey);
    if (activeSessionValue) try {
      const key = JSON.parse(localStorage.getItem("msal.account.keys")).find((key => key.includes(activeSessionValue)));
      if (key) {
    ...
    let configText;
    try {
      const tsUserInfo = localStorage.getItem("ts.userInfo");
      if (tsUserInfo) return JSON.parse(tsUserInfo).email;
      if (configText = document.getElementById("clientStartupConfiguration")?.textContent, 
      configText) {
        return JSON.parse(configText).userPrincipalName;
      }
      let sessionTrackingKey = null;
      const preloadUserStorageId = localStorage.getItem("EwPreloadUserStorageId");
      if (preloadUserStorageId) sessionTrackingKey = "sessionTracking_" + JSON.parse(preloadUserStorageId); else {
        const sessionTracking = localStorage.getItem("sessionTracking_SignedInAccountList");
        if (!sessionTracking) return;
        const signedInAccountList = JSON.parse(sessionTracking);
        sessionTrackingKey = signedInAccountList.reduce((function(a, b) {
          return a.lastActiveTime >= b.lastActiveTime ? a : b;
        }), {}).sessionTrackingKey;
      }
      const accountInfo = JSON.parse(localStorage.getItem(sessionTrackingKey) || "{}");
      if (accountInfo && accountInfo.upn) return accountInfo.upn;
    } catch {}
  };
  !function(config, extras, options = {}) {
