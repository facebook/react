(() => {
  ...
  const isIDE = window.top?.location?.pathname.includes("/-/ide/");
  CyberhavenDomEvents.register({
    ...
    delayAddListeners: isIDE ? 1e3 : 0
  }), isIDE && setTimeout((() => {
    !function(className, elementFound) {
      const config = {
        childList: !0,
        subtree: !0
  ...
  }), 1e3), window.self === window.top && (service_communication_onMessage("get-page-data", measureTimeExecution("com.gitlab:get-page-data", (async () => {
    let metadata;
    ...
    service: "gitlab",
    scriptId: "gitlab_cloud_data",
    scriptUrl: "src/apps/com.gitlab/cloud_data.web.ts"
  }, (() => {
