(() => {
  ...
  }, logWebAppFeatureUsage = (webApp, featureName) => {
    log("info", {
      web_app: webApp,
  ...
  }("src/apps/so.notion/cloud_data.web.ts", {
    pageData: {
      cloud_provider: "notion",
      default_object_type: "notion_page"
    }
  }, {
    accountDataTimeout: 2e3
  }), window.self === window.top && (proxy("Cyberhaven_WebAppStorage", "webapp-storage"), 
  async function({service, scriptId, scriptUrl}, onInjectScript) {
    ...
    service: "notion",
    scriptId: "notion_xhr",
    scriptUrl: "src/apps/so.notion/xhr.web.ts"
  }, (() => {
    var fn;
    proxy("Cyberhaven_ShareData", "share-data"), proxy("Cyberhaven_UploadCloudData", "upload-cloud-data"), 
    window.self === window.top && (fn = () => {
      logWebAppFeatureUsage("notion", "Visited Notion");
