(() => {
  ...
  }("Cyberhaven_GetAccountDataWithConfig", (event => {
    let email = null;
    const emailConfig = event.detail.email;
    try {
      if ("window" in emailConfig) email = get(window, emailConfig.window.path); else if ("session" in emailConfig) {
        const {session} = emailConfig;
        if ("string" == typeof session) email = sessionStorage.getItem(session); else if ("key" in session) {
          const item = sessionStorage.getItem(session.key);
          if (item && session.parseJsonValuePath) {
            email = get(JSON.parse(item), session.parseJsonValuePath);
          } else email = item;
        } else logError({
          error: "Unknown get page data config structure",
          config: event.detail
        });
      }
      return {
