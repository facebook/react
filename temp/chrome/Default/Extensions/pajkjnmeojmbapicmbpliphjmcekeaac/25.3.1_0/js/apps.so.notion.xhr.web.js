(() => {
  ...
  const fetch_intercept = function(env) {
    ...
    return env.fetch = function(fetch) {
  ...
  }, abort_controller_getSignal = () => abortController.signal, abort_controller_onCleanup = callback => {
  ...
  const getCurrentScriptId = () => currentScript?.getAttribute("scriptId"), isCurrentScriptOptimisticallyInjected = () => !(!getCurrentScriptId() || globals_isWebWorkerContext()) && optimisticallyInjectedScripts.includes(getCurrentScriptId());
  ...
  }, notionAPI_uploadStartUrl = "api/v3/getUploadFileUrl", notionAPI_backlinksForBlockUrl = "api/v3/getBacklinksForBlock", notionAPI_saveTransactionsFanoutUrl = "api/v3/saveTransactionsFanout", notionAPI_findUserUrl = "/api/v3/findUser", notionAPI_createUserUrl = "/api/v3/createUser", notionAPI_syncRecordValuesUrl = "/api/v3/syncRecordValuesMain", notionAPI_deleteBlocks = "/api/v3/deleteBlocks", notionAPI_fileUploadUrl = "prod-files-secure";
  const uploadKeyMap = {};
  async function handleUploadsResponse(response) {
    const responseData = await response.json(), startedAt = Date.now(), data = JSON.parse(response.body), uploadFile = await async function(file, timestamp) {
    ...
    if (!uploadFile || !data.record) return;
    const file = {
      id: data.record.id,
      title: data.name,
    ...
    if (!file) return;
    const uploadKey = responseData.fields.key;
    uploadKey && (uploadKeyMap[uploadKey] = file);
  }
  const isModalView = () => window.location.search.includes("p="), getPageNameFromDocument = () => {
    if (isModalView()) {
  ...
  }, getFileIdFromUrl = (formatToUuid = !0) => {
  ...
  const {dispatchOnWindow, dispatchOnWindowWithReply, onTopFrameWindow} = function(topFrameOrigins, communicationTimeout = 2500) {
    return {
      ...
      onTopFrameWindow: function(eventName, callback) {
        ...
        window.addEventListener("message", listener), abort_controller_onCleanup((() => {
          window.removeEventListener("message", listener);
        }));
      }
    };
  }([]), pagesFullPaths = {}, fetchPromiseForId = {};
  onTopFrameWindow("ch_get_current_fullpath", (async _data => {
    const currentPageId = getFileIdFromUrl();
    if (currentPageId && !pagesFullPaths[currentPageId]) {
      if (fetchPromiseForId[currentPageId]) return null;
      fetchPromiseForId[currentPageId] = fetch("https://www.notion.so/api/v3/getBacklinksForBlock", {
        method: "POST",
        headers: {
      ...
      }), await fetchPromiseForId[currentPageId], fetchPromiseForId[currentPageId] = void 0;
    }
    const pageFullPath = currentPageId && pagesFullPaths[currentPageId] || null;
    if (pageFullPath) {
      const pathParts = pageFullPath.split("/");
      return pathParts[pathParts.length - 1] = getPageNameFromDocument(), pathParts.join("/");
    }
    return null;
  }));
  const handleLogRequest = config => {
    const body = (formDataString = config.body, Object.fromEntries(new URLSearchParams(formDataString)));
    var formDataString;
    const e = JSON.parse(body.e);
    for (const event of e) {
      if ([ "import_button_click", "import_source_auth", "external_sync_teamspace_selection" ].includes(event.event_type)) {
        const importSource = event.event_properties.integration || event.event_properties.import_source;
        logWebAppFeatureUsage("notion", `Import data from ${importSource}`);
      }
      "block_action" === event.event_type && "Delete" === event.event_properties.analyticsName && "page" === event.event_properties.block_type && logWebAppFeatureUsage("notion", "Delete Page"), 
      "block_action" === event.event_type && "Duplicate" === event.event_properties.analyticsName && "page" === event.event_properties.block_type && logWebAppFeatureUsage("notion", "Duplicate Page"), 
      event.event_type;
    }
  };
  ...
  const Roles = {
    editor: "writer",
    read_and_write: "writer",
    comment_only: "commenter",
    content_only_editor: "writer",
    reader: "reader"
  }, AccessBy = {
    group_permission: "audience",
    explicit_team_permission: "audience",
    explicit_team_owner_permission: "audience",
    user_permission: "user",
    space_permission: "audience"
  }, usersCache = {};
  ((service, scriptId) => {
    ((value = !0) => {
      addVersionToEventNameByDefault = !isCurrentScriptOptimisticallyInjected() && value;
    })(), scriptId ? (async (service, onReady) => {
  ...
  const fetchConfigs = [ {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_uploadStartUrl
    } ],
    after(response) {
      handleUploadsResponse(response);
    },
    requireRequestBodyInAfter: !0
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_backlinksForBlockUrl
    } ],
    after(response) {
      !async function(response) {
        const data = await response.json();
        let currentPageId = getFileIdFromUrl();
        currentPageId || logError({
          event: "GetBacklinksForBlock",
          error: "No current page id detected",
          url: response.url
        });
        const pageId = currentPageId, path = [];
        let safeguardDepth = 7;
        for (;safeguardDepth && currentPageId && data.recordMap.block[currentPageId]; ) {
          let value = data.recordMap.block?.[currentPageId]?.value;
          value.value && (value = value.value);
          const pageName = value.properties?.title[0]?.[0] || getPageNameFromDocument();
          path.push(pageName), currentPageId = value.parent_id, safeguardDepth--;
        }
        if (currentPageId) {
          let value = data.recordMap.team?.[currentPageId]?.value;
          if (value) {
            value.value && (value = value.value);
            const teamName = value?.name;
            teamName && path.push(teamName);
          }
        }
        pagesFullPaths[pageId] = path.reverse().join("/");
      }(response);
    }
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_findUserUrl
    } ],
    after(response) {
      (async response => {
        const requestData = JSON.parse(response.body), responseData = await response.json(), email = requestData.email, userId = responseData.value?.value?.id;
        email && userId && (usersCache[userId] = email);
      })(response);
    },
    requireRequestBodyInAfter: !0
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_createUserUrl
    } ],
    after(response) {
      (async response => {
        const requestData = JSON.parse(response.body), responseData = await response.json(), email = requestData.email, userId = responseData.userId;
        email && userId && (usersCache[userId] = email);
      })(response);
    },
    requireRequestBodyInAfter: !0
    ...
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_saveTransactionsFanoutUrl
    } ],
    after: response => {
      (async response => {
        const requestBody = JSON.parse(response.body), requests = [];
        for (const transaction of requestBody.transactions) {
          let userId, role, type, isTeamPermission = !1;
          for (const operation of transaction.operations) if ("update" === operation.command) {
            if ("invite" === operation.pointer.table) {
              const inviteeId = operation.args.invitee_id;
              usersCache[inviteeId] && (userId = usersCache[inviteeId]);
            }
          } else if ("setPermissionItem" === operation.command) {
            if ("explicit_team_permission" === operation.args.type && (isTeamPermission = !0), 
            !isTeamPermission && !userId && "user_permission" === operation.args.type) {
              if (!usersCache[operation.args.user_id]) {
                const body = {
                  requests: [ {
                    version: -1,
                    pointer: {
                      table: "notion_user",
                      id: operation.args.user_id
                    }
                  } ]
                }, notionUserId = getCookie("notion_user_id");
                await fetch(notionAPI_syncRecordValuesUrl, {
                  method: "POST",
                  credentials: "include",
              ...
              userId = usersCache[operation.args.user_id];
            }
            role || type || (role = Roles[operation.args.role], type = AccessBy[operation.args.type]);
          }
          role && type && requests.push({
            role,
            type,
            userIds: userId && "user" === type ? [ userId ] : [],
            object_type: "notion_page"
          });
        }
        if (0 === requests.length) return;
        const accountData = await dispatchOnPageWithReply("Cyberhaven_GetAccountData", {});
        if (accountData) {
          for (const request of requests) request.notion_page_id = accountData.notion_page_id, 
          request.notion_page_name = accountData.notion_page_name, request.notion_page_full_path = accountData.notion_page_full_path;
          await dispatchOnPageWithReply("Cyberhaven_ShareData", {
            requests
          });
        }
      })(response);
    ...
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_syncRecordValuesUrl
    } ],
    after(response) {
      (async response => {
        const responseData = await response.json(), notionUsersObj = responseData.recordMap?.notion_user;
        if (notionUsersObj) for (const userId in notionUsersObj) {
          const user = notionUsersObj[userId];
          usersCache[userId] = user.value?.value.email;
        }
      })(response);
    }
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_deleteBlocks
    } ],
    after() {
      (async () => {
        logWebAppFeatureUsage("notion", "Delete Page");
      })();
    }
  } ], xhrConfigs = [ {
    after({config}) {
      handleLogRequest(config);
    },
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: "/api/v3/teV1"
    } ]
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: notionAPI_fileUploadUrl
    } ],
    after({config}) {
      !async function(config) {
        if (![ 200, 204 ].includes(config.status)) return;
        if (!config.formData) return;
        dispatchOnPage("Cyberhaven_UploadCloudData", {
          files: [ uploadKeyMap[config.formData.key] ],
          metadata: {
            object_type: "cloud_file_notion"
          }
        });
      }(config);
    },
    requireRequestBodyInAfter: !0
  } ], {unhookFetch} = function(options) {
  ...
  }(fetchConfigs), {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
  ...
  abort_controller_onCleanup(unhookFetch), abort_controller_onCleanup(unhook);
})();