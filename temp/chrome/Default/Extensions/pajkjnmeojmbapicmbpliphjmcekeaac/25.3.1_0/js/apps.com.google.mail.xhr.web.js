(() => {
  ...
  var events = [ "load", "loadend", "timeout", "error", "readystatechange", "abort" ], OriginXhr = "__origin_xhr";
  function configEvent(event, xhrProxy) {
    var e = {};
    for (var attr in event) e[attr] = event[attr];
    return e.target = e.currentTarget = xhrProxy, e;
  }
  ...
  function triggerListener(xhr, name) {
    var evt, xhrProxy = xhr.getProxy(), callback = "on" + name + "_", event = configEvent({
      type: name
    }, xhrProxy);
    xhrProxy[callback] && xhrProxy[callback](event), "function" == typeof Event ? evt = new Event(name, {
      bubbles: !1
    }) : (evt = document.createEvent("Event")).initEvent(name, !1, !0), getEventTarget(xhr).dispatchEvent(evt);
  }
  function Handler(xhr) {
    this.xhr = xhr, this.xhrProxy = xhr.getProxy();
  }
  function makeHandler(next) {
    function sub(xhr) {
      Handler.call(this, xhr);
    }
    return sub.prototype = Object.create(Handler.prototype), sub.prototype.next = next, 
    sub;
  }
  Handler.prototype = Object.create({
    resolve: function(response) {
      var xhrProxy = this.xhrProxy, xhr = this.xhr;
      xhrProxy.readyState = 4, xhr.resHeader = response.headers, xhrProxy.response = xhrProxy.responseText = response.response, 
      xhrProxy.statusText = response.statusText, xhrProxy.status = response.status, triggerListener(xhr, eventReadyStateChange), 
      triggerListener(xhr, eventLoad), triggerListener(xhr, eventLoadEnd);
    },
    reject: function(error) {
      this.xhrProxy.status = 0, triggerListener(this.xhr, error.type), triggerListener(this.xhr, eventLoadEnd);
    }
  });
  var RequestHandler = makeHandler((function(rq) {
    var xhr = this.xhr;
    for (var key in rq = rq || xhr.config, xhr.withCredentials = rq.withCredentials, 
    xhr.open(rq.method, rq.url, !1 !== rq.async, rq.user, rq.password), rq.headers) xhr.setRequestHeader(key, rq.headers[key]);
    xhr.send(rq.body);
  })), ResponseHandler = makeHandler((function(response) {
    this.resolve(response);
  })), ErrorHandler = makeHandler((function(error) {
    this.reject(error);
  }));
  let cache = !0;
  ...
  let promise;
  const registerEvents = (service, scriptId) => {
    currentDomain = service, function(eventName, callback, {ignoreSignal = !1, wrapVersion = addVersionToEventNameByDefault} = {}) {
  ...
  function isEmailNew(item) {
    return item && item[0] && ((id = item[0]) && "string" == typeof id && !id.includes("bump-") && /^msg-[a|f]:/.test(id)) && item.length > 10;
    var id;
  }
  function extractEmails(obj) {
    return function(obj, predicate) {
      const result = [], safePredicate = function(item) {
        try {
          return predicate(item);
        } catch (err) {
          return !1;
        }
      }, forEachGraph = function(obj) {
        if (safePredicate(obj)) result.push(obj); else for (const key in obj) {
          const item = obj[key];
          if (safePredicate(item)) result.push(item); else if (Array.isArray(item)) for (const listItem of item) forEachGraph(listItem); else "object" == typeof item && forEachGraph(item);
        }
      };
      return forEachGraph(obj), result;
    }(obj, isEmailNew);
  }
  function mapAttachment(attachment) {
    const [mimeType, name, size, , xAttachmentId, attachmentUrl, , syncData, , uploadResponseBase64, , , , driveId] = attachment;
    return {
      mimeType,
      name,
      size,
      xAttachmentId,
      attachmentUrl,
      uploadResponseBase64,
      blobId: syncData?.[0],
      driveId
    };
  }
  function processEmailData(body) {
    const parsed = JSON.parse(body), [email] = extractEmails(parsed), msgId = email[0], recipients = [ email[2], email[3], email[4] ].filter((group => Array.isArray(group))).map((recipientsInGroup => recipientsInGroup.map((recipient => recipient[1])))).flat();
    const subject = email[7], attachments = (email[11] || []).map(mapAttachment), bodyContent = email[8]?.[1]?.[0]?.[1], inlineAttachments = function(bodyContent) {
      const escaleHTMLPolicy = window.trustedTypes?.createPolicy("forceInner", {
        createHTML: toEscape => toEscape
      }), div = document.createElement("div");
      div.innerHTML = escaleHTMLPolicy ? escaleHTMLPolicy.createHTML(bodyContent) : bodyContent;
      const attachmentUrls = Array.from(div.querySelectorAll(".gmail_drive_chip")).map((chip => {
        const a = chip.querySelector("a");
        return a?.getAttribute("href");
      })).filter((url => !!url));
      return [ ...new Set(attachmentUrls) ];
    }(bodyContent), labels = email[10], isSent = [ LABELS.SENT, LABELS.SCHEDULED ].some((label => labels.includes(label))), sendTimestamp = email[6].toString();
    return {
      msgId,
      recipients,
      subject,
  ...
  function parseDownloadUrl(messageId, partId) {
    const [spanEl] = function(expression, obj = document, root = document, resolver, type, result) {
      const evalResult = root.evaluate(expression, obj, resolver, type, result);
      return Array.from((xpr = evalResult, {
        [Symbol.iterator]: () => ({
          next: () => {
      ...
      var xpr;
    }(`//span[contains(@download_url, "${messageId}") and contains(@download_url, "${partId}") and contains(@download_url, "realattid")]`), downloadUrl = decodeURI(spanEl.getAttribute("download_url")), [mimeType, fileName, ...urlParts] = downloadUrl.split(":").map(decodeURIComponent), [, , url] = urlParts.join(":").split("https://"), objectUrl = new URL(`https://${url}`);
    return {
      emailId: objectUrl.searchParams.get("permmsgid"),
      attachmentId: objectUrl.searchParams.get("realattid"),
      mimeType,
      fileName,
      downloadUrl
    };
  ...
  ((service, scriptId) => {
    setAddVersionToEventNameByDefault(), scriptId ? (async (service, onReady) => {
  ...
  const mailCache = new class {
    completedUploads={};
    uploadIdToFileUpload={};
    inlineAttachmentsQueue={};
    gdriveCache=null;
    filesCache={};
    async startUpload({name, size}, uploadId, startedAt) {
      const uploadItem = await async function(file, timestamp) {
      ...
      uploadItem && (this.uploadIdToFileUpload[uploadId] = uploadItem);
    }
    cancelUpload(uploadId) {
      this.uploadIdToFileUpload[uploadId] && delete this.uploadIdToFileUpload[uploadId];
    }
    finishUpload(uploadId, responseBase64) {
      this.completedUploads[responseBase64] = uploadId;
    }
    popCompletedUpload(uploadResponseBase64) {
      const gmailUploadId = this.completedUploads[uploadResponseBase64], uploadQueueItem = this.uploadIdToFileUpload[gmailUploadId];
      if (!uploadQueueItem) return null;
      const uploadId = uploadQueueItem?.upload_id;
      return delete this.completedUploads[uploadResponseBase64], delete this.uploadIdToFileUpload[gmailUploadId], 
      uploadId;
    }
    retrieveUpload(uploadId) {
      const uploadQueueItem = this.uploadIdToFileUpload[uploadId];
      return delete this.uploadIdToFileUpload[uploadId], uploadQueueItem;
    }
    popInlineAttachment(url) {
      const attachment = this.inlineAttachmentsQueue[url];
      return delete this.inlineAttachmentsQueue[url], attachment;
    }
    popDocFromDriveCache(comparator) {
      const index = this.gdriveCache.selection.findIndex(comparator);
      if (-1 === index) return null;
      const item = this.gdriveCache.selection[index];
      return this.gdriveCache.selection.splice(index, 1), 0 === this.gdriveCache.selection.length && (this.gdriveCache = null), 
      item;
    }
    addInlineAttachment(url, attachment) {
      this.inlineAttachmentsQueue[url] = attachment;
    }
    hasCompletedUploads() {
      return Object.keys(this.completedUploads).length > 0;
    }
    hasDriveSelection() {
      return null !== this.gdriveCache?.selection;
    }
    isDriveSelectionIsAttachmentLike() {
      return this.gdriveCache?.selection.some((item => item.isAttachment));
    }
    hasInlineAttachments() {
      return Object.keys(this.inlineAttachmentsQueue).length > 0;
    }
    saveFileToCache(fileId, file) {
      this.filesCache[fileId] = file;
    }
    getFileFromCache(fileId) {
      return this.filesCache[fileId];
    }
  };
  const proxyOnResponse = measureTimeExecution("com.google.mail:upload", (response => {
    const {url, method} = response.config;
    "GET" === method && /\/drive\/v2internal\/files\/(.*)\?/.test(url) ? function(response) {
      const {url} = response.config, searchParams = new URL(url).searchParams, fileIdMatch = url.match(/\/drive\/v2internal\/files\/(.*)\?/), fileId = fileIdMatch ? fileIdMatch[1] : null;
      if (!fileId) return;
      if ("media" === searchParams.get("alt")) {
        const fileData = JSON.parse(mailCache.getFileFromCache(fileId));
        fileData && dispatchOnPage("Cyberhaven_DownloadCloudFile", {
          fileId,
          fileName: fileData.title,
          downloadUrl: fileData.downloadUrl
        });
      } else searchParams.get("fields")?.includes("title") && mailCache.saveFileToCache(fileId, response.response);
    }(response) : url.includes("/_/upload") ? function(response) {
      const requestHeaders = response.config.headers, xGoogUploadCommand = requestHeaders["x-goog-upload-command"], {url} = response.config;
      if ("start" === xGoogUploadCommand) {
        const filename = decodeURIComponent(requestHeaders["x-goog-upload-file-name"]), contentLength = requestHeaders["x-goog-upload-header-content-length"], gogoleUploadId = response.headers["x-guploader-uploadid"], startedAt = response.config._startedAt;
        mailCache.startUpload({
          name: filename,
          size: contentLength
        }, gogoleUploadId, startedAt);
      } else if ("upload, finalize" === xGoogUploadCommand) {
        const uploadId = new URL(url).searchParams.get("upload_id");
        if (!uploadId) return void logError({
          error: "upload_id not found in finalize request",
          requestHeaders,
          responseHeaders: response.headers,
          url
        });
        const responseBase64 = response.response;
        if (!/^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)?$/.test(responseBase64)) return void logError({
          error: "response not base64",
          requestHeaders,
          responseHeaders: response.headers,
          url
        });
        mailCache.finishUpload(uploadId, responseBase64);
      } else if ("cancel" === xGoogUploadCommand) {
        const uploadId = new URL(url).searchParams.get("upload_id");
        mailCache.cancelUpload(uploadId);
      } else logError({
        error: "unknown upload command",
        requestHeaders,
        responseHeaders: response.headers,
        url
      });
    }(response) : url.startsWith("https://upload.docs.google.com") ? function(response) {
      const uploadId = new URL(response.config.url).searchParams.get("upload_id"), [attachmentUrl, attachmentDriveData] = function(response) {
        try {
          const responseBody = JSON.parse(response);
          if ("FINALIZED" === responseBody.sessionStatus.state) {
            const {status, customerSpecificInfo} = responseBody.sessionStatus.additionalInfo["uploader_service.GoogleRupioAdditionalInfo"].completionInfo;
            if ("SUCCESS" === status) {
              const [action] = JSON.parse(customerSpecificInfo.actions);
              if (action.url) return [ action.url, customerSpecificInfo ];
            }
          }
        } catch {
          logError({
      ...
      if (attachmentUrl && attachmentDriveData && uploadId) {
        const upload = mailCache.retrieveUpload(uploadId);
        if (upload) {
          const {fileSize, id, mimeType, name} = attachmentDriveData;
          dispatchOnPage("Cyberhaven_UploadCloudData", {
            files: [ {
              id,
              title: name,
          ...
          }), mailCache.addInlineAttachment(attachmentUrl, {
            driveId: id,
            fileSize,
            mimeType,
            name
    ...
    }(response) : url.includes("/upload/drive/resumable") || url.includes("/upload/resumableuploadsc") ? function(response) {
      const responseBody = JSON.parse(response.response);
      if ("OPEN" === responseBody.sessionStatus?.state) {
        const uploadId = responseBody.sessionStatus.upload_id, formData = JSON.parse(response.config.body), [name, size, startedAt] = function(formData) {
          const nameField = formData.createSessionRequest.fields.find((field => "file" === field.external?.name)), finalName = nameField.external.filename, size = nameField.external.size, addedTs = formData.createSessionRequest.fields.find((field => "addtime" === field.inlined?.name));
          return [ finalName, size, parseInt(addedTs.content, 10) || Date.now() ];
        }(formData);
        name && startedAt && mailCache.startUpload({
          name,
          size
        }, uploadId, startedAt);
      }
    }(response) : url.includes("/drive/v2internal/attachments?") && function(response) {
      const {messageId, partId} = JSON.parse(response.config.body), {fileId, kind} = JSON.parse(response.response), {emailId, attachmentId, mimeType, fileName, downloadUrl} = parseDownloadUrl(messageId, partId);
      if ("drive#attachment" !== kind) return void logError({
        error: "unknown response kind",
        downloadUrl,
        response,
        body: response.config.body
      });
      emailId && attachmentId && dispatchOnPage("Cyberhaven_SaveAttachment", {
        attachmentId,
        fileId,
        emailId,
        fileName,
  ...
  const proxyOnRequest = measureTimeExecution("com.google.mail:send-email", (async config => {
    try {
      if (config._startedAt = Date.now(), /https:\/\/mail.google.com\/sync\/(u\/\d+\/)?i\/s.*/.test(config.url)) {
        const email = processEmailData(config.body);
        if (email) if (config.body.includes(LABELS.SCHEDULED) || config.body.includes(LABELS.SENT)) {
          const attachments = function(email) {
            return email.attachments.map((attachment => ({
              attachmentId: attachment.xAttachmentId,
              name: attachment.name,
              size: attachment.size
            })));
          }(email), batchRequest = function(recipients, attachments, emailId, subject) {
            const requests = [];
          ...
          }(email.recipients, attachments, email.msgId, email.subject);
          if (batchRequest.requests.length) {
            const response = await dispatchOnPageWithReply("Cyberhaven_SentEmail", batchRequest);
            if ("success" === response?.status && "block" === response.result) return function(config) {
              return {
                status: 502,
                config,
                headers: {},
                response: '["er",null,null,null,null,502,null,null,null,3]'
              };
            }(config);
          }
        } else mailCache.hasCompletedUploads() ? function(email) {
          for (const attachment of email.attachments) {
            const uploadId = mailCache.popCompletedUpload(attachment.uploadResponseBase64);
            uploadId && dispatchOnPage("Cyberhaven_AddEmailAttachment", {
              uploadId,
              emailId: email.msgId,
              attachmentId: attachment.xAttachmentId,
              name: attachment.name,
        ...
        }(email) : mailCache.hasDriveSelection() && function(email) {
          if (mailCache.isDriveSelectionIsAttachmentLike()) for (const attachment of email.attachments) {
            const cacheItem = mailCache.popDocFromDriveCache((item => item.id === attachment.driveId));
            cacheItem && dispatchOnPage("Cyberhaven_AddEmailAttachment", {
              emailId: email.msgId,
              attachmentId: attachment.xAttachmentId,
              cloudFileId: cacheItem.id,
              name: attachment.name,
              size: attachment.size
            });
          }
  ...
  const {unProxy} = function(proxy, win) {
    return function(proxy, win) {
      ...
      function getResponseData(xhrProxy) {
        ...
        if (!responseType || "text" === responseType) return xhrProxy.responseText;
        var response = xhrProxy.response;
        if ("json" === responseType && !response) try {
      ...
      function handleResponse(xhr, xhrProxy) {
        var handler = new ResponseHandler(xhr), ret = {
          ...
          headers: xhr.resHeader || xhr.getAllResponseHeaders().split("\r\n").reduce((function(ob, str) {
            if ("" === str) return ob;
            var m = str.split(":");
            return ob[m.shift()] = function(str) {
      ...
      function onerror(xhr, xhrProxy, error, errorType) {
        ...
        error = {
          ...
          error,
          type: errorType
        }, onError ? onError(error, handler) : handler.next(error);
      }
      ...
      function errorCallback(errorType) {
        return function(xhr, e) {
          return onerror(xhr, this, e, errorType), !0;
        };
      }
      function stateChangeCallback(xhr, xhrProxy) {
        return 4 === xhr.readyState && 0 !== xhr.status ? handleResponse(xhr, xhrProxy) : 4 !== xhr.readyState && triggerListener(xhr, eventReadyStateChange), 
        !0;
      }
      var {originXhr, unHook} = function(proxy, win) {
        var originXhr = (win = win || window).XMLHttpRequest, hooking = !0, HookXMLHttpRequest = function() {
          for (var xhr = new originXhr, i = 0; i < events.length; ++i) {
            var key = "on" + events[i];
            void 0 === xhr[key] && (xhr[key] = null);
          }
          for (var attr in xhr) {
            var type = "";
            try {
            ...
            "function" === type ? this[attr] = hookFunction(attr) : attr !== OriginXhr && Object.defineProperty(this, attr, {
        ...
        function getterFactory(attr) {
          return function() {
            ...
            if (hooking) {
              var v = this.hasOwnProperty(attr + "_") ? this[attr + "_"] : originValue, attrGetterHook = (proxy[attr] || {}).getter;
        ...
        function setterFactory(attr) {
          return function(v) {
            ...
            if (hooking) {
              var that = this, hook = proxy[attr];
              if ("on" === attr.substring(0, 2)) that[attr + "_"] = v, xhr[attr] = function(e) {
                e = configEvent(e, that), proxy[attr] && proxy[attr].call(that, xhr, e) || v.call(that, e);
              }; else {
                var attrSetterHook = (hook || {}).setter;
                v = attrSetterHook && attrSetterHook(v, that) || v, this[attr + "_"] = v;
                try {
                  xhr[attr] = v;
                } catch (e) {}
              }
        ...
        return (HookXMLHttpRequest.prototype = originXhr.prototype).constructor = HookXMLHttpRequest, 
        win.XMLHttpRequest = HookXMLHttpRequest, Object.assign(win.XMLHttpRequest, {
          UNSENT: 0,
          OPENED: 1,
          HEADERS_RECEIVED: 2,
          LOADING: 3,
          ...
          originXhr,
          unHook: function() {
            hooking = !1, win.XMLHttpRequest === HookXMLHttpRequest && (win.XMLHttpRequest = originXhr, 
            HookXMLHttpRequest.prototype.constructor = originXhr, originXhr = void 0);
          }
        };
      }({
        ...
        open: function(args, xhr) {
          var _this = this, config = xhr.config = {
            headers: {}
          };
          config.method = args[0], config.url = args[1], config.async = args[2], config.user = args[3], 
          config.password = args[4], config.xhr = xhr;
        ...
        send: function(args, xhr) {
          ...
          if (config.withCredentials = xhr.withCredentials, config.body = args[0], onRequest) {
            var req = function() {
              onRequest(config, new RequestHandler(xhr));
            };
            return !1 === config.async ? req() : setTimeout(req), !0;
          }
        },
        setRequestHeader: function(args, xhr) {
          if (xhr.config.headers[args[0].toLowerCase()] = args[1], onRequest) return !0;
        },
        addEventListener: function(args, xhr) {
          var _this = this;
          if (-1 !== events.indexOf(args[0])) {
            var handler = args[1];
            return getEventTarget(xhr).addEventListener(args[0], (function(e) {
              var event = configEvent(e, _this);
              event.type = args[0], event.isTrusted = !0, handler.call(_this, event);
            })), !0;
          }
        },
    ...
    }(proxy, win = win || window);
  }({
    onRequest: async (config, handler) => {
      const customResponse = await proxyOnRequest(config);
      if (customResponse) return handler.resolve(customResponse);
      handler.next(config);
    },
    onResponse: (response, handler) => {
      handler.next(response), proxyOnResponse(response);
    }
  });
  var callback;
  callback = unProxy, getSignal().addEventListener("abort", callback), window.addEventListener("message", (event => {
    try {
      if ("string" == typeof event.data && event.data.startsWith("!_")) {
        const parsed = JSON.parse(event.data.substring(2)), attachments = JSON.parse(parsed.a[0]["iframe-command"])[1][1];
        mailCache.gdriveCache = {
          selection: attachments.map((item => ({
            id: item[0][0],
            name: item[1],
            driveId: item[3],
            mimeType: item[6],
            isAttachment: Boolean(item[30])
          })))
        };
      }
    } catch {}
