(() => {
  ...
  }, getSignal = () => abortController.signal, browser = __webpack_require__.g.browser || __webpack_require__.g.chrome;
  ...
  function reverseProxy(sourceEventName, destEventName) {
    onMessage(sourceEventName, (({data}) => function(eventName, data, {timeout = 1e3, wrapVersion = addVersionToEventNameByDefault} = {}) {
  ...
  })), XHR_WHITELIST_ORIGINS = [ "https://drive.google.com", "https://docs.google.com" ], XHR_ALLOWED_IFRAMES = [ "/static/proxy.html", "/driveshare", "/picker/v2/home", "/picker" ];
  CyberhavenDomEvents.register({
    getPageData: window.self === window.top ? getPageData : void 0,
    domExtras: {
      dynamicInputs: !0
    }
  }), window.self === window.top && (onMessage("get-page-data", getPageData), onMessage("get-document-title", measureTimeExecution("com.google:get-document-title", (() => function() {
    ...
    return null;
  }() ? getCurrentDocumentTitle() : null)))), (window.self === window.top ? XHR_WHITELIST_ORIGINS.includes(location.origin) : XHR_ALLOWED_IFRAMES.find((path => location.pathname.includes(path)))) && async function() {
    const [isGoogleWorkspaceTrackingEnabled, isFullPathTrackingEnabled] = await Promise.all([ service_communication_sendMessageToBackground("get-webapp-status", "google_workspace"), service_communication_sendMessageToBackground("get-webapp-status", "google_workspace_fullpath") ]);
    isGoogleWorkspaceTrackingEnabled && (injectScript("google_xhr", "src/apps/com.google/xhr.web.ts", {
      trackFullpath: !!isFullPathTrackingEnabled
    }), dom_proxy_proxy("Cyberhaven_UploadCloudData", "upload-cloud-data"), dom_proxy_proxy("Cyberhaven_ShareData", "share-data"), 
    dom_proxy_proxy("Cyberhaven_RenameCloudFile", "rename-cloud-file"), dom_proxy_proxy("Cyberhaven_CopyCloudFile", "copy-cloud-file"), 
    dom_proxy_proxy("Cyberhaven_WebAppStorage", "webapp-storage"), dom_proxy_proxy("Cyberhaven_Log", "log"), 
    reverseProxy("get-fullpath", "Cyberhaven_GetFileFullPath"));
  }();
})();