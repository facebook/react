(() => {
  ...
  const globals_isWebWorkerContext = () => "function" == typeof importScripts && !getContextName().includes("background"), abortController = (globals_isWebWorkerContext() ? self : window, 
  ...
  location.hostname.includes("officeapps.live.com") && (CyberhavenDomEvents.register(), 
  async function({cssSelector, elementXPath, document = window.document, commonParentXPath}) {
    let nextPasteInfo;
    listenPageEvent("Cyberhaven_IsPasteAllowed", (({detail}) => {
      ...
      blockPaste && event.stopImmediatePropagation(), service_communication_sendMessageToBackground("copy-paste", {
        action: "paste",
    ...
    elementXPath: "//*[@data-unique-id = 'ContextMenu-Paste']"
  }));
})();