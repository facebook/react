(() => {
  ...
  const XPATHS_cloudAccount = '//*[local-name()="header" or @role="menubar" or @id="gb"]//*[(self::a or self::button) and contains(@aria-label, "@") and (//img or //svg)]/@aria-label', XPATHS_cloudAccount2 = '//a[contains(@aria-label, "@") or contains(@aria-label, "Google")]/@aria-label', XPATHS_debugCloudAccount = '//a[contains(@aria-label, "@") or contains(@aria-label, "Google")]', XPATHS_gcloudAccount = '//*[@id="pcc-account-chooser-container"]//button[contains(@aria-label, "@") and ./img]/@aria-label', XPATHS_htmlOnlyCloudAccount = '//*[@id="guser"]//*[contains(text(), "@") and not(contains(text(), " "))]/text()', XPATHS_lockerstudioCloudAccount = '//*[contains(local-name(), "header")]//button[contains(@class, "suite") and //img[contains(@alt, "@")]]//@alt';
  ...
  function getAccountEmail() {
    let result;
    try {
      if (result = tryExtractEmail(XPATHS_cloudAccount) || tryExtractEmail(XPATHS_cloudAccount2), 
      !result) if (location.hostname.includes("console.cloud.google.com")) result = tryExtractEmail(XPATHS_gcloudAccount); else {
        if (/^https:\/\/mail\.google\.com\/mail\/\w*\/\d\/h\//.test(location.href)) {
          const profile = xpath_xpath(XPATHS_htmlOnlyCloudAccount);
          return profile[0]?.textContent ?? "";
        }
        if (function() {
          const view = new URLSearchParams(window.location.search).get("view") || "";
          return [ "lg", "pt" ].includes(view);
        }()) result = function() {
          const emailElements = document.querySelectorAll(".bodycontainer font b");
          for (const element of emailElements) {
            const text = element.parentElement?.textContent, email = extractEmail(text || "");
            if (email) return email;
          }
          return;
        }(); else if ("https://lookerstudio.google.com" === window.origin) return tryExtractEmail(XPATHS_lockerstudioCloudAccount);
      }
      if (!result) {
        const coldStartEl = document.querySelector("#googlebar-coldstart"), tooltip = coldStartEl?.getAttribute("data-tooltip");
        tooltip && (result = extractEmail(tooltip));
      }
    } catch {}
    return result;
  }
  function tryExtractEmail(xpathExpression) {
    const profile = xpath_xpath(xpathExpression);
    if (profile[0]?.textContent) return extractEmail(profile[0].textContent);
  }
  ...
  function getCurrentDocumentTitle() {
    return location.pathname.startsWith("/forms/d/") ? document.querySelector('div[data-action-id="freebird-forms-home"] + div input[data-initial-value]')?.getAttribute("data-initial-value") : document.getElementById("docs-title-input-label-inner")?.textContent || function() {
      ...
      return jsonParseNoException(node[0]?.textContent ?? "")?.title ?? void 0;
    }() || null;
  }
  function getFileIdFromDocument() {
    if ("docs.google.com" !== location.hostname) return null;
    if (void 0 !== window._docs_flag_initialData && window._docs_flag_initialData["docs-ci"]) return window._docs_flag_initialData["docs-ci"];
    const {id: pathnameDocumentId} = (() => {
      const match = window.location.pathname.match(/\/(?<productType>document|spreadsheets|presentation|videos|forms|drawings)\/d\/(?<id>[a-zA-Z0-9_-]+)/);
      return {
        productType: match?.groups?.productType,
        id: match?.groups?.id
      };
    })();
    return pathnameDocumentId ?? function() {
      const node = xpath_xpath('//*[@id="drive-active-item-info"]');
      return jsonParseNoException(node[0]?.textContent ?? "")?.id ?? null;
    }();
  }
  const testEmailPattern = /https:\/\/drive\.google\.com\/drive\/u\/\d+\/[a-z-]+|https:\/\/mail\.google\.com\/mail\/u\/\d\/.*/, testEmailPresence = function(func, timeFrame) {
    let lastTime = 0;
    return function(...args) {
      const now = new Date;
      now.getTime() - lastTime >= timeFrame && (lastTime = now.getTime(), func(...args));
    };
  }((pageData => {
    try {
      const isPageLoaded = "complete" === document.readyState, hasNoEmail = !pageData.metadata?.email, matchesTestEmailPattern = testEmailPattern.test(pageData.url || "");
      if (isPageLoaded && hasNoEmail && matchesTestEmailPattern) {
        const emailElement = xpath_xpath(XPATHS_debugCloudAccount)[0];
        error = {
          error: "Missing com.google account email for known url",
          html: emailElement?.outerHTML
        }, page_logging_logError({
          ...error,
    ...
    var error;
  }), 1e4), getPageData = measureTimeExecution("com.google:get-page-data", (() => {
    const pageData = (pageData => {
      ...
      default_object_type: "cloud_file_gdrive",
      metadata: {
        email: getAccountEmail(),
        account_type: "gsuite",
        cloud_file_id: getFileIdFromDocument(),
        cloud_file_name: getCurrentDocumentTitle()
      }
    });
    return setTimeout((() => {
      testEmailPresence(pageData);
    })), pageData;
  }));
  async function isGmailAppEnabled() {
    const [gmail, salesforceAddin] = await Promise.all([ await service_communication_sendMessageToBackground("get-webapp-status", "gmail"), await service_communication_sendMessageToBackground("get-extension-info", "jjghhkepijgakdammjldcbnjehfkfmha") ]);
    return Number(gmail) > 0 && (!salesforceAddin || !salesforceAddin.enabled);
  }
  async function injectGmailProxy() {
    await isGmailAppEnabled() && (dom_proxy_proxy("Cyberhaven_AddEmailAttachment", "email-attachment"), 
    dom_proxy_proxy("Cyberhaven_SaveAttachment", "save-attachment"), dom_proxy_proxy("Cyberhaven_UploadCloudData", "upload-cloud-data"), 
    dom_proxy_proxy("Cyberhaven_SentEmail", "email-sent"), dom_proxy_proxy("Cyberhaven_DownloadCloudFile", "download-cloud-file"), 
    dom_proxy_proxy("Cyberhaven_Log", "log"), injectScript("gmail_xhr", "src/apps/com.google.mail/xhr.web.ts"));
  }
  CyberhavenDomEvents.register({
    getPageData: window.self === window.top ? getPageData : void 0
  }), window.self === window.top ? (service_communication_onMessage("get-page-data", getPageData), 
  injectGmailProxy()) : location.pathname.endsWith("/picker") ? injectGmailProxy() : location.href.includes("/driveshare") && async function() {
    await isGmailAppEnabled() && (dom_proxy_proxy("Cyberhaven_ShareData", "share-data"), 
    dom_proxy_proxy("Cyberhaven_Log", "log"), injectScript("gmail_drive_share", "src/apps/com.google.mail/xhr.share.web.ts"));
  }();
})();