(() => {
  ...
  var __webpack_modules__ = {
    8556: (__unused_webpack_module, exports) => {
      ...
      exports.t = function(email) {
        if (!email) return !1;
        if (email.length > 254) return !1;
        if (!tester.test(email)) return !1;
        var parts = email.split("@");
        return !(parts[0].length > 64) && !parts[1].split(".").some((function(part) {
          return part.length > 63;
        }));
      };
    }
  }, __webpack_module_cache__ = {};
  const abortController = new AbortController, unsubscribe = () => {
  ...
  const globals_isWebWorkerContext = () => "function" == typeof importScripts && !function() {
    ...
    return "unknown";
  }().includes("background"), awaitTimeout = (globals_isWebWorkerContext() ? self : window, 
  (delay, reason, defaultResolve = void 0) => new Promise(((resolve, reject) => setTimeout((() => void 0 === reason ? resolve(defaultResolve) : reject(reason)), delay))));
  var util, objectUtil;
  !function(util) {
  ...
  const getCurrentScriptId = () => currentScript?.getAttribute("scriptId"), optimistically_injected_isCurrentScriptOptimisticallyInjected = () => !(!getCurrentScriptId() || globals_isWebWorkerContext()) && optimisticallyInjectedScripts.includes(getCurrentScriptId());
  ...
  function testIsShareLink(url) {
    return function(url, pattern) {
      return url instanceof URL && (url = url.href), new RegExp(pattern).test(url);
    }(getUrlPathname(url), "/ShareLink");
  }
  function testIsShareLinkOneDrivePersonal(url) {
    return getUrlPathname(url).endsWith("/share");
  }
  function testIsShareObjectRequest(url) {
    return getUrlPathname(url).endsWith("/ShareObject");
  }
  ...
  const jsonParseNoException = json => {
    if (json) try {
  ...
  var email_validator = function __webpack_require__(moduleId) {
  ...
  const OfficeRoles = {
    ...
    "roletype:Reviewer": "commenter",
    "role:1073741827": "writer",
    "role:1073741826": "reader",
    "role:1073741832": "reader"
  }, OfficeAccessBy = {
    0: "anyone",
    1: "audience",
    2: "user"
  }, sharingRequestsMap = new WeakMap;
  async function handleAccessSharingRequestConfig(config, sharingInfo) {
    const {url, body} = config;
    if ("string" != typeof body) return body;
    const isCorporateSite = testIsShareLink(url) || testIsShareObjectRequest(url), isPersonalSite = testIsShareLinkOneDrivePersonal(url);
    if (!isCorporateSite && !isPersonalSite) return body;
    const requestBody = jsonParseNoException(body);
    invariant(requestBody, "Failed to parse request body");
    const effectiveBody = testIsShareLink(url) ? requestBody.request : requestBody, response = await async function(url, requestData, config, sharingInfo) {
      const isNewSharingExperience = testIsShareObjectRequest(url);
      try {
        if (requestData.settings?.inviteesToRemove?.results?.length) return {
          result: "allow"
        };
        const users = function(peoplePickerInput) {
          if (!peoplePickerInput) return [];
          const parsedInput = jsonParseNoException(peoplePickerInput);
          return invariant(Array.isArray(parsedInput), "Failed to parse peoplePickerInput"), 
          parsedInput.map((user => {
            let email = user.EntityData?.Email;
            if (email) return email;
            if (user.Key.includes("|")) {
              const keyParts = user.Key.split("|");
              return email = keyParts[keyParts.length - 1], invariant((0, email_validator.t)(email), "Failed to parse email from key: %s", user.Key), 
              email;
            }
            return (0, email_validator.t)(user.Key) ? user.Key : (invariant((0, email_validator.t)(user.Description), "Failed to parse email from description: %s", user.Description), 
            user.Description);
          }));
        }(requestData.peoplePickerInput), contextInfo = window._sharingContextInfo, {uniqueId, itemName, webAbsoluteUrl, clientId, sharingInformation: ctxSharingInfo} = contextInfo;
        (sharingInfo = sharingInfo || ctxSharingInfo) || (sharingInfo = await async function() {
          const {listItemId, listId, webAbsoluteUrl} = window._sharingContextInfo, endpoint = webAbsoluteUrl + "/_api/web/Lists(@a1)/GetItemById(@a2)/GetSharingInformation", url = new URL(endpoint);
          url.searchParams.set("__a1", `'${listId}'`), url.searchParams.set("__a2", `'${listItemId}'`);
          const response = await fetch(url.toString().replaceAll("__a", "@a"), {
            headers: {
        ...
        const fileDetails = await async function(clientId, sharingInfo, contextInfo) {
          const {authToken, itemName, webAbsoluteUrl, uniqueId, sharingInformation: ctxSharingInfo} = contextInfo;
          if (authToken && ctxSharingInfo?.itemUrl) {
            const {itemUrl} = ctxSharingInfo, url = new URL(itemUrl);
            url.searchParams.set("$select", "name,eTag,size,webUrl,parentReference,sharepointIds"), 
            url.searchParams.set("$expand", "listItem($expand=list)");
            ...
            if (r.ok) {
              ...
              return {
                fileId: trimCurlyBrackets(data.eTag).toLowerCase(),
                fileName: data.name,
                fileSize: String(data.size),
                filePath: decodeURIComponent(new URL(data.listItem.webUrl).pathname),
                fileUrl: data.listItem.webUrl
              };
            }
          }
          if ("officecom" === clientId) {
            const sharingInfoWebUrlPath = new URL(sharingInfo.webUrl).pathname, documentPathParts = new URL(sharingInfo.directUrl).pathname.split(sharingInfoWebUrlPath), filePath = documentPathParts.length > 1 ? new URL(sharingInfo.webUrl).pathname + decodeURIComponent(documentPathParts[1]) : "", fileUrl = filePath ? encodeURI(new URL(sharingInfo.webUrl) + documentPathParts[1]) : "";
            return {
              fileId: sharingInfo?.itemUniqueId || uniqueId || ctxSharingInfo?.itemUniqueId,
              fileName: sharingInfo?.displayName || itemName,
              filePath: filePath || sharingInfo?.displayName || itemName,
              fileUrl
            };
          }
          {
            const fileData = await async function({listId, listItemId, uniqueId}) {
              const {webAbsoluteUrl} = window._sharingContextInfo;
              invariant(webAbsoluteUrl, "Web absolute URL not found");
              const fieldsParam = [ "ETag", "ServerRelativeUrl", "Name", "Length" ].join(","), url = uniqueId ? `${webAbsoluteUrl}/_api/web/GetFileById('${uniqueId}')?$select=${fieldsParam}` : `${webAbsoluteUrl}/_api/web/lists(guid'${listId}')/items(${listItemId})/File?$select=${fieldsParam}`, response = await fetch(url, {
                headers: {
                  Accept: "application/json;odata=verbose"
                }
              });
              invariant(response.ok, "Failed to get file in list %s;%s;%s", uniqueId, listId, listItemId);
              const data = await response.json();
              return null === data.d.File ? null : data.d;
            }({
              listId: contextInfo.listId,
              listItemId: contextInfo.listItemId,
              uniqueId: uniqueId || ctxSharingInfo?.itemUniqueId
            });
            return fileData ? {
              fileId: trimCurlyBrackets(fileData.ETag).toLowerCase(),
              fileName: fileData.Name,
              filePath: fileData.ServerRelativeUrl,
              fileSize: String(fileData.Length),
              fileUrl: webAbsoluteUrl + fileData.ServerRelativeUrl
            } : null;
          }
        }(clientId, sharingInfo, contextInfo);
        if (!fileDetails) return {
          result: "allow"
        };
        fileDetails.fileId = fileDetails.fileId || uniqueId || ctxSharingInfo?.itemUniqueId, 
        fileDetails.fileName = fileDetails.fileName || itemName, fileDetails.fileUrl = fileDetails.fileUrl || webAbsoluteUrl;
        const role = isNewSharingExperience ? NewOfficeRoles[requestData.roleValue] : OfficeRoles[requestData.settings.role], accessType = void 0 !== requestData.settings?.scope ? OfficeAccessBy[requestData.settings.scope] : "user", sharingRequests = [ {
          ...fileDetails,
          role,
          type: accessType,
          userIds: users
        } ], beReply = await dispatchOnPageWithReply("Cyberhaven_ShareData", {
          requests: sharingRequests
        });
        if ("success" === beReply?.status) return sharingRequestsMap.set(config.xhr, sharingRequests), 
        beReply;
      } catch (error) {
        logError({
          error: "Error occurred in handleAccessSharingRequest in SharePoint",
    ...
    }(url, effectiveBody, config, sharingInfo);
    return "block" === response?.result ? null : body;
  }
  let promise;
  ...
  function matchSharingBeforeAfter(method, url) {
    const urlStr = url.toString();
    return "POST" === method && (testIsShareLink(urlStr) || testIsShareLinkOneDrivePersonal(urlStr) || testIsShareObjectRequest(urlStr));
  }
  ((service, scriptId) => {
    ((value = !0) => {
      addVersionToEventNameByDefault = !optimistically_injected_isCurrentScriptOptimisticallyInjected() && value;
    })(), scriptId ? onServiceReady(service, (() => {
      registerEvents(service, scriptId);
    })) : registerEvents(service, scriptId);
  })("office_365", "sharepoint_xhr");
  const [sharingInfoPromise, resolveSharingInfo] = function() {
    let res, err;
    ...
    sharingInfo: void 0,
    promise: sharingInfoPromise,
    resolve: resolveSharingInfo
  };
  const {unhook, enableHook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
    ...
    return isXhrHooked = !0, enabled = !optimistically_injected_isCurrentScriptOptimisticallyInjected(), 
    ...
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: "/_api/web/.*/GetSharingInformation?.*"
    } ],
    after: function({responseText, config}) {
      if (200 !== config.xhr?.status) return;
      const parsed = jsonParseNoException(responseText);
      invariant(parsed.d, "Failed to parse response");
      const info = {
        displayName: parsed.d.displayName,
        itemUniqueId: parsed.d.itemUniqueId,
        siteId: parsed.d.siteId,
        webUrl: parsed.d.webUrl,
        directUrl: parsed.d.directUrl
      };
      sharingContext.sharingInfo = info, sharingContext.resolve(info);
    }
  }, {
    matchUrlBefore: matchSharingBeforeAfter,
    matchUrlAfter: matchSharingBeforeAfter,
    before: async function(_body, config) {
      return await Promise.race([ sharingContext.promise, awaitTimeout(500) ]), await handleAccessSharingRequestConfig(config, sharingContext?.sharingInfo);
    },
    after: async function(response) {
      if (200 !== response.config.xhr?.status) return;
      const shareResponse = jsonParseNoException(response.responseText)?.d;
      shareResponse && !shareResponse.ErrorMessage && await async function(xhr) {
        const sharingRequests = sharingRequestsMap.get(xhr);
        if (!sharingRequests) return;
        const requestId = xhr.getResponseHeader("request-id");
        invariant(requestId, "Request ID not found");
        const updatedRequests = sharingRequests.map((request => ({
          ...request,
          requestId
        })));
        await dispatchOnPage("Cyberhaven_ShareData", {
  ...
  var callback;
  onServiceReady("office_365", enableHook), callback = unhook, abort_controller_getSignal().addEventListener("abort", callback);
})();