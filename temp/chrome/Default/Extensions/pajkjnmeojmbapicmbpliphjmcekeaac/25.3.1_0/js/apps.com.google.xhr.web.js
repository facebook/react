(() => {
  ...
  function hookXhr(options, {enablePreviousScriptOnReload = !1} = {}) {
  ...
  function generateGroupRequestKey(request) {
    const {fileId, permissionId, payload} = request, additionalRoles = `[${payload.additionalRoles?.join(".")}]`;
    return [ fileId, permissionId, payload.type, payload.role, additionalRoles ].join(";;");
  }
  function getGoogleAppName(origin) {
    if (origin) return new URL(origin).hostname.split(".")[0];
    const error = "Unknown Pathname format", topLocation = window.top?.location;
    if (topLocation) {
      const parts = topLocation.pathname.split("/");
      return parts.length > 1 ? parts[1] : error;
    }
    return error;
  }
  function constants_logError(error) {
    logError({
      ...error,
      page_app: getGoogleAppName(),
      url: location.href
    });
  }
  function parseResponseTextSplitter(contentType) {
    if (!contentType) return;
    const parts = contentType.split("boundary=");
    return parts?.[1] ? "--" + parts[1] + "\r\n" : void 0;
  }
  const body_parsers_jsonParseNoException = json => {
    if (json) try {
  ...
  const isFullPathOn = "true" === document.currentScript?.dataset.trackFullpath, TOP_FRAME_ORIGINS = [ "https://drive.google.com", "https://docs.google.com" ], {dispatchOnWindow, dispatchOnWindowWithReply, onTopFrameWindow} = function(topFrameOrigins, communicationTimeout = 2500) {
    return {
  ...
  }(TOP_FRAME_ORIGINS, 2500);
  async function getTeamdrive(teamdriveId) {
    return isFullPathOn && TOP_FRAME_ORIGINS.includes(window.origin) ? (await dispatchOnWindowWithReply("ch_get_teamdrive", {
      teamdriveId
    }))?.teamdrive : Promise.resolve(void 0);
  }
  async function getPathFromMainWindow(parentId) {
    return isFullPathOn ? (await dispatchOnWindowWithReply("ch_get_path", {
      parentId,
      useApi: window.self === window.top
    }))?.path : Promise.resolve([]);
  }
  const getApiClient = () => window.gapi?.client;
  let tokenInfo, resolveTokenInfo;
  function getDriveQueryParams(query) {
    const searchParams = new URLSearchParams(query), driveOptions = {
      supportsTeamDrives: "true" === searchParams.get("supportsTeamDrives"),
      languageCode: searchParams.get("languageCode"),
      ensureDiscoverableParent: "true" === searchParams.get("ensureDiscoverableParent"),
      enforceSingleParent: "true" === searchParams.get("enforceSingleParent")
    };
    return Object.entries(driveOptions).filter((([, value]) => Boolean(value))).reduce(((obj, [key, value]) => (obj[key] = String(value), 
    obj)), {});
  }
  async function getFullPathViaApi(id, query) {
    const apiClient = getApiClient(), path = [], start = performance.now();
    let currentId = id;
    for (;currentId; ) try {
      const performanceDiff = performance.now() - start;
      if (performanceDiff > 2500) {
        constants_logError({
          error: "getFullPathViaApi timeout",
          id,
          currentId,
          path,
          performanceDiff
      ...
      if (!tokenInfo) return constants_logError({
        error: "getFullPathViaApi error tokenInfo is not present"
      }), path;
      const cacheItem = getFromCache(currentId);
      if (cacheItem) path.unshift({
        title: cacheItem.title,
        id: cacheItem.id,
        isTeamDrive: cacheItem.teamDriveId === cacheItem.id
      }), currentId = cacheItem.parents[0]?.id; else {
        const {result} = await apiClient.request({
          method: "GET",
          path: `/drive/v2internal/files/${currentId}?key=${tokenInfo.key}`,
          params: {
            fields: "id,title,kind,mimeType,parents,teamDriveId",
            ...getDriveQueryParams(query)
          },
          headers: tokenInfo.headers
        });
        saveItem(result.id, result), path.unshift({
          title: result.title,
          id: result.id,
          isTeamDrive: result.teamDriveId === result.id
        }), currentId = result.parents[0]?.id;
      }
    } catch (error) {
      break;
    }
    if (path[0]?.isTeamDrive) {
      let teamDrive = await getTeamdrive(path[0].id);
      teamDrive || (teamDrive = await async function(teamDriveId, query) {
        const apiClient = getApiClient(), {result} = await apiClient.request({
          method: "GET",
          path: `/drive/v2internal/teamdrives/${teamDriveId}?key=${tokenInfo.key}`,
          params: {
            fields: "id,name,kind",
            ...getDriveQueryParams(query)
          },
        ...
        return saveItem(result.id, result), result;
      }(path[0].id, query)), path[0].title = teamDrive.name;
    }
    return path;
  }
  onTopFrameWindow("ch_set_apitoken", (async data => {
    tokenInfo = data, resolveTokenInfo?.(data);
  })), window.CH_DATA = window.CH_DATA ?? {
    cache: new Map,
    teamdrives: new Map
  };
  const {cache: object_cache_cache, teamdrives} = window.CH_DATA, object_cache_isFullPathOn = "true" === document.currentScript?.dataset.trackFullpath;
  function saveItem(key, value) {
    if (window.top !== window.self && "https://drive.google.com" === window.origin) dispatchOnWindow("ch_save_item", {
      key,
      value
    }); else if ("https://clients6.google.com" === window.origin) {
      const parentOrigin = new URLSearchParams(location.hash?.substring(1)).get("parent");
      parentOrigin && parent.postMessage({
        event: "save_to_cache",
        payload: {
          key,
          value
    ...
    } else if ("drive#teamDrive" === value.kind) {
      const extended = {
        ...teamdrives.get(key),
        ...value
      };
      teamdrives.set(key, extended);
    } else {
      const extended = {
        ...object_cache_cache.get(key),
        ...value
      };
      object_cache_cache.set(key, extended);
    }
  }
  function getFolderTitle(id) {
    return teamdrives.has(id) ? teamdrives.get(id)?.name : object_cache_cache.get(id)?.title;
  }
  onTopFrameWindow("ch_get_path", (async payload => {
    let path = function(parentId) {
      if (!object_cache_isFullPathOn) return null;
      const userDrive = function() {
        try {
          const initData = JSON.stringify(window.__initData);
          if (!initData) return null;
          const match = initData.match(/\["(?<id>[a-zA-Z0-9]*?)",(\[\]|null),"(?<name>[a-zA-Z\s]*)","application\/vnd\.google-apps\.folder"/);
          return match ? {
            id: match.groups.id,
          ...
          return constants_logError({
            error,
            fn: "getWindowCurrentDriveData"
          }), null;
        }
      }();
      userDrive?.id && saveItem(userDrive.id, userDrive);
      const pathname = new URL(location.href).pathname, currentFolderParts = parentId ? [ "", parentId ] : pathname.split("/folders/");
      if (2 === currentFolderParts.length) {
        const currentFolderId = currentFolderParts[1], fullPath = [];
        let current = object_cache_cache.get(currentFolderId);
        for (;current; ) {
          fullPath.unshift({
            title: getFolderTitle(current.id) || current.title,
            id: current.id
          });
          const parentId = current.parents[0]?.id;
          if (!parentId) break;
          if (current = object_cache_cache.get(current.parents[0]?.id), !current) return null;
        }
        return fullPath;
      }
      return [];
    }(payload.parentId);
    return !payload.parentId || path && path.length || !payload.useApi || (path = await getFullPathViaApi(payload.parentId, "supportsTeamDrives=true")), 
    {
      path: path || void 0
    };
  })), onTopFrameWindow("ch_get_teamdrive", (async payload => ({
    teamdrive: teamdrives.get(payload.teamdriveId)
  }))), onTopFrameWindow("ch_save_item", (async payload => {
    saveItem(payload.key, payload.value);
  }));
  const getItemById = async id => await async function(id, query) {
    try {
      tokenInfo || await Promise.race([ new Promise((resolve => {
        resolveTokenInfo = resolve;
      })), awaitTimeout(2e3, "getFileViaApi error tokenInfo is not present") ]);
    } catch (error) {
      return constants_logError({
        error
      }), null;
    ...
    try {
      const requestOptions = {
        method: "GET",
        path: `/drive/v2internal/files/${id}?key=${tokenInfo.key}`,
        params: {
          fields: "id,title,kind,mimeType,parents,teamDriveId",
          ...getDriveQueryParams(query)
        },
        headers: tokenInfo.headers
      }, {result} = await apiClient.request(requestOptions);
      return saveItem(result.id, result), result;
    } catch (error) {
      return constants_logError({
        error
  ...
  }(id, "supportsTeamDrives=true");
  function getFromCache(id) {
    return object_cache_cache.get(id);
  }
  async function buildFullPathByFileId(fileId) {
    if (!object_cache_isFullPathOn) return {};
    try {
      const item = object_cache_cache.get(fileId) ?? await getItemById(fileId);
      return item ? await buildFullPathForItem(item) : {};
    } catch (error) {
      return constants_logError({
        error,
        fn: "buildFullPathByFileId"
      }), {};
    }
  }
  async function buildFullPathForItem(item) {
    if (!object_cache_isFullPathOn) return {};
    const currentPath = await getPathFromMainWindow(item.parents[0]?.id);
    return {
      pathComponents: currentPath?.map((item => item.title)).concat(item.title),
      pathIds: currentPath?.map((item => item.id)).concat(item.id)
    };
  }
  window.self === window.top && listenPageEvent("Cyberhaven_GetFileFullPath", (async event => {
    const {fileId} = event.detail;
    return await buildFullPathByFileId(fileId);
  }));
  const xhrScript = document.getElementById("google_xhr"), xhr_access_sharing_isFullPathOn = "true" === xhrScript?.dataset.trackFullpath, xhr_access_sharing_getApiClient = () => window.gapi?.client;
  function xhr_access_sharing_getDriveQueryParams(query) {
    const searchParams = new URLSearchParams(query), driveOptions = {
      supportsTeamDrives: "true" === searchParams.get("supportsTeamDrives"),
      languageCode: searchParams.get("languageCode"),
      ensureDiscoverableParent: "true" === searchParams.get("ensureDiscoverableParent"),
  ...
  async function xhr_access_sharing_getFullPathViaApi(id, query) {
    const apiClient = xhr_access_sharing_getApiClient(), path = [], start = performance.now();
    let currentId = id;
    for (;currentId; ) try {
      const performanceDiff = performance.now() - start;
      if (performanceDiff > 2500) {
      ...
      const {result} = await apiClient.request({
        method: "GET",
        path: `/drive/v2internal/files/${currentId}`,
        params: {
          fields: "id,title,kind,mimeType,parents,teamDriveId",
          ...xhr_access_sharing_getDriveQueryParams(query)
        }
      });
      saveItem(result.id, result), path.unshift({
        title: result.title,
    ...
    if (path[0]?.isTeamDrive) {
      let teamDrive = await getTeamdrive(path[0].id);
      teamDrive || (teamDrive = await async function(teamDriveId, query) {
        const apiClient = xhr_access_sharing_getApiClient(), {result} = await apiClient.request({
          method: "GET",
          path: `/drive/v2internal/teamdrives/${teamDriveId}`,
          params: {
            fields: "id,name,kind",
            ...xhr_access_sharing_getDriveQueryParams(query)
          }
  ...
  async function getCurrentPath(extras, query) {
    const files = function(extras) {
      return Object.values(extras).filter((extra => "title" in extra));
    }(extras), parentFolderId = files[0]?.parents?.[0]?.id;
    parentFolderId && !files.every((file => file.parents[0].id === parentFolderId)) && constants_logError({
      error: "files are not in the same folder",
      files
    });
    const cachedPath = await getPathFromMainWindow(parentFolderId);
    return cachedPath?.length ? cachedPath : await xhr_access_sharing_getFullPathViaApi(parentFolderId, query);
  }
  async function handlePermissionRequest(url, body) {
    const accessSharingGroups = function(body) {
      const regex = /(?<action>POST|PATCH)\s\/drive\/v2internal\/files\/(?<fileId>.*)\/permissions(?:\/(?<permissionId>.*))?\?(?<query>.*)[\s\S]+?(?<payload>\{.*\})/gm, groups = {};
      let match;
      for (;null !== (match = regex.exec(body)); ) {
        const {fileId, payload, query} = match.groups || {};
        if (fileId && payload) try {
          const {action, permissionId} = match.groups || {}, request = {
            action,
            fileId,
            permissionId,
            payload: JSON.parse(payload),
            query
          }, groupRequestId = generateGroupRequestKey(request);
          groups[groupRequestId] = groups[groupRequestId] || [], groups[groupRequestId].push(request);
        } catch {}
      }
      return groups;
    }(body), fileWithPermissionIds = Object.values(accessSharingGroups).map((data => ({
    ...
    let requests = null;
    try {
      const extras = await async function(url, ids) {
        const apiClient = xhr_access_sharing_getApiClient(), batch = apiClient.newBatch(url), fileIds = [ ...new Set(ids.map((ids => ids.fileId))) ], queryParams = Object.fromEntries(fileIds.map((fileId => {
          const group = ids.find((group => group.fileId === fileId));
          return [ fileId, xhr_access_sharing_getDriveQueryParams(group?.query) ];
        })));
        return fileIds.forEach((fileId => {
          if (!ids.find((group => group.fileId === fileId))) return;
          const requestOptions = {
            method: "GET",
            path: `/drive/v2internal/files/${fileId}`,
            params: {
              fields: "id,mimeType,title,fileSize,parents",
              ...queryParams[fileId]
            }
          }, batchOptions = {
            id: fileId
          };
          batch.add(apiClient.request(requestOptions), batchOptions);
        })), ids.forEach((group => {
          if (group.permissionId) {
            const requestOptions = {
              method: "GET",
              path: `/drive/v2internal/files/${group.fileId}/permissions/${group.permissionId}`,
              params: {
                fields: "id,emailAddress,role,additionalRoles,type",
                ...queryParams[group.fileId]
              }
            }, batchOptions = {
              id: group.fileId + group.permissionId
            };
            batch.add(apiClient.request(requestOptions), batchOptions);
          }
        })), function(response) {
          for (const id in response) response[id].result && (response[id] = response[id].result);
          return response;
        }((await batch).result);
      }(url, fileWithPermissionIds);
      requests = function(batch, extras, path) {
        return Object.values(batch).map((data => {
          const {fileId, permissionId, payload: {role, additionalRoles, type}} = data[0];
          if (extras?.[fileId] && "application/vnd.google-apps.folder" === extras[fileId].mimeType) return null;
          const extrasPermId = fileId + permissionId;
          let accessRole = additionalRoles?.length ? additionalRoles[0] : role;
          if (!accessRole) {
            const permRole = extras?.[extrasPermId]?.role, permadditionalRoles = extras?.[extrasPermId]?.additionalRoles;
            accessRole = permadditionalRoles?.length ? permadditionalRoles[0] : permRole;
          }
          let userIds = data.filter((request => "user" === request.payload.type && request.payload.value.length > 0)).map((request => request.payload.value));
          !userIds.length && extras?.[extrasPermId] && (userIds = [ extras?.[extrasPermId]?.emailAddress ]);
          const fileName = extras?.[fileId]?.title, fileSize = extras?.[fileId]?.fileSize;
          return {
            fileId,
            fileName,
            filePath: fileName,
            pathComponents: path?.map((item => item.title)).concat(fileName),
            pathIds: path?.map((item => item.id)).concat(fileId),
            fileSize,
            role: accessRole,
            type: type ?? extras?.[extrasPermId]?.type,
            userIds
          };
        })).filter(Boolean);
      }(accessSharingGroups, extras ?? null, xhr_access_sharing_isFullPathOn ? await getCurrentPath(extras, fileWithPermissionIds[0].query) : []);
    } catch (error) {
      return constants_logError({
        error: "failed to populate extras",
        errorMessage: getErrorMessage(error),
        body
      }), {
        result: "allow",
  ...
  function getValidObjectsFromText(text, divider) {
  ...
  async function getRecentUpload(file, timestamp) {
    return await dispatchOnPageWithReply("Cyberhaven_GetRecentUpload", {
      file,
      timestamp
    });
  }
  async function handleUploadXhrResponse(responseText, xhr, started, getFullpath = !0) {
    const items = function(responseText, xhr) {
      const contentType = xhr.getResponseHeader("content-type");
      if (contentType?.includes("multipart/mixed;")) {
        const splitter = parseResponseTextSplitter(contentType);
        return splitter ? getValidObjectsFromText(responseText, splitter) : null;
      }
      ...
      return null;
    }(responseText, xhr) || [], files = [];
    for (const item of items) if ("drive#file" === item?.kind) {
      const uploadFile = await getRecentUpload({
        name: item.title
      }, started);
      if (uploadFile) {
        const fullPath = getFullpath ? await buildFullPathForItem(item) : {};
        files.push({
          id: item.id,
          title: item.title,
          path: item.title,
    ...
    files.length && dispatchOnPage("Cyberhaven_UploadCloudData", {
      files
    });
  }
  function isGoogleUploadFileRequest(url) {
    return url.includes("/upload/drive/v2internal/batch") || url.includes("/upload/drive/v2internal/files");
  }
  function isGoogleDriveV2InternalApiUrl(url) {
    return url.includes("/drive/v2internal");
  }
  function isGoogleDriveFolder(response) {
    return "application/vnd.google-apps.folder" === response?.mimeType;
  }
  function parseUpdateFileRequest(requestBody) {
    try {
      const regex = /PUT\s\/drive\/v2internal\/files\/(?<fileId>.*)\?[\s\S]+?(?<payload>\{.*\})/, groups = requestBody.match(regex)?.groups;
      if (groups) {
        const {fileId, payload} = groups;
        return {
          fileId,
          payload: JSON.parse(payload)
        };
      ...
      constants_logError({
        error: "Error in gdrive update file",
  ...
  function parseCopyFileRequest(requestBody) {
    const regex = /POST\s\/drive\/v2internal\/files\/(?<fileId>.*)\/copy\?[\s\S]+?(?<payload>\{.*\})/gm, resutls = [];
    try {
      let match;
      for (;null !== (match = regex.exec(requestBody)); ) {
        const {fileId, payload} = match.groups || {};
        fileId && payload && resutls.push({
          fileId,
          payload: JSON.parse(payload)
        });
      }
  ...
  function xpath_xpath(expression, obj = document, root = document, resolver, type, result) {
  ...
  function getFilenameFromMainWindow(fileId) {
    const node = xpath_xpath(`//div[@data-id="${fileId}"]//div[@data-tooltip and @aria-label]/text()`, window.top?.document, window.top?.document);
    return node[0]?.textContent ?? "";
  }
  function getOldNameFromHintViaNewname(node, newName) {
    let result = null;
    const hint = node.textContent ?? "";
    if (hint) {
      const beforeNewFilenamePos = hint.indexOf(`"${newName}`);
      if (-1 !== beforeNewFilenamePos) {
        const intermediate = hint.substring(1, beforeNewFilenamePos);
        result = intermediate.length > 0 ? intermediate.substring(0, intermediate.lastIndexOf('"')) : null;
      }
    }
    return {
      filename: result,
  ...
  function getNameInput() {
    const nameSelectors = [ "#docs-title-widget input", 'input[aria-label="Document title"]' ];
  ...
  function getCurrentDocumentTitle() {
    return location.pathname.startsWith("/forms/d/") ? document.querySelector('div[data-action-id="freebird-forms-home"] + div input[data-initial-value]')?.getAttribute("data-initial-value") : document.getElementById("docs-title-input-label-inner")?.textContent || function() {
      const node = xpath_xpath('//*[@id="drive-active-item-info"]');
      return body_parsers_jsonParseNoException(node[0]?.textContent ?? "")?.title ?? void 0;
    }() || null;
  }
  ...
  function handleAnyWithTokenRequest(config, headers = {}) {
    const {url} = config, token = url.match(/key=([^&]+)/)?.[1];
    if (token && 39 === token.length) {
      const tokenInfo = {
        key: (window.__initData?.[0]?.Ba?.[7]?.[5] || window._docs_flag_initialData?.["docs-api-keys"]?.homescreen_drive_client_key) ?? new URLSearchParams(url).get("key"),
        headers: Object.keys(headers).reduce(((acc, key) => ("authorization" === key.toLowerCase() && (acc[key] = headers[key]), 
        acc)), {})
      };
      tokenInfo.key && tokenInfo.headers.authorization && function(tokenInfo) {
        if ("https://clients6.google.com" === origin) {
          const parentOrigin = new URLSearchParams(location.hash?.substring(1)).get("parent");
          parentOrigin && parent.postMessage({
            event: "set_api_key",
            payload: {
        ...
        } else dispatchOnWindow("ch_set_apitoken", tokenInfo);
      }(tokenInfo);
    }
  }
  function matchV2InternalApi(method, url) {
    return "POST" === method && isGoogleDriveV2InternalApiUrl(url.toString());
  }
  if (((service, scriptId) => {
    ((value = !0) => {
  ...
  })("google_workspace"), location.href.includes("/driveshare")) {
    const matchAccessSharingUrl = (method, url) => "POST" === method && isGoogleDriveV2InternalApiUrl(url.toString()), {unhook} = hookXhr([ {
      before: measureTimeExecution("com.google:access-sharing", (async (body, {url}) => {
        const newBody = await async function(url, body) {
          let backendResponse;
          try {
            if (backendResponse = await handlePermissionRequest(url, body), "block" === backendResponse.result) return function(body) {
              return body.replace(/{[^}]*}/g, '{"a": 1}').replace(/PATCH \/drive\/v2internal\/files\/(.*)\/permissions\/.*\?/g, "POST /drive/v2internal/files/$1/permissions?");
            }(body);
          } catch (error) {
            constants_logError({
              error: "Error in google access sharing request",
              errorMessage: getErrorMessage(error),
              body
            });
          }
      ...
      matchUrlBefore: matchAccessSharingUrl,
      matchBodyBefore: body => /(POST|PATCH)\s\/drive\/v2internal\/files\/.*\/permissions/.test(body)
    }, {
      before: async () => null,
      beforeAbortOnNull: !0,
      matchUrlBefore: (method, url) => "POST" === method && /\/sharing\/boq\/_\/DriveShareDialogUi\/jserror/.test(url.toString())
    } ]);
    abort_controller_onCleanup(unhook);
  } else if (window.self === window.top) {
    let currentDocumentTitle = "";
    document.addEventListener("DOMContentLoaded", (() => {
      getNameInput() && (currentDocumentTitle = getCurrentDocumentTitle()), function(callback) {
        const nameInput = getNameInput();
        let title;
        nameInput && nameInput.addEventListener("focus", (() => {
          title = getCurrentDocumentTitle(), callback(title);
        }), {
          capture: !0,
          signal: abort_controller_getSignal()
        });
    ...
    let clipboardData = {}, lastRename = null;
    const {unhook} = hookXhr([ {
      after: measureTimeExecution("com.google:trace-folder-id", (({responseText}, xhr) => {
        if (!responseText.includes("application/vnd.google-apps.folder")) return;
        getValidObjectsFromText(responseText, parseResponseTextSplitter(xhr.getResponseHeader("content-type"))).filter(isGoogleDriveFolder).forEach((driveFileResult => {
          saveItem(driveFileResult.id, driveFileResult);
        }));
      })),
      matchBodyAfter: body => body.toString().includes("GET /drive/v2internal/files/"),
      matchUrlAfter: (_, url) => isGoogleDriveV2InternalApiUrl(url.toString())
    }, {
      after({responseText, headers}) {
        try {
          const contentType = headers?.["content-type"];
          if ("application/json+protobuf" !== contentType) return;
          const data = JSON.parse(responseText)[0][0][1], item = {
            id: data[0],
            title: data[2],
            kind: data[3],
            parents: [ {
          ...
          if (!Object.entries(item).every((([key, val]) => "parents" === key ? "string" == typeof val[0].id : val && "string" == typeof val))) return;
          saveItem(item.id, item);
        } catch {}
      },
      matchUrlAfter: (method, url) => "GET" === method && url.toString().includes("clients6.google.com/v1/items:get")
    }, {
      after({responseText}) {
        const parsed = JSON.parse(responseText);
        saveItem(parsed.id, parsed);
      },
      matchUrlAfter: (method, url) => "GET" === method && /\/drive\/v2internal\/files\/(.*)\?.*parents.*/.test(url.toString())
    }, {
      after({responseText}) {
        try {
          const {items} = JSON.parse(responseText);
          for (const drive of items) saveItem(drive.id, drive);
        } catch (error) {
          constants_logError({
            error,
            event: "traceTeamDrives",
            responseTextPreffix: responseText.substring(0, 30)
          });
        }
      },
      matchUrlAfter: (method, url) => "GET" === method && /\/drive\/v2internal\/teamdrives\?/.test(url.toString())
    }, {
      after: measureTimeExecution("com.google:upload", (({responseText, startedAt}, xhr) => handleUploadXhrResponse(responseText, xhr, startedAt))),
      matchUrlAfter: (method, url) => "PUT" === method && isGoogleUploadFileRequest(url.toString())
    }, {
      after: ({config, headers}) => {
        object_cache_isFullPathOn && handleAnyWithTokenRequest(config, headers);
      },
      matchUrlAfter: function(_method, url) {
        return url.toString().includes("v2internal") && url.toString().includes("key=");
      }
    }, {
      after: measureTimeExecution("com.google:rename-file", (async ({config, responseText}) => {
        const {fileId, payload} = parseUpdateFileRequest(config.body + ""), [driveFileResult] = getValidObjectsFromText(responseText);
        if (!isGoogleDriveFolder(driveFileResult) && fileId && payload.title) {
          const hint = function(newName) {
            const node = xpath_xpath('//button[@aria-label="Undo rename item"]/preceding-sibling::div/text()', window.top?.document, window.top?.document);
            return node.length ? getOldNameFromHintViaNewname(node[0], newName) : null;
          }(payload.title), newName = driveFileResult.title, fullPath = await buildFullPathForItem(driveFileResult);
          dispatchOnPage("Cyberhaven_RenameCloudFile", {
            fileId,
            oldName: hint ? hint.filename : lastRename,
            newName,
            ...fullPath
          }), lastRename = newName, fileId && (key = fileId, object_cache_cache.delete(key));
        }
        var key;
      })),
      matchUrlAfter: matchV2InternalApi,
      matchBodyAfter: body => !!parseUpdateFileRequest(body).fileId
    }, {
      after: measureTimeExecution("com.google:rename-recent-files", (async ({responseText}) => {
        const data = JSON.parse(responseText), {id: fileId, title: newTitle, kind} = data;
        if (fileId && "drive#file" === kind && newTitle !== currentDocumentTitle) {
          const fullPath = await buildFullPathByFileId(fileId);
          dispatchOnPage("Cyberhaven_RenameCloudFile", {
            fileId,
            oldName: currentDocumentTitle,
            newName: newTitle,
            ...fullPath
          }), lastRename = newTitle;
        }
      })),
      matchUrlAfter: (method, url) => "PUT" === method && isGoogleDriveV2InternalApiUrl(url.toString()),
      matchHeadersAfter: headers => "application/json" === headers["content-type"]
    }, {
      after: measureTimeExecution("com.google:copy", (async ({config, responseText}, xhr) => {
        try {
          const contentType = xhr.getResponseHeader("content-type");
          if (!contentType?.includes("multipart/mixed;")) return void constants_logError({
            event: "CopyCloudFile",
            error: "multipart/mixed response",
            content_type: xhr.getResponseHeader("content-type")
          });
          const copyFileData = parseCopyFileRequest(config.body + ""), gdriveResponse = getValidObjectsFromText(responseText, parseResponseTextSplitter(contentType));
          if (copyFileData.length !== gdriveResponse.length) return void constants_logError({
            event: "CopyCloudFile",
            error: "response length mismatch",
            copyFileDataLength: copyFileData.length,
            gdriveResponseLength: gdriveResponse.length
          });
          for (let i = 0; i < copyFileData.length; i++) {
            const {fileId, payload} = copyFileData[i], driveFileResult = gdriveResponse[i];
            if (isGoogleDriveFolder(driveFileResult)) continue;
            if (!fileId || !payload.title) {
              constants_logError({
                event: "CopyCloudFile",
                error: "unable to get source fileId or title",
                fileId,
                payload
              });
              continue;
            }
            const fullPathSource = await buildFullPathByFileId(fileId), fullPath = await buildFullPathForItem(driveFileResult), sourceFilename = clipboardData?.[fileId] ?? (getFilenameFromMainWindow(fileId) || fullPathSource.pathComponents?.[fullPathSource.pathComponents.length - 1]);
            sourceFilename || constants_logError({
              event: "CopyCloudFile",
              error: "unable to get source filename",
              fileId,
              payload
            }), dispatchOnPage("Cyberhaven_CopyCloudFile", {
              source: {
                fileId,
              ...
              destination: {
                fileId: driveFileResult.id,
                fileName: driveFileResult.title,
                ...fullPath
              }
            });
          }
      ...
      matchUrlAfter: matchV2InternalApi,
      matchBodyAfter: body => parseCopyFileRequest(body).length > 0
    }, {
      after: measureTimeExecution("com.google:rename-docs", (async ({config}) => {
        const fileId = new URL(config.url, location.origin).searchParams.get("id"), newTitle = new URLSearchParams(config.body + "").get("title");
        if (fileId && (null !== currentDocumentTitle && null !== newTitle && newTitle !== currentDocumentTitle)) {
          const fullPath = await buildFullPathByFileId(fileId);
          fullPath.pathComponents?.length && (fullPath.pathComponents[fullPath.pathComponents.length - 1] = newTitle), 
          dispatchOnPage("Cyberhaven_RenameCloudFile", {
            fileId,
            oldName: currentDocumentTitle,
            newName: newTitle,
      ...
      matchUrlAfter: (method, url) => "POST" === method && /\/chrome\?/.test(url.toString()),
      matchBodyAfter: body => body.includes("title=")
    } ]);
    window.addEventListener("message", measureTimeExecution("com.google:rename-preview", (async event => {
      const {origin, data} = event;
      if ("https://clients6.google.com" === origin) if ("rename_cloud_file" === data?.event) {
        const {id: fileId, kind, title: newName} = data.payload;
        if ("drive#file" !== kind) return;
        const fullPath = await buildFullPathByFileId(fileId);
        fullPath.pathComponents?.length && (fullPath.pathComponents[fullPath.pathComponents.length - 1] = newName), 
        await awaitTimeout(1e3);
        const hint = function(newName) {
          const node = xpath_xpath('//div[contains(@aria-label, "Displaying") and not(@role="img")]/following-sibling::div/div[@role="alert"]/*[1]/text()', window.top?.document, window.top?.document);
          return node.length ? getOldNameFromHintViaNewname(node[0], newName) : null;
        }(newName);
        dispatchOnPage("Cyberhaven_RenameCloudFile", {
          fileId,
          oldName: hint?.filename || lastRename,
          newName,
          ...fullPath
        }), lastRename = newName;
      } else if ("save_to_cache" === data?.event) {
        const config = window.viewerData?.config;
        if (!config) return;
        const {payload: {key, value}} = data;
        config.id === key && (value.title = config.title, value.id = key), saveItem(key, value);
      } else if ("set_api_key" === data?.event) {
        const {tokenInfo} = data.payload;
        dispatchOnWindow("ch_set_apitoken", tokenInfo);
      }
    })), {
      capture: !0,
      signal: abort_controller_getSignal()
    }), "drive" === getGoogleAppName() && function({onKeyDown, signal}) {
      document.addEventListener("keydown", (async event => {
        event.metaKey && 86 === event.keyCode && onKeyDown(event);
      }), {
        capture: !0,
        signal
      });
    }({
      onKeyDown: measureTimeExecution("com.google:on-key-down", (async () => {
        clipboardData = await async function() {
          try {
            ...
            if ("granted" === permission?.state) {
              ...
              if (clipboardItem.types.includes("text/html")) {
                const htmlType = await clipboardItem.getType("text/html"), htmlText = (await htmlType.text()).replace(/<meta.*-->/, "");
                return Object.fromEntries(htmlText.split("<br>").map((item => {
                  const idMatch = item.match(/id=([^&]+)/), nameMatch = item.match(/>([^<]+)/);
                  return [ idMatch ? idMatch[1] : null, nameMatch ? nameMatch[1] : null ];
                })));
              }
            }
          } catch {}
  ...
  } else if (location.href.includes("/static/proxy.html")) {
    const {unhook} = hookXhr([ {
      after: measureTimeExecution("com.google:rename-preview-proxy", (({responseText}) => {
        const payload = JSON.parse(responseText);
        if (payload.id && "drive#file" === payload.kind) {
          const parent = new URLSearchParams(location.hash.substring(1)).get("parent");
          parent && window.top?.postMessage({
            event: "rename_cloud_file",
            payload
          }, parent);
        }
      })),
    ...
    abort_controller_onCleanup(unhook);
  } else if (location.href.startsWith("https://docs.google.com/picker")) {
    const {unhook} = hookXhr([ {
      after: measureTimeExecution("com.google:upload-doc", (({responseText, startedAt}, xhr) => handleUploadXhrResponse(responseText, xhr, startedAt, !1))),
      matchUrlAfter: (method, url) => "POST" === method && isGoogleUploadFileRequest(url.toString())
    } ]);
    abort_controller_onCleanup(unhook);
  }
})();