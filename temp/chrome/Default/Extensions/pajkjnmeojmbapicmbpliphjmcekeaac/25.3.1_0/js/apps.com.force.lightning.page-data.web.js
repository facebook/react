(() => {
  ...
  z.object({
    initConfig: z.object({
      context: z.object({
        globalValueProviders: z.array(z.union([ z.object({
          type: z.literal("$SObjectType"),
          values: z.object({
  ...
  let userEmail = null;
  function getUserEmailFromInlineScript() {
    if (userEmail) return userEmail;
    const initScript = function(expression, obj = document, root = document, resolver, type, result) {
      ...
      var xpr;
    }('//script[contains(., "CurrentUser")]')[0];
    return userEmail = initScript.textContent?.match(/CurrentUser.*Email":"([^"]+)"/)?.[1] || "", 
    userEmail;
  }
  var fn;
  fn = async function() {
    let email = "undefined" != typeof Aura && Aura.initConfig?.context?.globalValueProviders?.find((provider => "$SObjectType" === provider.type))?.values.CurrentUser?.Email;
    return email || (email = getUserEmailFromInlineScript()), {
      ...await function(eventName, data, {timeout = 1e3, wrapVersion = addVersionToEventNameByDefault} = {}) {
