...
If you like using JSX, there is an version of Babel that can be used in the browser. Babel uses this in their [online repl](https://babeljs.io/repl), and we use it for the tutorial and starter kit. `babel-standalone` can be used to directly transform & execute `<script type="text/babel">` content, supporting ES2015 and React features. It can also be used to transform more code via the API it exposes, along with access to all of the default Babel plugins. [Read more](https://github.com/Daniel15/babel-standalone#usage) about how to use this in your own projects.
