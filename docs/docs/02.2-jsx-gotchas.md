...
Don't forget the brackets! If your comment is within a <tag> you must include curly brackets around the comment: 

````javascript
   render: function() {
      return <div>{/* this is a comment within a <div> tag */} Hello {this.props.name}</div>;
   }
````    

If your comment is not within a <tag>, the brackets are optional:

````javascript
   render: function() {
      return /* this is a comment not in any tags*/<div>Hello {this.props.name}</div>;
   }
