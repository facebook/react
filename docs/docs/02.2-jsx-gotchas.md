...
JSX requires the style element to be a JS object, not a string like HTML. If you want to have inline css, simply add `{{' '}}`.

```javascript
<div style={{display: hidden}}>Hello World</div>
```
