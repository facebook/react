...
Don't forget the brackets! A comment in jsx opens with {/* and closes with */}, not just /* and */, like this:

````javascript
   render: function() {
      return <div>{/* this is a comment with crazy chars in it <>!@#$%^&**(()_+* */} Hello {this.props.name}</div>;
   }
````    

This is especially important for comments within html elements (as is the comment above within a <div>)
