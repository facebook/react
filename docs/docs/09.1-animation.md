...
A staggering animation is a set of animations that have a slight delay between one another, resulting in a curtain-like effect. If you would like this animation on your `ReactTransitionGroup`, you can add the `transitionStaggering={true}` prop to enable the effect. In addition, you must specify a `stagger` class which will have the `animation-delay` or `transition-delay` property.

As an example, to augment the example animation above, you would add a new class:

```css
.example-enter.example-enter-stagger {
  transition-delay: 0.5s;
}
```

Now there will be a staggered animation whenever children are added to the group, based upon that delay interval of 0.5 seconds.

### Directional Staggering

If you would like the animation to move in the direction of the added or removed children, add the property `transitionStaggeringDirectional={true}`. This will make the stagger move outward from the already existing children when adding components, and inwards when removing components. The effect works best when you only add or remove components from one end of the existing children.
