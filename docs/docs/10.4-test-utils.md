...
```javascript
ReactElement shallowRenderer.render(
  ReactElement element
  [object context]
)
```

Similar to `ReactDOM.render`. Returns shallowly rendered output. You can then begin to assert facts about the output. For example, if your component's render method returns:
...
var result = renderer.render(<YourComponent />);
...
`shallowRenderer.render` works only with custom components, not DOM components.

```javascript
ReactElement shallowRenderer.getRenderOutput()
```

After `render` has been called, you can get shallowly rendered output until unmount.

```javascript
ReactComponent shallowRenderer.getMountedInstance()
```

Returns rendered React component instance.
