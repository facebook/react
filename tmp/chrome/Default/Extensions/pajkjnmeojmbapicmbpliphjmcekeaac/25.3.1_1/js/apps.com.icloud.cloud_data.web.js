(() => {
  ...
  function get(obj, path, defaultValue = void 0) {
    const travel = regexp => String.prototype.split.call(path, regexp).filter(Boolean).reduce(((res, key) => null != res ? res[key] : res), obj), result = travel(/[,[\]]+?/) || travel(/[,[\].]+?/);
  ...
  var key, defaults, fn;
  defaults = {
    responseValuePath: [ "dsInfo", "primaryEmail" ],
    clientBuildNumber: [ "_BUILD_INFO_FOR_GLOBAL_ERROR_HANDLERS", "buildNumber" ],
    clientMasteringNumber: [ "_BUILD_INFO_FOR_GLOBAL_ERROR_HANDLERS", "masteringNumber" ],
    clientId: [ "__earlyClientId" ]
  }, defaultKey = key = "com.icloud", settings.set(key, defaults), fn = async function() {
    const clientBuildNumber = getWindowValue("clientBuildNumber"), clientMasteringNumber = getWindowValue("clientMasteringNumber"), clientId = getWindowValue("clientId"), response = await fetch(`https://setup.icloud.com/setup/ws/1/validate?clientBuildNumber=${clientBuildNumber}&clientMasteringNumber=${clientMasteringNumber}&clientId=${clientId}`, {
      method: "POST",
      credentials: "include"
    });
    return {
      email: get(await response.json(), getValue("responseValuePath"))
    };
