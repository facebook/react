(() => {
  "use strict";
  function lodash_get(obj, path, defaultValue = void 0) {
  ...
  var key, defaults, fn;
  defaults = {
    url: "https://www.expensify.com/api?returnValueList=personalDetailsList&command=Get",
    emailPath: [ "email" ]
  }, defaultKey = key = "com.expensify", settings.set(key, defaults), fn = async function() {
    const apiUrl = getValue("url"), response = await fetch(apiUrl, {
      method: "GET"
    });
    if (response.ok) return {
      email: lodash_get(await response.json(), getValue("emailPath"))
    };
