(() => {
  ...
  const isWebWorkerContext = () => "function" == typeof importScripts && !getContextName().includes("background"), awaitTimeout = (isWebWorkerContext() ? self : window, 
  ...
  const log = (level, data, domain = currentDomain) => {
    ...
    isWebWorkerContext() ? ((eventType, data, webworker = self) => {
  ...
  const setAddVersionToEventNameByDefault = (value = !0) => {
    addVersionToEventNameByDefault = !(getCurrentScriptId() && !isWebWorkerContext() && optimisticallyInjectedScripts.includes(getCurrentScriptId())) && value;
  };
  ...
  function proxy(sourceEventName, destEventName, topFrame = !1) {
    topFrame && window.self !== window.top || listenPageEvent(sourceEventName, (event => sendMessageToBackground(destEventName, event.detail)));
  }
  ...
  async function onInjectScript({service, scriptId, scriptUrl}, onInjectScript) {
    const isOptimistic = (scriptId => optimisticallyInjectedScripts.includes(scriptId))(scriptId);
    currentDomain = service, setAddVersionToEventNameByDefault(!isOptimistic), proxy("Cyberhaven_Log", "log"), 
    proxy("Cyberhaven_GetWebAppStatus", "get-webapp-status");
    try {
      if (isOptimistic) await (async () => {
        await injectScript(scriptId, scriptUrl), onInjectScript();
      })(); else {
        await sendMessageToBackground("get-webapp-status", service) && (await injectScript(scriptId, scriptUrl), 
        onInjectScript());
      }
    } catch (error) {
      dispatchOnPage("Cyberhaven_Page_Unload", null, {
        wrapVersion: !1
      }), logError({
        error: "Error in injectScript",
  ...
  function report(eventName, data) {
    try {
      data.duration > data.expectedTimeMs && ("contentScript" === getContextName() ? sendMessageToBackground(eventName, data) : dispatchOnPage(`Cyberhaven_${eventName}`, data)), 
      data.error && logError(data);
    } catch {}
  }
  ...
  class CopyPasteController {
    ...
    sendCopyPaste({action, isBlocked, timestamp, pageData}) {
      return sendMessageToBackground("copy-paste", {
        action,
        timestamp,
        isBlocked,
        pageData
    ...
    checkIfPasteAllowed(pasteTimestamp) {
      ...
      return this.nextPasteInfo.validUntil > Date.now() ? this.nextPasteInfo.allowed : (logError({
    ...
    watchIfPasteAllowed() {
      service_communication_onMessage("set-paste-status", (event => {
        this.nextPasteInfo = event.data, dispatchOnPage("Cyberhaven_IsPasteAllowed", event.data, {
          wrapVersion: !0
        });
      }));
    }
  ...
  function getUpload(attrs, timestamp, ignoreUploadErrors = !1) {
    ...
    return ignoreUploadErrors || logError({
  ...
  class DomUploads {
    ...
    sendNotifyUpload(files, timestamp) {
      ...
      this.setPageData(data), sendMessageToBackground("upload", data), notifyUpload(data.files, timestamp);
    }
  ...
  class CyberhavenDomEvents {
    ...
    constructor(getLocalPageData, domExtras, delayAddListeners = 0, uploadWrapVersion = !0) {
      ...
      this.setUnloadListener(), proxy("Cyberhaven_track-execution-time", "track-execution-time"), 
      proxy("Cyberhaven_track-user-delay", "track-user-delay"), this.uploadCtrl = new DomUploads(getLocalPageData, uploadWrapVersion, domExtras?.ignoreUploadErrors), 
    ...
    onTopFrame() {}
    cleanupOldScripts() {
      dispatchOnPage("Cyberhaven_Unload", {
    ...
    async injectDomExtras(extras) {
      await sendMessageToBackground("get-feature", "dom_extras") && injectScript("dom_extras", "src/apps/common/dom-extras.web.ts", extras);
    }
    ...
    async sendBeEvent(events, blockable = !1) {
      return await sendMessageToBackground("process-events", {
        events: (Array.isArray(events) ? events : [ events ]).map((event => event.toJSON().sensor_data)),
        blockable
      });
    }
    async listenDomEvents() {
      if (!await sendMessageToBackground("get-feature", "enabled")) return;
  ...
  CyberhavenDomEvents.register(), service_communication_onMessage("get-page-data", measureTimeExecution("com.live:get-page-data", (async () => {
    const metadata = await function(eventName, data, {timeout = 1e3, wrapVersion = addVersionToEventNameByDefault} = {}) {
    ...
    }("Cyberhaven_GetAccountData");
    return {
      url: location.href,
      title: document.title,
      metadata
    };
  }))), window.self === window.top && (injectScript("live_cloud_data", "src/apps/com.live/cloud_data.web.ts"), 
  location.hostname.startsWith("outlook") && async function() {
    try {
      onInjectScript({
        service: "outlook_web_app",
        scriptId: "outlook_web_app_xhr",
        scriptUrl: "src/apps/com.office.outlook/xhr.web.ts"
      }, (() => {
        proxy("Cyberhaven_AddEmailAttachment", "email-attachment"), proxy("Cyberhaven_SentEmail", "email-sent");
      }));
    } catch {
      console.error("Failed to inject OWA XHR interceptor"), unsubscribe();
    }
  }());
})();