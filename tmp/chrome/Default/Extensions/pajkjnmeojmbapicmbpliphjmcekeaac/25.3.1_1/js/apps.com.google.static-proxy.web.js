(() => {
  ...
  async function getFullPathViaApi(id, query) {
    ...
    if (path[0]?.isTeamDrive) {
      let teamDrive = await async function(teamdriveId) {
  ...
  async function buildFullPathByFileId(fileId) {
    ...
    try {
      ...
      return item ? await async function(item) {
        if (!object_cache_isFullPathOn) return {};
        const currentPath = await async function(parentId) {
  ...
  const {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
    ...
    after({responseText, config}) {
      if (config.xhr?._chRequestHeaders) return;
      const key = function(url) {
        try {
          if ("string" != typeof url) throw new Error("Invalid URL format");
          const fileIdPattern = /\/files\/([a-zA-Z0-9_-]+)/, match = url.match(fileIdPattern);
      ...
      if (key) {
        ...
        parsed.id || (parsed.id = key), saveItem(key, parsed);
      }
    },
    matchUrlAfter: (_, url) => /\/drive\/v2internal\/files\/(.*)\?.*parents.*/.test(url.toString())
  }, {
    after: ({config, headers}) => {
      handleAnyWithTokenRequest(config, headers);
    },
