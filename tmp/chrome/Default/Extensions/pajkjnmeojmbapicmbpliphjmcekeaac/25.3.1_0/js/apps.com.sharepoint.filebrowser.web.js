(() => {
  ...
  var __webpack_modules__ = {
    4698: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      __webpack_require__.d(__webpack_exports__, {
        a: () => cloud_data_web_getAccountEmail
      });
    ...
    882: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      ...
      }, getSignal = () => abortController.signal, onCleanup = callback => {
    ...
    2311: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      __webpack_require__.d(__webpack_exports__, {
        un: () => isCurrentScriptOptimisticallyInjected
      });
      ...
      let currentScript = (0, apps_common_globals__WEBPACK_IMPORTED_MODULE_0__.E)() ? null : document.currentScript;
      const getCurrentScriptId = () => currentScript?.getAttribute("scriptId"), isCurrentScriptOptimisticallyInjected = () => !(!getCurrentScriptId() || (0, 
      apps_common_globals__WEBPACK_IMPORTED_MODULE_0__.E)()) && optimisticallyInjectedScripts.includes(getCurrentScriptId());
    },
    3983: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      __webpack_require__.d(__webpack_exports__, {
        FS: () => dispatchOnPage,
        H4: () => dispatchOnPageWithReply,
        V5: () => listenPageEvent
      });
      var apps_common_globals__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(143), services_promise_timeout__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(6426), apps_common_abort_controller__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(882), _page_logging__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(1894);
      __webpack_require__(2311);
      const COMMUNICATION_TIMEOUT = 1e3;
      ...
      function dispatchOnPageWithReply(eventName, data, {timeout = COMMUNICATION_TIMEOUT, wrapVersion = addVersionToEventNameByDefault} = {}) {
        const replyEvent = getReplyEventName(eventName, wrapVersion), promise = new Promise((resolve => {
          document.addEventListener(replyEvent, (function(event) {
            ...
            signal: (0, apps_common_abort_controller__WEBPACK_IMPORTED_MODULE_1__.V1)(),
            once: !0,
            capture: !0
          });
          const finalEventName = wrapVersion ? addVersion(eventName) : eventName;
        ...
        return Promise.race([ promise, (0, services_promise_timeout__WEBPACK_IMPORTED_MODULE_4__.K)(timeout) ]);
      }
    },
    ...
    6426: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
      __webpack_require__.d(__webpack_exports__, {
        K: () => awaitTimeout
      });
  ...
  var abort_controller = __webpack_require__(882), POSITIONALS_EXP = /(%?)(%([sdijo]))/g;
  ...
  var page_communication = __webpack_require__(3983), get_error_message = __webpack_require__(7540), page_logging = __webpack_require__(1894), common_globals = __webpack_require__(143);
  const webWorkerBlob = (0, common_globals.E)() ? null : window.CH_WEBWORKER_SCRIPT_BLOB;
  new Blob([ webWorkerBlob ], {
    type: "application/javascript"
  });
  var optimistically_injected = __webpack_require__(2311);
  const rawOpen = XMLHttpRequest.prototype.open, rawSetRequestHeader = XMLHttpRequest.prototype.setRequestHeader, randomScriptId = Math.floor(1e3 * Math.random());
  let isEnablePreviousHookListenerRegistered = !1, enabled = !1, isXhrHooked = !1;
  new WeakMap;
  const registerEnablePreviousHookListener = () => {
    isEnablePreviousHookListenerRegistered || (0, common_globals.E)() || ((0, page_communication.V5)("Cyberhaven_EnablePreviousHookIfExist", (({detail: {initiatedByScriptId}}) => {
      if (randomScriptId !== initiatedByScriptId) return enabled = !0, {
        doesExist: !0
      };
    }), {
      ignoreSignal: !0
  ...
  const {sendMessage, onMessage} = {
    ...
    onMessage: (eventName, callback) => {
      const handleWindowMessage = message => {
        ...
        const parsed = body_parsers_jsonParseNoException(message.data);
        parsed?.event === "ch_" + String(eventName) && callback(parsed.payload);
      };
      window.addEventListener("message", handleWindowMessage), (0, abort_controller.Ki)((() => {
        window.removeEventListener("message", handleWindowMessage);
      }));
    }
  };
  async function getRecentUpload(file, timestamp) {
    return await (0, page_communication.H4)("Cyberhaven_GetRecentUpload", {
      file,
      timestamp
    });
  }
  var cloud_data_web = __webpack_require__(4698);
  const trimCurlyBrackets = str => {
    const match = str.match(/{(.*?)}/);
    return match ? match[1] : str;
  }, createCopyJobsMap = new WeakMap, copyMoveJobProgressData = new Map, recentListDataStream = {
    listData: [],
    listDataUrl: "",
    listDataBody: "",
    listDataHeaders: {},
  ...
  let completedJobsQTimeout = null;
  function queueCompletedJobs(completedJobs) {
    completedJobsQTimeout && clearTimeout(completedJobsQTimeout);
    for (const job of completedJobs) completedJobsQueue.find((j => j.JobId === job.JobId)) || completedJobsQueue.push(job);
    return new Promise((resolve => {
      completedJobsQTimeout = setTimeout((() => {
        const events = [ ...completedJobsQueue ];
        completedJobsQueue.length = 0, resolve(async function(jobs) {
          const requestToJobIds = new Map;
          for (const job of jobs) {
            const progressData = copyMoveJobProgressData.get(job.JobId);
            if (!progressData) continue;
            invariant(progressData.requestId, "requestId not found");
            const jobIds = requestToJobIds.get(progressData.requestId) || [];
            requestToJobIds.set(progressData.requestId, [ ...jobIds, job.JobId ]);
          }
          for (const requestId of requestToJobIds.keys()) {
            const requestIdJobs = jobs.filter((job => requestToJobIds.get(requestId)?.includes(job.JobId)));
            await procesJobEventsBatch(requestId, requestIdJobs);
          }
        }(events));
      }), 5e3);
    }));
  }
  async function procesJobEventsBatch(requestId, jobs) {
    const destPathes = new Set(jobs.map((job => {
      const progressData = copyMoveJobProgressData.get(job.JobId);
      invariant(progressData, "progress data not found");
      const path = decodeURIComponent(new URL(progressData.createJobRequest.destinationUri).pathname);
      return invariant(path, "destination path parse error"), path;
    })));
    invariant(1 === destPathes.size, "multiple destination paths found");
    const destPath = destPathes.values().next().value;
    invariant(destPath, "destination path not found");
    const destListData = await async function(path) {
      const url = new URL(recentListDataStream.listDataUrl);
      url.searchParams.set("RootFolder", path);
      const r = await fetch(url, {
        method: "POST",
        body: recentListDataStream.listDataBody,
        headers: recentListDataStream.listDataHeaders
      });
      invariant(r.ok, "failed to get list data");
      return (await r.json()).ListData.Row;
    }(destPath), accountEmail = (0, cloud_data_web.a)();
    let events = [], isMoveMode = null;
    for (const job of jobs) {
      const jobId = job.JobId, progressData = copyMoveJobProgressData.get(jobId);
      copyMoveJobProgressData.delete(jobId);
      const uniqueIds = progressData.createJobResponse.SourceListItemUniqueIds.results;
      null === isMoveMode ? isMoveMode = progressData.createJobRequest.options.IsMoveMode : invariant(isMoveMode === progressData.createJobRequest.options.IsMoveMode, "inconsistent move mode"), 
      events = events.concat(uniqueIds.map((uniqueId => {
        const source = progressData.listItems?.find((listItem => trimCurlyBrackets(listItem.UniqueId).toLowerCase() === uniqueId.toLowerCase()));
        if (invariant(source, "source list item not found"), "Folder" === source.ContentType) return null;
        const destination = destListData.filter((listItem => isMoveMode ? listItem.FileLeafRef === source.FileLeafRef : listItem.File_x0020_Size === source.File_x0020_Size && listItem.File_x0020_Type === source.File_x0020_Type)).sort(((a, b) => new Date(b["Modified."]).getTime() - new Date(a["Modified."]).getTime()))[0];
        return invariant(destination, "destination list item not found"), {
          source,
          destination
        };
      })).filter((item => null !== item)));
    }
    return invariant(null !== isMoveMode, "move mode not found"), sendMessage("copy_move", {
      events: events.filter((item => null !== item)),
      isMoveMode,
      accountEmail,
      requestId,
  ...
  const fileBrowserXhrHooks = [ {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlIncludes: "/_api/web/GetListUsingPath(DecodedUrl=@a1)/RenderListDataAsStream"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetListUsingPath(DecodedUrl=@a1)/RenderListDataAsStream"
    }, {
      method: "POST",
      urlIncludes: "/_api/SP.List.GetListDataAsStream"
    } ],
    after: function({config, responseText}) {
      const response = body_parsers_jsonParseNoException(responseText);
      response?.ListData?.Row && (recentListDataStream.listData = response.ListData.Row, 
      recentListDataStream.listDataUrl = config.url, recentListDataStream.listDataBody = String(config.body), 
      recentListDataStream.listDataHeaders = config.xhr?._chRequestHeaders || {});
    }
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlIncludes: "/_api/site/CreateCopyJobs"
    } ],
    matchUrlBeforeConfig: [ {
      method: "POST",
      urlRegex: "/_api/site/CreateCopyJobs"
    } ],
    before: async function(body, config) {
      if ("string" != typeof body) return body;
      const request = body_parsers_jsonParseNoException(body);
      invariant(request, "parse error on CreateCopyJobs request");
      const {destinationUri, exportObjectUris} = request, moveToPersonalDrive = new URL(destinationUri).pathname.startsWith("/personal"), moveAllFromPersonalDrive = exportObjectUris.some((uri => new URL(uri).pathname.startsWith("/personal")));
      if (!moveToPersonalDrive || !moveAllFromPersonalDrive) return body;
      const {listData, listDataUrl} = recentListDataStream, listItems = exportObjectUris.map((uri => {
        const listItem = listData.find((item => item.FileRef === decodeURIComponent(new URL(uri).pathname)));
        return invariant(listItem, "list item not found"), listItem;
      }));
      return createCopyJobsMap.set(config.xhr, {
        request,
        listItems,
        listDataUrl
      }), body;
    },
    after: async function({config, responseText}) {
      if (200 !== config.xhr?.status) return;
      if ("string" != typeof config.body) return;
      const request = body_parsers_jsonParseNoException(config.body);
      invariant(request, "parse error on CreateCopyJobs request");
      const createCopyJobsResults = body_parsers_jsonParseNoException(responseText).d.CreateCopyJobs.results, copyJob = createCopyJobsMap.get(config.xhr);
      if (!copyJob) return;
      const requestId = config.xhr.getResponseHeader("sprequestguid"), {listItems, listDataUrl} = copyJob;
      for (const job of createCopyJobsResults) copyMoveJobProgressData.set(job.JobId, {
        listItems,
        listDataUrl,
    ...
    matchUrlAfterConfig: [ {
      method: "POST",
      urlRegex: "/_api/site/GetCopyJobProgress"
    } ],
    after: async function({responseText}) {
      const results = body_parsers_jsonParseNoException(responseText).d.GetCopyJobProgress.Logs.results;
      invariant(Array.isArray(results), "no results in copy job progress");
      const events = results.map((result => body_parsers_jsonParseNoException(result))), completedJobs = events.filter((event => "JobEnd" === event.Event || "JobFinishedObjectInfo" === event.Event));
      if (!completedJobs) return;
      const jobError = events.find((event => "JobError" === event.Event));
      jobError ? copyMoveJobProgressData.delete(jobError.JobId) : await queueCompletedJobs(completedJobs);
    }
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddUsingPath(DecodedUrl=@a2,AutoCheckoutOnInvalidData=@a3,XorHash=@a4)?"
    }, {
      method: "POST",
      urlIncludes: "_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddUsingPath(DecodedUrl=@a2,overwrite=@a3,AutoCheckoutOnInvalidData=@a4,XorHash=@a5)?"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddUsingPath(DecodedUrl=@a2,EnsureUniqueFileName=@a3,AutoCheckoutOnInvalidData=@a4,XorHash=@a5)?"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddStubUsingPath(DecodedUrl=@a2)/Upload(uploadId=@a3)?"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddStubUsingPath(DecodedUrl=@a2,EnsureUniqueFileName=@a3)/Upload(uploadId=@a4)?"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/GetByPathOrAddStub(DecodedUrl=@a2)/Upload(uploadId=@a3)?"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddStubUsingPath(DecodedUrl=@a2)/StartUploadFile(uploadId=@a3)"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/GetByPathOrAddStub(DecodedUrl=@a2)/StartUploadFile(uploadId=@a3)"
    }, {
      method: "POST",
      urlIncludes: "_api/web/GetFolderByServerRelativePath(DecodedUrl=@a1)/Files/AddStubUsingPath(DecodedUrl=@a2,EnsureUniqueFileName=@a3)/StartUploadFile(uploadId=@a4)"
    } ],
    after: async function({responseText, startedAt, config}) {
      if (200 !== config.xhr?.status) return;
      const response = body_parsers_jsonParseNoException(responseText).d;
      let uploadEvent;
      invariant(response, "parse error on upload create");
      try {
        if (uploadEvent = await getRecentUpload({
          name: response.Name
        }, startedAt), !uploadEvent) throw new Error("failed to get recent upload");
      } catch {
        const baseFilename = response.Name.match(/^(.+\(\d+\))\s+\(\d+\)(?:\.[^.]+)?$/);
        invariant(baseFilename?.[1], "failed to get base filename: %s", response.Name), 
        uploadEvent = await getRecentUpload({
          startsWith: baseFilename[1]
        }, startedAt);
      }
      const requestId = config.xhr.getResponseHeader("request-id");
      sendMessage("upload_create", {
        uploadCreateResponse: response,
        uploadEvent,
        requestId,
        accountEmail: (0, cloud_data_web.a)(),
        webAbsoluteUrl: window._spPageContextInfo?.webAbsoluteUrl || null
      });
    }
  }, {
    matchUrlAfterConfig: [ {
      method: "POST",
      urlIncludes: "/_api/web/GetListUsingPath(DecodedUrl=@a1)/items(@a2)"
    }, {
      method: "POST",
      urlIncludes: "/_api/web/GetListUsingPath(DecodedUrl=@a1)/Items(@a2)"
    } ],
    after: async function({config}) {
      invariant("string" == typeof config.body, "body is not a string");
      const request = body_parsers_jsonParseNoException(config.body), listItemId = new URLSearchParams(config.url).get("@a2")?.slice(1, -1), listItem = recentListDataStream.listData.find((item => item.ID === listItemId));
      if (invariant(listItem, "recent list item not found"), "Folder" === listItem.ContentType) return;
      const newName = request.FileLeafRef;
      invariant(newName, "new file name not found");
      const requestId = config.xhr?.getResponseHeader("sprequestguid");
      sendMessage("rename", {
  ...
  const {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
        ...
        if (enabled) {
          ...
          try {
            ...
            (0, page_logging.vV)({
              ...
              errorMessage: (0, get_error_message.u)(error),
              url: url.toString(),
              method
            });
          }
    ...
    return isXhrHooked = !0, enabled = !(0, optimistically_injected.un)(), Object.defineProperty(XMLHttpRequest.prototype, "open", {
      get() {
        return this._open;
      },
      set(value) {
    ...
    }), XMLHttpRequest.prototype.open = handleXhrOpen, XMLHttpRequest.prototype.setRequestHeader = handleSetRequestHeader, 
    {
      enableHook: (toEnable = !0) => {
        enabled = toEnable, toEnable && enablePreviousScriptOnReload && (0, page_communication.H4)("Cyberhaven_EnablePreviousHookIfExist", {
          initiatedByScriptId: randomScriptId
        }).then((({doesExist} = {
          doesExist: !1
        }) => {
          doesExist && (enabled = !1);
