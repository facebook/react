(() => {
  ...
  }, logFeatureError = (eventType, data) => {
    const {error, ...rest} = data;
    ((data, domain) => {
      log("error", data, domain);
    })({
  ...
  detail);
  !function(eventName, callback, {ignoreSignal = !1, wrapVersion = addVersionToEventNameByDefault} = {}) {
    ...
    try {
      email = await async function() {
        let email = null;
        for (const key in localStorage) if (key.startsWith("@@auth0spajs@@")) try {
          const item = JSON.parse(localStorage.getItem(key));
          if (item && item.body && item.body.decodedToken && item.body.decodedToken.claims && item.body.decodedToken.claims.email) {
            email = item.body.decodedToken.claims.email;
            break;
          }
        } catch (e) {}
        return email;
