(() => {
  ...
  CyberhavenDomEvents.register(), defaults = {
    windowEmailPath: [ "Box", "prefetchedData", "/app-api/enduserapp/current-user", "email" ]
  }, defaultKey = key = "com.box", settings.set(key, defaults), function(config, extras, options = {}) {
    ...
    email: {
      window: {
        path: () => getValue("windowEmailPath")
      }
    }
  }, {
    pageData: {
      cloud_provider: "box",
      default_object_type: "cloud_file_box"
    },
    metadata: {
      account_type: "box"
    }
  }), window.self === window.top && async function({service, scriptId, scriptUrl}, onInjectScript) {
    ...
    service: "box",
    scriptId: "box_xhr",
    scriptUrl: "src/apps/com.box/xhr.web.ts"
  }, (() => {
    proxy("Cyberhaven_ShareData", "share-data"), proxy("Cyberhaven_UploadCloudData", "upload-cloud-data"), 
    proxy("Cyberhaven_CopyCloudFile", "copy-cloud-file"), proxy("Cyberhaven_RenameCloudFile", "rename-cloud-file");
  }));
})();