(() => {
  ...
  }, logRenameCloudFileError = details => {
    page_logging_logFeatureError("site-events/rename", details);
  ...
  function testUrl(url, pattern) {
    url instanceof URL && (url = url.href);
    return new RegExp(pattern).test(url);
  }
  function testIsSharedLink(method, url) {
    return "POST" === method && url.toString().endsWith("/shared-link");
  }
  const filesMap = new Map, polishFileId = fileId => fileId.replace("f_", "").replace("d_", ""), BoxRoles = {
    Editor: "writer",
    Viewer: "reader"
  }, BoxAccessBy_peopleWithTheLink = "anyone", BoxAccessBy_peopleInThisItem = "user";
  ...
  const {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
    ...
    matchUrlAfter: (method, url) => function(method, url) {
      if ("POST" === method) {
        const isOverrideUploadUrl = () => /\/api\/2\.0\/files\/\d{13,20}\/content/.test(url.toString()), isSessionUploadUrl = () => /\/api\/2.0\/files\/upload_sessions\/.*\/commit$/.test(url.toString());
        return url.toString().includes("/api/2.0/files/content") || isOverrideUploadUrl() || isSessionUploadUrl();
      }
      return !1;
    }(method, url),
    after: measureTimeExecution("com.box:upload", (({config, startedAt}) => {
      !async function(config, startedAt) {
        const data = JSON.parse(config.response);
        1 !== data?.total_count && logError({
          event: "UploadCreate",
          error: "total_count !== 1",
          data
        });
        ...
        for (const entry of data.entries) {
          const uploadFile = await getRecentUpload({
            name: entry.name,
            size: entry.size,
            lastModified: new Date(entry.content_modified_at).getTime()
          }, startedAt);
    ...
    matchUrlAfter: (method, url) => function(method, url) {
      return "GET" === method && testUrl(url, "/items");
    }(method, url) || function(method, url) {
      return "GET" === method && url.toString().includes("/app-api/enduserapp/folder?format=");
    }(method, url) || function(method, url) {
      return "GET" === method && testUrl(url, "/item");
    }(method, url),
    after: measureTimeExecution("com.box:list", (({config}) => {
      (config => {
        const response = JSON.parse(config.response);
    ...
    matchUrlBefore: (method, url) => function(method, url) {
      return "POST" === method && testUrl(url, "/invite-collaborators");
    }(method, url) || testIsSharedLink(method, url),
    before: measureTimeExecution("com.box:access-sharing", (async (_body, config) => {
      await async function(config) {
        let requests = [];
        try {
          const body = JSON.parse(config.body), isCreateShareLink = testIsSharedLink(config.method, config.url), fileId = config.url.split("/").find((item => item.startsWith("f_") || item.startsWith("d_")))?.replace("f_", "").replace("d_", ""), file = filesMap.get(fileId), fileName = file?.fileName;
          if (file?.isFolder) return void console.log("Ignoring folder sharing request");
          let role, type;
          isCreateShareLink ? (role = BoxRoles.Viewer, type = BoxAccessBy_peopleWithTheLink) : (role = BoxRoles[body.permission], 
          type = BoxAccessBy_peopleInThisItem), requests = [ {
            fileId,
            fileName,
            role,
            type,
            userIds: isCreateShareLink ? [] : body.emails.split(",")
          } ];
          ...
          logError({
            error: "Intercepting access sharing request error",
            errorMessage: getErrorMessage(error),
            requests
          });
        }
    ...
    matchUrlAfter: (method, url) => function(method, url) {
      return "POST" === method && testUrl(url, "/copy");
    }(method, url),
    after: measureTimeExecution("com.box:copy", (({config}) => {
      200 !== config.status && 400 !== config.status || (config => {
        const response = JSON.parse(config.response);
        for (const successItem of response.successes || response.data.successes) {
          if (successItem.itemTypedID.startsWith("d_")) continue;
          const sourceFileId = polishFileId(successItem.itemTypedID), sourceFileName = filesMap.get(sourceFileId)?.fileName;
          sourceFileName || (details = {
            error: "missing source file name",
            filesMapLen: filesMap.size,
            sourceFileId
          }, page_logging_logFeatureError("site-events/copy", details)), dispatchOnPage("Cyberhaven_CopyCloudFile", {
            source: {
              fileId: sourceFileId,
              fileName: sourceFileName
            },
            destination: {
              fileId: polishFileId(successItem.createdItemTypedID),
              fileName: successItem.itemName
            }
          });
        }
        var details;
    ...
    matchUrlAfter: (method, url) => function(method, url) {
      return "POST" === method && testUrl(url, "/rename");
    }(method, url),
    after: measureTimeExecution("com.box:rename", (({config}) => {
      (config => {
        try {
          const fileId = config.url.split("/").find((part => part.startsWith("f_") || part.startsWith("d_")))?.replace("f_", "").replace("d_", ""), {isFolder, fileName} = fileId && filesMap.get(fileId) || {};
          if (fileId && !isFolder) {
            ...
            }), fileName ? filesMap.set(fileId, {
              ...filesMap.get(fileId),
              fileName: payload.name
            }) : logRenameCloudFileError({
              error: "old file name not found",
              url: config.url
            });
          } else fileId || logRenameCloudFileError({
