(() => {
  ...
  fn = async function() {
    ...
    try {
      return email = await (selector = 'button[data-testid="heading:userdropdown"]', new Promise((resolve => {
        const observer = new MutationObserver((() => {
          const element = document.querySelector(selector);
          if (element) {
            const titleAttribute = element.getAttribute("title"), match = titleAttribute?.match(/<([^>]+)>/);
            if (match) {
              const email = match[1];
              observer.disconnect(), resolve(email);
            } else observer.disconnect(), resolve(void 0);
          }
        }));
