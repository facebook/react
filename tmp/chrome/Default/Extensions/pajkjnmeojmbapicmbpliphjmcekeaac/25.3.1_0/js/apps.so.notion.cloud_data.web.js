(() => {
  ...
  new AbortController), unsubscribe = () => {
  ...
  detail);
  const isModalView = () => window.location.search.includes("p="), getFileIdFromUrl = (formatToUuid = !0) => {
    let url = window.location.href;
    isModalView() && (url = new URL(window.location.href).searchParams.get("p") || url);
    const match = url.match(/[a-f0-9]{32}/i);
    return match ? formatToUuid ? (id => {
      if (/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(id)) return id;
      const unformattedRegex = /^([a-f0-9]{8})([a-f0-9]{4})([a-f0-9]{4})([a-f0-9]{4})([a-f0-9]{12})$/i;
      return unformattedRegex.test(id) ? id.replace(unformattedRegex, "$1-$2-$3-$4-$5") : void 0;
    })(match[0]) : match[0] : void 0;
  }, body_parsers_jsonParseNoException = json => {
    if (json) try {
  ...
  const {dispatchOnWindow, dispatchOnWindowWithReply, onTopFrameWindow} = function(topFrameOrigins, communicationTimeout = 2500) {
    return {
      dispatchOnWindow: function(eventName, data) {
        topFrameOrigins.length && !topFrameOrigins.includes(window.origin) || window.top?.postMessage(JSON.stringify({
          event: "ch_" + eventName,
          payload: data
        }), window.origin);
      },
      dispatchOnWindowWithReply: function(eventName, data) {
        return topFrameOrigins.length && !topFrameOrigins.includes(window.origin) ? Promise.resolve(void 0) : new Promise((resolve => {
          const abortCtrl = new AbortController;
          window.addEventListener("message", (function listener(event) {
            if (topFrameOrigins.length && !topFrameOrigins.includes(event.origin)) return;
            if ("string" != typeof event.data) return;
            const data = body_parsers_jsonParseNoException(event.data), responseEventName = "ch_" + eventName + "_response";
            data.event === responseEventName && (window.removeEventListener("message", listener), 
            clearTimeout(timeoutId), resolve(data.payload));
          }), {
          ...
          const timeoutId = setTimeout((() => {
            abortCtrl.abort(), resolve(void 0);
          }), communicationTimeout);
        }));
      },
      onTopFrameWindow: function(eventName, callback) {
        if (window.self !== window.top) return;
        const listener = async function(event) {
          const pEventName = "ch_" + eventName;
          if ("string" != typeof event.data) return;
          if (topFrameOrigins.length && !topFrameOrigins.includes(event.origin)) return;
          const eventData = body_parsers_jsonParseNoException(event.data);
          if (eventData?.event === pEventName) try {
            const response = await callback(eventData.payload);
            void 0 !== response && event.source?.postMessage(JSON.stringify({
              event: pEventName + "_response",
              payload: response
            }), {
            ...
            logError({
              error: "Error in onTopFrameWindow listener",
        ...
        window.addEventListener("message", listener), (callback => {
  ...
  var fn;
  fn = async function() {
    const notionUserId = function(name) {
      const parts = `; ${document.cookie}`.split(`; ${name}=`);
      if (2 === parts.length) return parts.pop().split(";").shift();
    }("notion_user_id");
    if (!notionUserId) return;
    const notion_page_id = getFileIdFromUrl(), notion_page_name = (() => {
      if (isModalView()) {
        let title = document.querySelector(`a[href*="${getFileIdFromUrl(!1)}"]:not(:has(svg.openAsPageThick))`)?.textContent;
        return title || (title = "New page"), title;
      }
      return document.title;
    })(), notion_page_full_path = await dispatchOnWindowWithReply("ch_get_current_fullpath", null) || void 0, {email, account_id, account_name} = await async function(notion_user_id) {
      if (notionUserCache[notion_user_id]) return notionUserCache[notion_user_id];
      const response = await fetch("https://www.notion.so/api/v3/getSpaces", {
        method: "POST",
        headers: {
          "notion-client-version": "23.13.0.124",
      ...
      if (!response.ok) throw new Error("Failed to fetch account email.");
      const userInfo = (await response.json())[notion_user_id].notion_user[notion_user_id].value.value, result = {
        email: userInfo.email,
        account_id: userInfo.id,
        account_name: userInfo.name
      };
      return notionUserCache[notion_user_id] = result, result;
    }(notionUserId);
    return {
      email,
      account_type: "notion",
      notion_account_id: account_id,
      notion_account_name: account_name,
      notion_page_id,
      notion_page_name,
      notion_page_full_path
    };
  }, isWebPage() && !isContentScript() && function(eventName, callback, {ignoreSignal = !1, wrapVersion = addVersionToEventNameByDefault} = {}) {
  ...
  }("Cyberhaven_GetAccountData", (async () => {
    try {
      return await fn();
    } catch {}
    return null;
  }));
  const notionUserCache = {};
})();