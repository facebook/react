(() => {
  ...
  !function(env) {
    ...
    var fetch;
    env.fetch = (fetch = env.fetch, function(...args) {
  ...
  }, abort_controller_getSignal = () => abortController.signal;
  var util, objectUtil;
  !function(util) {
  ...
  const log = (level, data, domain = currentDomain) => {
    ...
    })("Cyberhaven_Log", eventData) : dispatchOnPage("Cyberhaven_Log", eventData);
  }, logError = (data, domain = void 0) => log("error", data, domain), page_logging_logFeatureError = (eventType, data) => {
  ...
  }, optimisticallyInjectedScripts = [ "outlook_web_app_xhr" ];
  let currentScript = globals_isWebWorkerContext() ? null : document.currentScript;
  const getCurrentScriptId = () => currentScript?.getAttribute("scriptId");
  ...
  function dispatchOnPageWithReply(eventName, data, {timeout = 1e3, wrapVersion = addVersionToEventNameByDefault} = {}) {
    const replyEvent = getReplyEventName(eventName, wrapVersion), promise = new Promise((resolve => {
      document.addEventListener(replyEvent, (function(event) {
        ...
        signal: abort_controller_getSignal(),
        once: !0,
        capture: !0
      });
      const finalEventName = wrapVersion ? addVersion(eventName) : eventName;
  ...
  const webWorkerBlob = globals_isWebWorkerContext() ? null : window.CH_WEBWORKER_SCRIPT_BLOB, rawOpen = (new Blob([ webWorkerBlob ], {
    type: "application/javascript"
  }), XMLHttpRequest.prototype.open), rawSetRequestHeader = XMLHttpRequest.prototype.setRequestHeader, xhr_hooks_randomScriptId = Math.floor(1e3 * Math.random());
  let isEnablePreviousHookListenerRegistered = !1, enabled = !1, isXhrHooked = !1;
  new WeakMap;
  const registerEnablePreviousHookListener = () => {
    isEnablePreviousHookListenerRegistered || globals_isWebWorkerContext() || (!function(eventName, callback, {ignoreSignal = !1, wrapVersion = addVersionToEventNameByDefault} = {}) {
      ...
      document.addEventListener(listenerEventName, (async function(event) {
        try {
          ...
          console.error("Error in page event listener", error), page_logging_logFeatureError("page-communication", {
            error
          }), unsubscribe();
        }
      }), {
    ...
    }("Cyberhaven_EnablePreviousHookIfExist", (({detail: {initiatedByScriptId}}) => {
  ...
  function report(eventName, data) {
    try {
      data.duration > data.expectedTimeMs && ("contentScript" === getContextName() ? async function(destination, data) {
        try {
      ...
      }(eventName, data) : dispatchOnPage(`Cyberhaven_${eventName}`, data)), data.error && logError(data);
    } catch {}
  }
  ...
  let checkPermissionResult = null;
  async function handleFixPermissionRequest(payload) {
    const {fixOptionType, recipientEmailAddresses, fileIds, role} = payload, type = function(fixOptionType) {
      switch (fixOptionType) {
       case "ADD_COLLABORATORS":
        return "user";

       case "INCREASE_PUBLIC_VISIBILITY":
        return "anyone";
      }
      return null;
    }(fixOptionType);
    if (!type) return void logError({
      error: "Unknown fix option type",
      fixOptionType,
      payload
    });
    const requests = fileIds.map((fileId => {
      const itemInfo = checkPermissionResult.itemInfo.find((item => item.id === fileId));
      return {
        fileId,
        fileName: itemInfo?.title,
        filePath: itemInfo?.title,
        role: role.toLowerCase(),
        type,
        userIds: recipientEmailAddresses
      };
    }));
    if (requests?.length) {
      const response = await dispatchOnPageWithReply("Cyberhaven_ShareData", {
        requests
      });
  ...
  function blockAccessSharingRequest(body) {
    return body.replace(/{[^}]*}/g, '{"a": 1}').replace(/PATCH \/drive\/v2internal\/files\/(.*)\/permissions\/.*\?/g, "POST /drive/v2internal/files/$1/permissions?");
  }
  const {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
        ...
        if (enabled) {
          ...
          if (beforeCallers?.length || afterCallers?.length) {
            ...
            this.send = function(body) {
              ...
              if (async) for (const beforeCaller of beforeCallers) {
                if (!beforeCaller.matchBodyBefore || beforeCaller.matchBodyBefore(body + "")) return void beforeCaller.before?.(body, config).catch((error => {
                  logError({
    ...
    return isXhrHooked = !0, enabled = !(getCurrentScriptId() && !globals_isWebWorkerContext() && optimisticallyInjectedScripts.includes(getCurrentScriptId())), 
    ...
    before: measureTimeExecution("com.google.mail:fix-permission", (async function(body) {
      if (checkPermissionResult) {
        try {
          const bodyStr = String(body), parsedBody = function(body) {
            try {
              const match = /POST\s(?:\/drive\/v2internal\/files\/fixPermissions|\/v1\/fixPermissions)\?(?<query>.*)[\s\S]+?(?<payload>\{.*\})/gm.exec(body);
              return {
            ...
            return {};
          }(bodyStr), resp = await handleFixPermissionRequest(parsedBody.payload);
          if ("block" === resp?.result) return blockAccessSharingRequest(bodyStr);
        } catch (error) {
          logError({
            error: "Failed to parse fix permission request",
            errorMessage: getErrorMessage(error),
            body
          });
        }
    ...
    matchUrlBefore: (method, url) => {
      const urlStr = url.toString();
      return (function(url) {
        return url.includes("/drive/v2internal");
      }(urlStr) || function(url) {
        return url.includes("https://drivesharefrontend-pa.clients6.google.com/batch");
      }(urlStr)) && "POST" === method;
    },
    matchBodyBefore: body => /POST (\/drive\/v2internal\/files\/fixPermissions|\/v1\/fixPermissions)/.test(body)
  }, {
    after: measureTimeExecution("com.google.mail:check-permissions", (({responseText}, xhr) => {
      try {
        const splitter = function(contentType) {
          if (!contentType) return;
          const parts = contentType.split("boundary=");
          return parts?.[1] ? "--" + parts[1] + "\r\n" : void 0;
        }(xhr.getResponseHeader("content-type"));
        if (!splitter) return;
        if (checkPermissionResult = function(text, divider) {
          const splits = text.split(divider ?? /\n\s*\n/), res = [];
          for (const part of splits) try {
            if (!part.includes("{")) continue;
            let trimmed = part.substring(part.indexOf("\n{"));
            trimmed = trimmed.substring(0, trimmed.lastIndexOf("}") + 1);
            const payload = JSON.parse(trimmed);
            res.push(payload);
          } catch {}
          return res;
        }(responseText, splitter)?.[0], !checkPermissionResult) return void logError({
          error: "Failed to parse check permission result",
          responseText
        });
      } catch {}
    })),
    matchUrlAfter(method, url) {
      const urlStr = url.toString();
      return "POST" === method && (urlStr.includes("/batch?") || urlStr.includes("/batch/drive/v2internal?"));
    },
    matchBodyAfter: body => /POST (?:\/v1\/checkPermissions|\/drive\/v2internal\/files\/checkPermissions)/.test(body)
  }, {
    after({responseText, config}) {
      try {
        checkPermissionResult = JSON.parse(responseText);
      } catch {
        logError({
          error: "Failed to parse check permission result",
          responseText,
    ...
    matchUrlAfter: (method, url) => "POST" === method && url.toString().includes("v1/checkPermissions?alt=json")
  }, {
    before: measureTimeExecution("com.google.mail:fix-permission-json", (async body => {
      if (checkPermissionResult) {
        try {
          const bodyStr = String(body), resp = await handleFixPermissionRequest(JSON.parse(bodyStr));
          if ("block" === resp?.result) return blockAccessSharingRequest("{}");
        } catch {
          logError({
            error: "Failed to parse fix permission request",
            body
    ...
    matchUrlBefore: (method, url) => "POST" === method && url.toString().includes("v1/fixPermissions?alt=json")
  } ]);
  var callback;
  callback = unhook, abort_controller_getSignal().addEventListener("abort", callback);
})();