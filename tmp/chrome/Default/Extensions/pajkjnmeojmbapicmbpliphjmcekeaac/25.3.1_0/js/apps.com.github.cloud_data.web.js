(() => {
  ...
  async function getAccountEmail() {
    ...
    return userEmail || "github.com" !== location.hostname || (userEmail = await getUserEmail(), 
  ...
  })("github"), listenPageEvent("Cyberhaven_GetAccountData", (async () => ({
    email: await getAccountEmail(),
    account_type: "github"
  })));
  const getUserName = async () => "github.com" === location.hostname ? document.querySelector('meta[name="user-login"]')?.content : "github.dev" === location.hostname ? await dispatchOnPageWithReply("Cyberhaven_WebAppCookies") : null, getUserEmail = async () => {
    try {
      const url = "https://github.com/settings/emails", response = await fetch(url, {
        method: "GET"
      }), text = await response.text(), htmlDOM = (new DOMParser).parseFromString(text, "text/html"), cssSelector = "select[id='primary_email_select'] option[selected]";
      return htmlDOM.querySelector(cssSelector)?.innerHTML;
    } catch (e) {}
    return null;
  };
  var fn;
  fn = () => {
    "github.com" === location.hostname && getAccountEmail();
