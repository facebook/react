(() => {
  ...
  (delay, reason, defaultResolve = void 0) => new Promise(((resolve, reject) => setTimeout((() => void 0 === reason ? resolve(defaultResolve) : reject(reason)), delay)))), abortController = new AbortController, getSignal = () => abortController.signal;
  var util, objectUtil;
  !function(util) {
  ...
  async function getAccountEmail() {
    ...
    const storage = await dispatchOnPageWithReply("Cyberhaven_WebAppStorage", {
      type: "get",
      key: "gitlab",
      storage: "permanent"
    }) || {};
    let userEmail = storage[userName];
    return userEmail || location.pathname.includes("/-/ide/") || (userEmail = await getUserEmail(), 
    userEmail && dispatchOnPageWithReply("Cyberhaven_WebAppStorage", {
      type: "set",
      key: "gitlab",
      value: {
        ...storage,
  ...
  const getUsername = () => {
    if (location.pathname.includes("/-/ide/")) return window.gon.current_username;
    {
      const assigneeEls = document.querySelectorAll('a[href*="assignee_username"]'), userName = assigneeEls?.[0]?.getAttribute("href")?.split("=")[1];
      return userName;
    }
  }, getUserEmail = async () => {
    try {
      const url = "https://gitlab.com/-/user_settings/profile", response = await fetch(url, {
        method: "GET"
      }), text = await response.text(), htmlDOM = (new DOMParser).parseFromString(text, "text/html");
      return function(expression, obj = document, root = document, resolver, type, result) {
        ...
        var xpr;
      }('//*[@id="user_email"]', htmlDOM, htmlDOM, null, XPathResult.ANY_TYPE, null)[0].value;
    } catch {}
    return null;
  };
  var fn;
  fn = getAccountEmail, "loading" === document.readyState ? document.addEventListener("DOMContentLoaded", fn) : fn();
})();