(() => {
  ...
  fn = async function() {
    let email = "";
    if ("chatgpt.com" === location.hostname) email = "undefined" != typeof __reactRouterContext && __reactRouterContext.state?.loaderData?.root?.clientBootstrap?.user?.email || ""; else if ("platform.openai.com" === location.hostname) {
      const userTraits = (json => {
        if (json) try {
      ...
      userTraits.$email && (email = userTraits.$email);
      const key = Object.keys(localStorage).find((key => key.includes("@@auth0spajs@@") && key.includes("email")));
      key && (email = JSON.parse(localStorage.getItem(key) || "{}").body.decodedToken.user.email);
    }
    return {
      email,
      account_type: "openai"
