(() => {
  ...
  (delay, reason, defaultResolve = void 0) => new Promise(((resolve, reject) => setTimeout((() => void 0 === reason ? resolve(defaultResolve) : reject(reason)), delay)))), abortController = new AbortController, abort_controller_getSignal = () => abortController.signal, onCleanup = callback => {
  ...
  const log = (level, data, domain = currentDomain) => {
    ...
    globals_isWebWorkerContext() ? webworker_communication_dispatchOnWebWorker("Cyberhaven_Log", eventData) : function(eventName, data, {wrapVersion = addVersionToEventNameByDefault} = {}) {
  ...
  const xhrConfigs = self.CYBERHAVEN_XHR_CONFIGS, fetchConfigs = self.CYBERHAVEN_FETCH_CONFIGS;
  if (fetchConfigs) {
    for (let i = 0; i < fetchConfigs.length; i++) fetchConfigs[i].before = async (body, config) => {
      const promise = new Promise((resolve => {
        var eventType, callback;
        eventType = "ch_fetch_hook_reply", callback = data => {
          resolve(data.body);
        }, self.onmessage = event => {
          const {type, eventData} = event.data;
          type === eventType && callback(eventData);
      ...
      return await Promise.race([ promise, awaitTimeout(1e3, void 0, config.body) ]);
    ...
    const {unhookFetch} = function(options) {
      if (isFetchHooked) throw Error("Fetch Hooker is already hooked!");
      registerEnablePreviousHookListener(), isFetchHooked = !0, enabled = !optimistically_injected_isCurrentScriptOptimisticallyInjected();
  ...
  if (xhrConfigs) {
    for (let i = 0; i < xhrConfigs.length; i++) xhrConfigs[i].before = async (_body, config) => config.body, 
    xhrConfigs[i].after = (responseObject, _xhr) => {
      responseObject.config.xhr = void 0, webworker_communication_dispatchOnWebWorker("ch_xhr_hook", {
        configIndex: i,
        configType: "after",
        args: [ responseObject ]
      });
    ...
    const {unhook} = function(options, {enablePreviousScriptOnReload = !1} = {}) {
      ...
      }), XMLHttpRequest.prototype.open = handleXhrOpen, XMLHttpRequest.prototype.setRequestHeader = handleSetRequestHeader, 
      {
        enableHook: (toEnable = !0) => {
          enabled = toEnable, toEnable && enablePreviousScriptOnReload && function(eventName, data, {timeout = 1e3, wrapVersion = addVersionToEventNameByDefault} = {}) {
          ...
          }("Cyberhaven_EnablePreviousHookIfExist", {
