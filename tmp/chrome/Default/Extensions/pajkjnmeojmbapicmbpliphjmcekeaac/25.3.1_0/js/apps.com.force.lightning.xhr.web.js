(() => {
  ...
  var __webpack_modules__ = {
    7242: (__unused_webpack_module, exports, __webpack_require__) => {
      var until_1 = __webpack_require__(5958);
      exports.T = until_1.until;
    },
    5958: (__unused_webpack_module, exports) => {
    ...
    6296: (__unused_webpack_module, exports, __webpack_require__) => {
      ...
      const MemoryLeakError_1 = __webpack_require__(7240);
      class Emitter {
        constructor() {
          this.events = new Map, this.maxListeners = Emitter.defaultMaxListeners, this.hasWarnedAboutPotentialMemoryLeak = !1;
        }
        ...
        _wrapOnceListener(eventName, listener) {
          const onceListener = (...data) => {
            this.removeListener(eventName, onceListener), listener.apply(this, data);
          };
          return onceListener;
        }
        ...
        addListener(eventName, listener) {
          ...
          if (this.events.set(eventName, nextListeners), this.maxListeners > 0 && this.listenerCount(eventName) > this.maxListeners && !this.hasWarnedAboutPotentialMemoryLeak) {
        ...
        removeListener(eventName, listener) {
          const listeners = this._getListeners(eventName);
          return listeners.length > 0 && (this._removeListener(listeners, listener), this.events.set(eventName, listeners), 
          this._emitInternalEvent("removeListener", eventName, listener)), this;
        }
        off(eventName, listener) {
          return this.removeListener(eventName, listener);
        }
        removeAllListeners(eventName) {
          return eventName ? this.events.delete(eventName) : this.events.clear(), this;
        }
        listeners(eventName) {
      ...
      exports.Emitter = Emitter, Emitter.defaultMaxListeners = 10;
    },
    7240: (__unused_webpack_module, exports) => {
      ...
      class MemoryLeakError extends Error {
        constructor(emitter, type, count) {
          ...
          this.emitter = emitter, this.type = type, this.count = count, this.name = "MaxListenersExceededWarning";
        }
      }
    ...
    8306: function(__unused_webpack_module, exports, __webpack_require__) {
      ...
      Object.defineProperty(exports, "__esModule", {
        value: !0
      }), __exportStar(__webpack_require__(6296), exports), __exportStar(__webpack_require__(7240), exports);
    }
  ...
  var RESPONSE_STATUS_CODES_WITHOUT_BODY = new Set([ 101, 103, 204, 205, 304 ]);
  ...
  var DeferredPromise = class extends Promise {
    ...
    constructor(executor = null) {
      const deferredExecutor = function() {
        const executor = (resolve, reject) => {
          executor.state = "pending", executor.resolve = data => {
            if ("pending" === executor.state) return executor.result = data, resolve(data instanceof Promise ? data : Promise.resolve(data).then((value => (executor.state = "fulfilled", 
            value))));
          }, executor.reject = reason => {
            if ("pending" === executor.state) return queueMicrotask((() => {
      ...
      super(((originalResolve, originalReject) => {
        deferredExecutor(originalResolve, originalReject), executor?.(deferredExecutor.resolve, deferredExecutor.reject);
      })), this.#executor = deferredExecutor, this.resolve = this.#executor.resolve, this.reject = this.#executor.reject;
    }
    get state() {
  ...
  }, lib = __webpack_require__(7242), InterceptorError = class extends Error {
    constructor(message) {
      super(message), this.name = "InterceptorError", Object.setPrototypeOf(this, InterceptorError.prototype);
    }
  }, kRequestHandled = Symbol("kRequestHandled"), kResponsePromise = Symbol("kResponsePromise"), RequestController = class {
    constructor(request) {
      this.request = request, this[kRequestHandled] = !1, this[kResponsePromise] = new DeferredPromise;
    }
    respondWith(response) {
      ...
      this[kRequestHandled] = !0, this[kResponsePromise].resolve(response);
    }
    errorWith(error) {
      ...
      this[kRequestHandled] = !0, this[kResponsePromise].resolve(error);
    }
  };
  async function emitAsync(emitter, eventName, ...data) {
    const listners = emitter.listeners(eventName);
    if (0 !== listners.length) for (const listener of listners) await listener.apply(emitter, data);
  }
  async function handleRequest(options) {
    const handleResponse = async response => (response instanceof Error ? options.onError(response) : !function(response) {
    ...
    }(response) ? await options.onResponse(response) : options.onRequestError(response), 
    !0), handleResponseError = async error => {
      if (error instanceof InterceptorError) throw result.error;
      return function(error) {
        return null != error && error instanceof Error && "code" in error && "errno" in error;
      }(error) ? (options.onError(error), !0) : error instanceof Response && await handleResponse(error);
    };
    options.emitter.once("request", (({requestId: pendingRequestId}) => {
      pendingRequestId === options.requestId && "pending" === options.controller[kResponsePromise].state && options.controller[kResponsePromise].resolve(void 0);
    }));
    const requestAbortPromise = new DeferredPromise;
    options.request.signal && options.request.signal.addEventListener("abort", (() => {
      requestAbortPromise.reject(options.request.signal.reason);
    }), {
      once: !0
    });
    const result = await (0, lib.T)((async () => {
      const requestListtenersPromise = emitAsync(options.emitter, "request", {
        requestId: options.requestId,
        request: options.request,
        controller: options.controller
      });
      await Promise.race([ requestAbortPromise, requestListtenersPromise, options.controller[kResponsePromise] ]);
      return await options.controller[kResponsePromise];
    }));
    if ("rejected" === requestAbortPromise.state) return options.onError(requestAbortPromise.rejectionReason), 
    !0;
    if (result.error) {
      ...
      if (options.emitter.listenerCount("unhandledException") > 0) {
        ...
        await emitAsync(options.emitter, "unhandledException", {
          ...
          "pending" === unhandledExceptionController[kResponsePromise].state && unhandledExceptionController[kResponsePromise].resolve(void 0);
        }));
        const nextResult = await (0, lib.T)((() => unhandledExceptionController[kResponsePromise]));
        if (nextResult.error) return handleResponseError(nextResult.error);
        if (nextResult.data) return handleResponse(nextResult.data);
      }
      return options.onResponse((body = result.error, new Response(JSON.stringify(body instanceof Error ? {
        name: body.name,
  ...
  function isNodeProcess() {
    ...
    if ("undefined" != typeof process) {
      ...
      return "renderer" !== type && "worker" !== type && !(!process.versions || !process.versions.node);
    }
    return !1;
  }
  var __defProp = Object.defineProperty, colors_exports = {};
  ...
  var IS_NODE = isNodeProcess(), Logger = class {
    constructor(name) {
      this.name = name, this.prefix = `[${this.name}]`;
      const LOGGER_NAME = getVariable("DEBUG"), LOGGER_LEVEL = getVariable("LOG_LEVEL");
      "1" === LOGGER_NAME || "true" === LOGGER_NAME || void 0 !== LOGGER_NAME && this.name.startsWith(LOGGER_NAME) ? (this.debug = isDefinedAndNotEquals(LOGGER_LEVEL, "debug") ? noop : this.debug, 
      this.info = isDefinedAndNotEquals(LOGGER_LEVEL, "info") ? noop : this.info, this.success = isDefinedAndNotEquals(LOGGER_LEVEL, "success") ? noop : this.success, 
      this.warning = isDefinedAndNotEquals(LOGGER_LEVEL, "warning") ? noop : this.warning, 
      this.error = isDefinedAndNotEquals(LOGGER_LEVEL, "error") ? noop : this.error) : (this.info = noop, 
      this.success = noop, this.warning = noop, this.error = noop, this.only = noop);
    }
    prefix;
    extend(domain) {
    ...
    info(message, ...positionals) {
      ...
      return (message2, ...positionals2) => {
        performance2.measure(), this.logEntry({
          level: "info",
    ...
    logEntry(args) {
      const {level, message, prefix, colors: customColors, positionals = []} = args, entry = this.createEntry(level, message), colorize = {
        timestamp: colors_exports[customColors?.timestamp || "gray"],
        prefix: colors_exports[customColors?.prefix || "gray"]
      };
      this.getWriter(level)([ colorize.timestamp(this.formatTimestamp(entry.timestamp)) ].concat(null != prefix ? colorize.prefix(prefix) : []).concat(serializeInput(message)).join(" "), ...positionals.map(serializeInput));
    }
  ...
  function log(message, ...positionals) {
    IS_NODE ? process.stdout.write(format(message, ...positionals) + "\n") : console.log(message, ...positionals);
  }
  function warn(message, ...positionals) {
    IS_NODE ? process.stderr.write(format(message, ...positionals) + "\n") : console.warn(message, ...positionals);
  }
  function error(message, ...positionals) {
    IS_NODE ? process.stderr.write(format(message, ...positionals) + "\n") : console.error(message, ...positionals);
  }
  function getVariable(variableName) {
    return IS_NODE ? process.env[variableName] : globalThis[variableName]?.toString();
  }
  function isDefinedAndNotEquals(value, expected) {
    return void 0 !== value && value !== expected;
  }
  function serializeInput(message) {
    return void 0 === message ? "undefined" : null === message ? "null" : "string" == typeof message ? message : "object" == typeof message ? JSON.stringify(message) : message.toString();
  }
  var strict_event_emitter_lib = __webpack_require__(8306);
  function getGlobalSymbol(symbol) {
    return globalThis[symbol] || void 0;
  }
  var Interceptor = class {
    constructor(symbol) {
      this.symbol = symbol, this.readyState = "INACTIVE", this.emitter = new strict_event_emitter_lib.Emitter, 
      this.subscriptions = [], this.logger = new Logger(symbol.description), this.emitter.setMaxListeners(0), 
    ...
    apply() {
      ...
      if (logger.info("applying the interceptor..."), "APPLIED" === this.readyState) return void logger.info("intercepted already applied!");
      if (!this.checkEnvironment()) return void logger.info("the interceptor cannot be applied in this environment!");
      this.readyState = "APPLYING";
      const runningInstance = this.getInstance();
      if (runningInstance) return logger.info("found a running instance, reusing..."), 
      this.on = (event, listener) => (logger.info('proxying the "%s" listener', event), 
      runningInstance.emitter.addListener(event, listener), this.subscriptions.push((() => {
        runningInstance.emitter.removeListener(event, listener), logger.info('removed proxied "%s" listener!', event);
      })), this), void (this.readyState = "APPLIED");
      logger.info("no running instance found, setting up a new instance..."), this.setup(), 
      this.setInstance(), this.readyState = "APPLIED";
    }
    setup() {}
    on(event, listener) {
      const logger = this.logger.extend("on");
      return "DISPOSING" === this.readyState || "DISPOSED" === this.readyState ? (logger.info("cannot listen to events, already disposed!"), 
      this) : (logger.info('adding "%s" event listener:', event, listener), this.emitter.on(event, listener), 
      this);
    }
    once(event, listener) {
    ...
    dispose() {
      ...
      if ("DISPOSED" !== this.readyState) if (logger.info("disposing the interceptor..."), 
      this.readyState = "DISPOSING", this.getInstance()) {
        if (this.clearInstance(), logger.info("global symbol deleted:", getGlobalSymbol(this.symbol)), 
        this.subscriptions.length > 0) {
          logger.info("disposing of %d subscriptions...", this.subscriptions.length);
          for (const dispose of this.subscriptions) dispose();
          this.subscriptions = [], logger.info("disposed of all subscriptions!", this.subscriptions.length);
        }
        this.emitter.removeAllListeners(), logger.info("destroyed the listener!"), this.readyState = "DISPOSED";
      } else logger.info("no interceptors running, skipping dispose..."); else logger.info("cannot dispose, already disposed!");
    }
    getInstance() {
      var _a;
      const instance = getGlobalSymbol(this.symbol);
      return this.logger.info("retrieved global instance:", null == (_a = null == instance ? void 0 : instance.constructor) ? void 0 : _a.name), 
      instance;
    }
    setInstance() {
      var symbol, value;
      symbol = this.symbol, value = this, globalThis[symbol] = value, this.logger.info("set global instance!", this.symbol.description);
    }
    clearInstance() {
      var symbol;
      symbol = this.symbol, delete globalThis[symbol], this.logger.info("cleared global instance!", this.symbol.description);
    }
  };
  var EventPolyfill = class {
    constructor(type, options) {
      this.NONE = 0, this.CAPTURING_PHASE = 1, this.AT_TARGET = 2, this.BUBBLING_PHASE = 3, 
      this.type = "", this.srcElement = null, this.currentTarget = null, this.eventPhase = 0, 
      this.isTrusted = !0, this.composed = !1, this.cancelable = !0, this.defaultPrevented = !1, 
      this.bubbles = !0, this.lengthComputable = !0, this.loaded = 0, this.total = 0, 
      this.cancelBubble = !1, this.returnValue = !0, this.type = type, this.target = (null == options ? void 0 : options.target) || null, 
      this.currentTarget = (null == options ? void 0 : options.currentTarget) || null, 
    ...
    constructor(type, init) {
      super(type), this.lengthComputable = (null == init ? void 0 : init.lengthComputable) || !1, 
      this.composed = (null == init ? void 0 : init.composed) || !1, this.loaded = (null == init ? void 0 : init.loaded) || 0, 
      this.total = (null == init ? void 0 : init.total) || 0;
    }
  ...
  function createProxy(target, options) {
    const proxy = new Proxy(target, function(options) {
      const {constructorCall, methodCall, getProperty, setProperty} = options, handler = {};
      void 0 !== constructorCall && (handler.construct = function(target, args, newTarget) {
        const next = Reflect.construct.bind(null, target, args, newTarget);
        return constructorCall.call(newTarget, args, next);
      });
      return handler.set = function(target, propertyName, nextValue) {
        const next = () => {
          const propertySource = findPropertySource(target, propertyName) || target, ownDescriptors = Reflect.getOwnPropertyDescriptor(propertySource, propertyName);
          return void 0 !== (null == ownDescriptors ? void 0 : ownDescriptors.set) ? (ownDescriptors.set.apply(target, [ nextValue ]), 
          !0) : Reflect.defineProperty(propertySource, propertyName, {
            writable: !0,
            enumerable: !0,
            configurable: !0,
            value: nextValue
        ...
        return void 0 !== setProperty ? setProperty.call(target, [ propertyName, nextValue ], next) : next();
      }, handler.get = function(target, propertyName, receiver) {
        const next = () => target[propertyName], value = void 0 !== getProperty ? getProperty.call(target, [ propertyName, receiver ], next) : next();
        return "function" == typeof value ? (...args) => {
          const next2 = value.bind(target, ...args);
          return void 0 !== methodCall ? methodCall.call(target, [ propertyName, args ], next2) : next2();
        } : value;
      }, handler;
    }(options));
    return proxy;
  ...
  function createResponse(request, body) {
    const responseBodyOrNull = (status = request.status, RESPONSE_STATUS_CODES_WITHOUT_BODY.has(status) ? null : body);
    var status;
  ...
  function createHeadersFromXMLHttpReqestHeaders(headersString) {
    const headers = new Headers, lines = headersString.split(/[\r\n]+/);
    for (const line of lines) {
      if ("" === line.trim()) continue;
      const [name, ...parts] = line.split(": "), value = parts.join(": ");
  ...
  async function getBodyByteLength(input) {
    ...
    if (null != explicitContentLength && "" !== explicitContentLength) return Number(explicitContentLength);
    return (await input.arrayBuffer()).byteLength;
  }
  var kIsRequestHandled = Symbol("kIsRequestHandled"), chunk_KOFJYPUE_IS_NODE = isNodeProcess(), kFetchRequest = Symbol("kFetchRequest"), XMLHttpRequestController = class {
    constructor(initialRequest, logger) {
      this.initialRequest = initialRequest, this.logger = logger, this.method = "GET", 
      this.url = null, this[kIsRequestHandled] = !1, this.events = new Map, this.uploadEvents = new Map, 
      this.requestId = Math.random().toString(16).slice(2), this.requestHeaders = new Headers, 
      this.responseBuffer = new Uint8Array, this.request = createProxy(initialRequest, {
        setProperty: ([propertyName, nextValue], invoke) => {
          if ("ontimeout" === propertyName) {
            const eventName = propertyName.slice(2);
            return this.request.addEventListener(eventName, nextValue), invoke();
          }
          return invoke();
        },
           ...
           case "open":
            {
              ...
              return void 0 === url ? (this.method = "GET", this.url = toAbsoluteUrl(method)) : (this.method = method, 
              this.url = toAbsoluteUrl(url)), this.logger = this.logger.extend(`${this.method} ${this.url.href}`), 
              this.logger.info("open", this.method, this.url.href), invoke();
            }

           case "addEventListener":
            {
              const [eventName, listener] = args;
              return this.registerEvent(eventName, listener), this.logger.info("addEventListener", eventName, listener), 
              invoke();
            }
           ...
           case "setRequestHeader":
            {
              ...
              return this.requestHeaders.set(name, value), this.logger.info("setRequestHeader", name, value), 
              invoke();
            }
           ...
           case "send":
            {
              ...
              const requestBody = "string" == typeof body ? (text = body, encoder.encode(text)) : body, fetchRequest = this.toFetchApiRequest(requestBody);
              this[kFetchRequest] = fetchRequest;
              ((null == (_a = this.onRequest) ? void 0 : _a.call(this, {
                request: fetchRequest,
                requestId: this.requestId
              })) || Promise.resolve()).finally((() => {
                if (!this[kIsRequestHandled]) return this.logger.info("request callback settled but request has not been handled (readystate %d), performing as-is...", this.request.readyState), 
                chunk_KOFJYPUE_IS_NODE && this.request.setRequestHeader("x-interceptors-internal-request-id", this.requestId), 
                invoke();
              }));
              break;
            }
      ...
      }), chunk_KOFJYPUE_define(this.request, "upload", createProxy(this.request.upload, {
        setProperty: ([propertyName, nextValue], invoke) => {
          switch (propertyName) {
           case "onloadstart":
           case "onprogress":
        ...
        methodCall: ([methodName, args], invoke) => {
          if ("addEventListener" === methodName) {
            const [eventName, listener] = args;
            return this.registerUploadEvent(eventName, listener), this.logger.info("upload.addEventListener", eventName, listener), 
            invoke();
          }
        }
      }));
    ...
    registerEvent(eventName, listener) {
      ...
      this.events.set(eventName, nextEvents), this.logger.info('registered event "%s"', eventName, listener);
    }
    registerUploadEvent(eventName, listener) {
      const nextEvents = (this.uploadEvents.get(eventName) || []).concat(listener);
      this.uploadEvents.set(eventName, nextEvents), this.logger.info('registered upload event "%s"', eventName, listener);
    }
    async respondWith(response) {
      if (this[kIsRequestHandled] = !0, this[kFetchRequest]) {
        const totalRequestBodyLength = await getBodyByteLength(this[kFetchRequest].clone());
      ...
      chunk_KOFJYPUE_define(this.request, "status", response.status), chunk_KOFJYPUE_define(this.request, "statusText", response.statusText), 
      chunk_KOFJYPUE_define(this.request, "responseURL", this.url.href), this.request.getResponseHeader = new Proxy(this.request.getResponseHeader, {
        apply: (_, __, args) => {
          if (this.logger.info("getResponseHeader", args[0]), this.request.readyState < this.request.HEADERS_RECEIVED) return this.logger.info("headers not received yet, returning null"), 
          null;
          const headerValue = response.headers.get(args[0]);
          return this.logger.info('resolved response header "%s" to', args[0], headerValue), 
          headerValue;
        }
        ...
        apply: () => {
          ...
          "";
          const allHeaders = Array.from(response.headers.entries()).map((([headerName, headerValue]) => `${headerName}: ${headerValue}`)).join("\r\n");
          return this.logger.info("resolved all response headers to", allHeaders), allHeaders;
        }
      ...
      this.logger.info("calculated response body length", totalResponseBodyLength), this.trigger("loadstart", this.request, {
        loaded: 0,
        total: totalResponseBodyLength
      }), this.setReadyState(this.request.HEADERS_RECEIVED), this.setReadyState(this.request.LOADING);
      const finalizeResponse = () => {
        this.logger.info("finalizing the mocked response..."), this.setReadyState(this.request.DONE), 
      ...
      if (response.body) {
        ...
        const reader = response.body.getReader(), readNextResponseBodyChunk = async () => {
          const {value, done} = await reader.read();
          if (done) return this.logger.info("response body stream done!"), void finalizeResponse();
          value && (this.logger.info("read response body chunk:", value), this.responseBuffer = function(left, right) {
            const result = new Uint8Array(left.byteLength + right.byteLength);
            return result.set(left, 0), result.set(right, left.byteLength), result;
          }(this.responseBuffer, value), this.trigger("progress", this.request, {
            loaded: this.responseBuffer.byteLength,
            total: totalResponseBodyLength
    ...
    responseBufferToText() {
      return buffer = this.responseBuffer, new TextDecoder(encoding).decode(buffer);
      var buffer, encoding;
    }
    get response() {
      if (this.logger.info("getResponse (responseType: %s)", this.request.responseType), 
      this.request.readyState !== this.request.DONE) return null;
       ...
       case "json":
        {
          ...
          return this.logger.info("resolved response JSON", responseJson), responseJson;
        }

       case "arraybuffer":
        {
          const arrayBuffer = (array = this.responseBuffer).buffer.slice(array.byteOffset, array.byteOffset + array.byteLength);
          return this.logger.info("resolved response ArrayBuffer", arrayBuffer), arrayBuffer;
        }

       case "blob":
        {
          const mimeType = this.request.getResponseHeader("Content-Type") || "text/plain", responseBlob = new Blob([ this.responseBufferToText() ], {
            type: mimeType
          });
          return this.logger.info("resolved response Blob (mime type: %s)", responseBlob, mimeType), 
          responseBlob;
        }
       ...
       default:
        {
          ...
          return this.logger.info('resolving "%s" response type as text', this.request.responseType, responseText), 
          responseText;
        }
      }
      var array;
    ...
    get responseText() {
      if (invariant("" === this.request.responseType || "text" === this.request.responseType, "InvalidStateError: The object is in invalid state."), 
      this.request.readyState !== this.request.LOADING && this.request.readyState !== this.request.DONE) return "";
      const responseText = this.responseBufferToText();
      return this.logger.info('getResponseText: "%s"', responseText), responseText;
    }
    get responseXML() {
      if (invariant("" === this.request.responseType || "document" === this.request.responseType, "InvalidStateError: The object is in invalid state."), 
      this.request.readyState !== this.request.DONE) return null;
      const contentType = this.request.getResponseHeader("Content-Type") || "";
      return "undefined" == typeof DOMParser ? (console.warn("Cannot retrieve XMLHttpRequest response body as XML: DOMParser is not defined. You are likely using an environment that is not browser or does not polyfill browser globals correctly."), 
      null) : (type = contentType, [ "application/xhtml+xml", "application/xml", "image/svg+xml", "text/html", "text/xml" ].some((supportedType => type.startsWith(supportedType))) ? (new DOMParser).parseFromString(this.responseBufferToText(), contentType) : null);
      var type;
    }
    errorWith(error) {
      this[kIsRequestHandled] = !0, this.logger.info("responding with an error"), this.setReadyState(this.request.DONE), 
      this.trigger("error", this.request), this.trigger("loadend", this.request);
    }
    setReadyState(nextReadyState) {
      this.logger.info("setReadyState: %d -> %d", this.request.readyState, nextReadyState), 
      this.request.readyState !== nextReadyState ? (chunk_KOFJYPUE_define(this.request, "readyState", nextReadyState), 
      this.logger.info("set readyState to: %d", nextReadyState), nextReadyState !== this.request.UNSENT && (this.logger.info('triggerring "readystatechange" event...'), 
      this.trigger("readystatechange", this.request))) : this.logger.info("ready state identical, skipping transition...");
    }
    trigger(eventName, target, options) {
      const callback = target[`on${eventName}`], event = function(target, type, init) {
        const ProgressEventClass = SUPPORTS_PROGRESS_EVENT ? ProgressEvent : ProgressEventPolyfill;
        return [ "error", "progress", "loadstart", "loadend", "load", "timeout", "abort" ].includes(type) ? new ProgressEventClass(type, {
          lengthComputable: !0,
          loaded: (null == init ? void 0 : init.loaded) || 0,
          total: (null == init ? void 0 : init.total) || 0
        }) : new EventPolyfill(type, {
          target,
          currentTarget: target
        });
      }(target, eventName, options);
      this.logger.info('trigger "%s"', eventName, options || ""), "function" == typeof callback && (this.logger.info('found a direct "%s" callback, calling...', eventName), 
      callback.call(target, event));
      const events = target instanceof XMLHttpRequestUpload ? this.uploadEvents : this.events;
      for (const [registeredEventName, listeners] of events) registeredEventName === eventName && (this.logger.info('found %d listener(s) for "%s" event, calling...', listeners.length, eventName), 
      listeners.forEach((listener => listener.call(target, event))));
    }
    toFetchApiRequest(body) {
      this.logger.info("converting request to a Fetch API Request...");
      const resolvedBody = body instanceof Document ? body.documentElement.innerText : body, fetchRequest = new Request(this.url.href, {
        method: this.method,
        headers: this.requestHeaders,
        credentials: this.request.withCredentials ? "include" : "same-origin",
        body: [ "GET", "HEAD" ].includes(this.method.toUpperCase()) ? null : resolvedBody
      }), proxyHeaders = createProxy(fetchRequest.headers, {
        methodCall: ([methodName, args], invoke) => {
          switch (methodName) {
           case "append":
           case "set":
      ...
      return chunk_KOFJYPUE_define(fetchRequest, "headers", proxyHeaders), this.logger.info("converted request to a Fetch API Request!", fetchRequest), 
      fetchRequest;
    }
  };
  function toAbsoluteUrl(url) {
    return "undefined" == typeof location ? new URL(url) : new URL(url.toString(), location.href);
  }
  function chunk_KOFJYPUE_define(target, property, value) {
  ...
  var _XMLHttpRequestInterceptor = class extends Interceptor {
    ...
    setup() {
      ...
      globalThis.XMLHttpRequest = function({emitter, logger}) {
        const XMLHttpRequestProxy = new Proxy(globalThis.XMLHttpRequest, {
          construct(target, args, newTarget) {
            logger.info("constructed new XMLHttpRequest");
            const originalRequest = Reflect.construct(target, args, newTarget), prototypeDescriptors = Object.getOwnPropertyDescriptors(target.prototype);
            ...
            return xhrRequestController.onRequest = async function({request, requestId}) {
              const controller = new RequestController(request);
              this.logger.info("awaiting mocked response..."), this.logger.info('emitting the "request" event for %s listener(s)...', emitter.listenerCount("request"));
              const isRequestHandled = await handleRequest({
                ...
                onError: error => {
                  this.logger.info("request errored!", {
                    error
                  }), error instanceof Error && this.errorWith(error);
                }
              });
              isRequestHandled || this.logger.info("no mocked response received, performing request as-is...");
      ...
      Object.defineProperty(globalThis.XMLHttpRequest, IS_PATCHED_MODULE, {
        ...
        Object.defineProperty(globalThis.XMLHttpRequest, IS_PATCHED_MODULE, {
          value: void 0
        }), globalThis.XMLHttpRequest = PureXMLHttpRequest, logger.info('native "XMLHttpRequest" module restored!', globalThis.XMLHttpRequest.name);
      }));
    }
  ...
  const isWebWorkerContext = () => "function" == typeof importScripts && !function() {
    ...
    return "unknown";
  }().includes("background"), awaitTimeout = (isWebWorkerContext() ? self : window, 
  ...
  function getErrorMessage(error) {
    let message = "string" == typeof error ? error : function(maybeError) {
      ...
      if (function(error) {
        return "object" == typeof error && null !== error && "message" in error && "string" == typeof error.message;
      }(maybeError)) return maybeError;
      try {
  ...
  let currentDomain;
  const page_logging_log = (level, data, domain = currentDomain) => {
    const eventData = {
      level,
      domain
    };
  ...
  }, logError = (data, domain = void 0) => page_logging_log("error", data, domain), logFeatureError = (eventType, data) => {
  ...
  const registerEvents = (service, scriptId) => {
  ...
  }, actionShape = z.object({
    id: z.string(),
    descriptor: z.string(),
    params: z.record(z.string(), z.any())
  }), actionsSchemaShape = z.array(actionShape), actionsSchema = z.preprocess((function(str, ctx) {
    if ("string" == typeof str) try {
      const decoded = decodeURIComponent(str);
      return JSON.parse(new URLSearchParams(decoded).get("message") || "{}").actions;
    } catch (e) {
      ctx.addIssue({
        code: ZodIssueCode.custom,
    ...
    return str;
  }), actionsSchemaShape), responseActionsSchema = z.object({
    ...
    context: z.object({
      globalValueProviders: z.array(z.object({
        type: z.string(),
        values: z.any()
      })).optional()
    })
  }), saveContentActionParams = (z.object({
    Id: z.string(),
    Name: z.string(),
    sobjectType: z.string()
  }), z.object({
    id: z.string(),
    apiName: z.union([ z.literal("Opportunity"), z.literal("Case"), z.literal("Account"), z.literal("ContentDocument"), z.literal("Contact") ]),
    eTag: z.string(),
    ...
    LatestPublishedVersion: z.object({
      ContentSize: z.number(),
      FileExtension: z.string(),
      FileType: z.string(),
      Id: z.string(),
      PathOnClient: z.string(),
      sobjectType: z.literal("ContentVersion")
    }),
    LastModifiedDate: z.string(),
    OwnerId: z.string(),
    sobjectType: z.literal("ContentDocument"),
    apiName: z.literal("ContentDocument")
  }), z.object({
    contentBodyId: z.string(),
    firstPublishLocationId: z.string(),
    pathOnClient: z.string(),
    title: z.string()
  }));
  ((service, scriptId) => {
  ...
  const SUPPORTED_DELETE_RECORDS = [ "Opportunity", "Account", "Case" ], xhr_web_cache = getCache(), interceptor = new XMLHttpRequestInterceptor;
  interceptor.apply();
  const SUPPORTED_REQUESTS = {
    LIST_REPORTS: {
      QUERY: "ui-folder-components-aura-components-controller.FolderHome.getRecords=1",
      DESCRIPTOR: "serviceComponent://ui.folder.components.aura.components.controller.FolderHomeController/ACTION$getRecords"
    },
    LIST_OBJECTS: {
      QUERY: "ui-force-components-controllers-lists-listViewDataManager.ListViewDataManager.getItems=1",
    ...
    RELATED_LIST_OBJECTS: {
      QUERY: "aura.RelatedListUi.postRelatedListRecords=1",
      DESCRIPTOR: "aura://RelatedListUiController/ACTION$postRelatedListRecords"
    },
    DESCRIBE_REPORT: {
      QUERY: "ui-analytics-reporting-runpage.ReportPage.describeReport=1",
      DESCRIPTOR: "serviceComponent://ui.analytics.reporting.runpage.ReportPageController/ACTION$describeReport"
    },
    GET_RECORD: {
      QUERY: "aura.RecordUi.getRecordWithFields=1",
      DESCRIPTOR: "aura://RecordUiController/ACTION$getRecordWithFields"
    },
    SAVE_CONTENT_VER: {
      QUERY: "ui-chatter-components-aura-components-forceChatter-chatter.PublisherFileAttachment.saveContentVersion",
      DESCRIPTOR: "serviceComponent://ui.chatter.components.aura.components.forceChatter.chatter.PublisherFileAttachmentController/ACTION$saveContentVersion"
    },
    FILTER_DATA_PROVIDER_GET_ITEMS: {
      QUERY: "ui-content-components-forceContent-fileDataProvider.FileDataProvider.getItems",
      DESCRIPTOR: "serviceComponent://ui.content.components.forceContent.fileDataProvider.FileDataProviderController/ACTION$getItems"
    },
    SHARE_FILE_WITH_RECORD: {
      QUERY: "ui-content-components-forceContent-relatedListFileSelect.RelatedListFileSelect.shareFilesWithRecord",
      DESCRIPTOR: "serviceComponent://ui.content.components.forceContent.relatedListFileSelect.RelatedListFileSelectController/ACTION$shareFilesWithRecord"
    },
    DELETE_RECORD: {
      QUERY: "ui-force-components-controllers-recordGlobalValueProvider.RecordGvp.deleteRecord",
      DESCRIPTOR: "serviceComponent://ui.force.components.controllers.recordGlobalValueProvider.RecordGvpController/ACTION$deleteRecord"
    }
  }, SUPPORTED_QUERIES = Object.keys(SUPPORTED_REQUESTS).map((request => [ SUPPORTED_REQUESTS[request].QUERY, request ])), requestsMap = new Map;
  const startedAt = new Map;
  function mapRecordToMetadata(record, addSfPrefix = !1) {
    let metadata;
    switch (record.apiName) {
     case "Opportunity":
      metadata = {
        opportunity_id: record.id,
        opportunity_name: record.fields.Name?.value,
        account_id: record.fields.Account?.value.id,
        account_name: record.fields.Account?.value.fields.Name?.value
      };
      break;
     ...
     case "Account":
      metadata = {
        ...
        account_name: record.fields.Name?.value
      };
      break;

     case "Case":
      {
        const account = record.fields.Account;
        metadata = {
          case_id: record.fields.CaseNumber?.value,
          case_name: record.fields.Subject?.value,
          account_id: account?.value?.id || record.fields.AccountId?.value,
          account_name: account?.value?.fields.Name?.value || account?.displayValue
        };
        break;
      }

     default:
      throw new Error("Invalid sf record type");
    }
    if (addSfPrefix) {
      const result = {};
      for (const [key, value] of Object.entries(metadata)) result[`salesforce_${key}`] = value;
      return result;
    }
    return metadata;
  }
  function getCurrentRecordMetadata(addSfPrefix = !0) {
    const record = /\/(?<name>Opportunity|Account|Case)\/(?<id>.*?)\//.exec(location.pathname)?.groups;
    if (record) {
      try {
        const cachedRecord = getCacheItemByType(record.name, record.id);
        return invariant(cachedRecord, `missing ${record.name} record data in cache`), mapRecordToMetadata(cachedRecord, addSfPrefix);
      } catch {}
      return {};
    }
  }
  function extendRecord(map, record) {
  ...
  function cacheRecord(record) {
    switch (record.apiName) {
     case "Account":
      extendRecord(xhr_web_cache.accounts, record);
      break;

     case "Opportunity":
      extendRecord(xhr_web_cache.opportunities, record);
      break;

     case "Case":
      extendRecord(xhr_web_cache.cases, record);
    }
  }
  function stripSalesfoceKeys(object) {
    if (!object) return object;
    const result = {};
    for (const [key, value] of Object.entries(object)) {
      result[key.startsWith("salesforce_") ? key.slice(11) : key] = value;
    }
    return result;
  }
  function getCacheItemByType(recordType, recordId) {
    switch (recordType) {
     case "Account":
      return xhr_web_cache.accounts.get(recordId);

     case "Opportunity":
      return xhr_web_cache.opportunities.get(recordId);

     case "Case":
      return xhr_web_cache.cases.get(recordId);
  ...
  function getCache() {
    return globalThis.CH_DATA || (globalThis.CH_DATA = {
      opportunities: new Map,
      accounts: new Map,
      cases: new Map,
      reports: new Map,
      contentDocuments: new Map
    }, new Map), globalThis.CH_DATA;
  }
  interceptor.on("request", (async function({request, requestId}) {
    try {
      if (startedAt.set(requestId, Date.now()), !function(request) {
        return request.url.includes("/aura") || "POST" !== request.method;
      }(request)) return;
      for (const [query, key] of SUPPORTED_QUERIES) if (request.url.includes(query)) {
        const requestActions = actionsSchema.safeParse(await request.clone().text());
        if (!requestActions.success) return;
        const action = requestActions.data.find((action => action.descriptor === SUPPORTED_REQUESTS[key].DESCRIPTOR));
        action && requestsMap.set(requestId, action);
      }
    } catch (error) {
      logError({
        error: "error occured in salesforce xhr handle request ",
        errorMessage: getErrorMessage(error)
      });
    }
  })), interceptor.on("response", (async function({requestId, request, response}) {
    try {
      if (request.url.includes("/chatter/handlers/chatterfile")) return void async function(requestId, request, response) {
        const formData = await request.clone().formData();
        if (formData.has("file")) {
          const file = formData.get("file"), event = await getRecentUpload({
            name: file.name
          }, startedAt.get(requestId)), docId = await async function(response) {
            const textAreaContent = await response.clone().text(), jsonStartIndex = textAreaContent.indexOf("{"), jsonEndIndex = textAreaContent.lastIndexOf("}") + 1, jsonString = textAreaContent.slice(jsonStartIndex, jsonEndIndex);
            try {
              return JSON.parse(jsonString).docid;
            } catch {
              return "";
            }
          }(response);
          dispatchOnWindow("create_record", {
            id: docId,
            name: file.name,
      ...
      const action = requestsMap.get(requestId);
      requestsMap.delete(requestId);
      const responseJson = await response.clone().json(), responseActions = responseActionsSchema.safeParse(responseJson);
      if (!responseActions.success) return;
      const resAction = responseActions.data.actions.find((resAction => resAction.id === action.id));
      if ("SUCCESS" !== resAction?.state) return;
      switch (action.descriptor) {
       case SUPPORTED_REQUESTS.GET_RECORD.DESCRIPTOR:
        cacheRecord(resAction.returnValue);
        break;

       case SUPPORTED_REQUESTS.LIST_OBJECTS.DESCRIPTOR:
        {
          const apiName = action.params.entityName;
          if (!SUPPORTED_DELETE_RECORDS.includes(apiName)) return;
          const recordValues = responseActions.data.context?.globalValueProviders?.find((provider => "$Record" === provider.type));
          if (!recordValues) return;
          for (const [_, value] of Object.entries(recordValues?.values?.records)) {
            const recordData = value?.[apiName];
            recordData && cacheRecord(recordData.record);
          }
          break;
        }

       case SUPPORTED_REQUESTS.RELATED_LIST_OBJECTS.DESCRIPTOR:
        {
          const records = resAction.returnValue?.records;
          if (!records) return;
          for (const record of records) cacheRecord(record);
          break;
        }

       case SUPPORTED_REQUESTS.DESCRIBE_REPORT.DESCRIPTOR:
        {
          const {reportId, reportName, type} = resAction.returnValue.attributes;
          xhr_web_cache.reports.set(reportId, {
            Id: reportId,
            Name: reportName,
            sobjectType: type
          });
       ...
       case SUPPORTED_REQUESTS.LIST_REPORTS.DESCRIPTOR:
        {
          const {result} = resAction.returnValue;
          for (const report of result) xhr_web_cache.reports.set(report.Id, report);
          break;
        }

       case SUPPORTED_REQUESTS.SAVE_CONTENT_VER.DESCRIPTOR:
        {
          const id = resAction?.returnValue.docid;
          if (!id) return;
          const saveContentRequestParams = saveContentActionParams.safeParse(action.params);
          if (!saveContentRequestParams.success) return;
          const {pathOnClient: title} = saveContentRequestParams.data, event = await getRecentUpload({
            name: title
          }, startedAt.get(requestId)), extras = stripSalesfoceKeys(getCurrentRecordMetadata());
          dispatchOnWindow("create_record", {
            id,
            name: title,
            uploadId: event?.upload_id,
       ...
       case SUPPORTED_REQUESTS.FILTER_DATA_PROVIDER_GET_ITEMS.DESCRIPTOR:
        {
          const files = resAction.returnValue.result;
          for (const file of files) {
            const record = file.record;
            getCache().contentDocuments.set(record.Id, record);
          }
          break;
        }

       case SUPPORTED_REQUESTS.SHARE_FILE_WITH_RECORD.DESCRIPTOR:
        {
          const files = action.params.docIds.map((docId => xhr_web_cache.contentDocuments.get(docId))).filter((document => resAction.returnValue[document.Id].shareSuccess));
          if (!files.length) return;
          const metadata = stripSalesfoceKeys(getCurrentRecordMetadata());
          metadata && dispatchOnWindow("attach_record", {
            files: files.map((file => ({
              id: file.Id,
              title: file.LatestPublishedVersion.PathOnClient
            }))),
       ...
       case SUPPORTED_REQUESTS.DELETE_RECORD.DESCRIPTOR:
        {
          const [recordId, recordType] = resAction.returnValue, cacheItem = getCacheItemByType(recordType, recordId);
          if (!cacheItem) return;
          const record = mapRecordToMetadata(cacheItem);
          dispatchOnWindow("remove_record", {
            record: {
              ...record,
              ...getCurrentRecordMetadata(!1)
            }
          });
          break;
        }
      ...
      logError({
        error: "error occured in salesforce xhr handle response",
        errorMessage: getErrorMessage(error)
      });
    }
  })), interceptor.on("unhandledException", (({requestId}) => {
    requestsMap.delete(requestId);
  })), abort_controller_onCleanup((() => {
    interceptor.dispose();
  })), listenPageEvent("Cyberhaven_GetMetadata", (() => getCurrentRecordMetadata(!0))), 
  listenPageEvent("Cyberhaven_GetReportMetadata", (function(event) {
    const reportId = event.detail.reportId;
    if (reportId && xhr_web_cache.reports.has(reportId)) {
      const report = xhr_web_cache.reports.get(reportId);
      if (report) return {
        salesforce_report_id: report.Id,
        salesforce_report_name: report.Name
      };
    } else logError({
      error: "missing report metadata",
      reportId,
      availableKeys: Array.from(xhr_web_cache.reports.keys())
    });
    return {};
  }));
})();