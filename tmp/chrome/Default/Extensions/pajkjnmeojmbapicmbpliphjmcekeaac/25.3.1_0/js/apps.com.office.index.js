(() => {
  ...
  const globals_isWebWorkerContext = () => "function" == typeof importScripts && !getContextName().includes("background");
  globals_isWebWorkerContext() ? self : window;
  var util, objectUtil;
  !function(util) {
  ...
  }), window.self === window.top && service_communication_onMessage("get-page-data", measureTimeExecution("com.office:get-page-data", (() => {
    const metadata = {
      email: getAccountEmail(),
      account_type: "1" === new URL(location.href).searchParams.get("auth") ? "onedrive" : "sharepoint"
    };
    return pageData = {
      cloud_provider: "office_365",
      default_object_type: "cloud_file_onedrive",
      page_app: "office_home",
      metadata
    }, {
      url: location.href,
