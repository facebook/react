 ...
 * @format
 * @oncall ws_tools
 * @providesLegacy CometDevToolsPopupRootImpl
 */__d("legacy:CometDevToolsPopupRootImpl",[],(function $module_legacy_CometDevToolsPopupRootImpl(global,require,requireDynamic,requireLazy,__DO_NOT_USE__module,__DO_NOT_USE__exports){
...
if(form===null){
throw new Error('No advanced settings form.');
}

var sourceRadioButtons=form.elements.hostname_source;

if(sourceRadioButtons===null){
throw new Error('No radio buttons.');
}

var sourceTextInput=form.elements.manual_hostname;

if(sourceTextInput===null){
throw new Error('No text input for manual hostname source.');
}


sourceTextInput.oninput=function(){
sourceRadioButtons.value='manual';
};

var statusMessage=document.getElementById('status_message');

if(statusMessage===null){
throw new Error('No status message for advanced settings form.');
}

var manualServerSaved=null;
var currentTabId=null;
storage.get(
{

hostnameSource:'production',
manualServer:'www.my-od.internalfb.com'},


function storage_get_$1(result){var
hostnameSource=result.hostnameSource,manualServer=result.manualServer;
manualServerSaved=manualServer;

sourceRadioButtons.value=hostnameSource;
sourceTextInput.value=manualServer;

renderDebugLink();

form.onsubmit=function(_event){
var updatedManualServer=sourceTextInput.value.
...
storage.set(
{
hostnameSource:sourceRadioButtons.value,
manualServer:updatedManualServer},

function storage_set_$1(){
manualServerSaved=updatedManualServer;
...
var tier1button=window.document.getElementById('tier_1_debug');
if(tier1button==null){
throw new Error('No tier1 button for tier1 debugging.');
}

var tierDebuggingTable=window.document.getElementById('tier_debugging');
if(tier1button==null){
throw new Error('No tier debugging section.');
}

function truty(str){
return str!=null&&(str==null?void 0:str.length)>0&&str!=='0'&&str!=='false';
}

function renderDebugLink(){var _currentTabId,_currentTabId2;
var debugLink=window.document.getElementById('debug_tab');
if(debugLink==null){
throw new Error('No debug_tab');
}

debugLink.href="https://"+(
manualServerSaved||'www.internalfb.com')+"/intern/comet_dev_tools/main?tab_id="+((_currentTabId=
currentTabId)!=null?_currentTabId:0);
debugLink.innerText="Open tools in browser tab ("+((_currentTabId2=currentTabId)!=null?_currentTabId2:0)+")";
}

tabs.query({active:true,lastFocusedWindow:true},function tabs_query_$1(tabs){var _url$searchParams$get;
var urlStr=tabs[0].url;
currentTabId=tabs[0].id;
if(!urlStr){
console.error('No url for active tab!');
return;
}

var url=new URL(urlStr);
var debugging=
truty(url.searchParams.get('gk_debug'))&&
/comet_delay_tier_one_debug/.test((_url$searchParams$get=url.searchParams.get('gk_enable'))!=null?_url$searchParams$get:'');

if(debugging){

tierDebuggingTable.style.display='inherit';
}

renderDebugLink();
});

tier1button.addEventListener('click',function tier1button_addEventListener_$1(){
tabs.executeScript(null,{
code:"let s=document.createElement('script');s.innerHTML='window.__continueT1();';document.head.appendChild(s);"});
 ...
 * @format
 * @oncall ws_tools
 * @providesMeta CometDevToolsPopupRoot
 * @requires CometDevToolsPopupRootImpl
