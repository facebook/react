 ...
 * @format
 * @oncall ws_tools
 */__d("CometDevToolsBackgroundScriptFlipperConnection",[],(function $module_CometDevToolsBackgroundScriptFlipperConnection(global,require,requireDynamic,requireLazy,module,exports){
...
function CometDevToolsBackgroundScriptFlipperConnection(){
var ws;

var pagePorts=new Map();


var deviceId="webbroswer."+Math.random();

function connectPortToFlipper(port){
if(!ws||ws.readyState!==ws.OPEN){
return;
}var _port$name$split=
port.name.split(':'),_prefix=_port$name$split[0],app=_port$name$split[1],plugins=_port$name$split[2];
console.log("Connected to web page: "+app);
...
port.onMessage.addListener(function port_onMessage_addListener_$0(message){
if(
...
var wsMessage={app:app,type:'send',payload:message.payload};
if(!ws||ws.readyState!==ws.OPEN){
return;
}
...
port.onDisconnect.addListener(function port_onDisconnect_addListener_$0(){
pagePorts["delete"](app);
console.log("Disconnected from web page: "+app);
var disconnectMessage={app:app,type:'disconnect'};
if(!ws||ws.readyState!==ws.OPEN){
return;
}
ws.send(JSON.stringify(disconnectMessage));
});
port.postMessage({method:'onConnect'});
}

function connectToFlipper(){
try{
ws=new WebSocket("ws://localhost:8333?deviceId="+deviceId);
ws.onopen=function(){
if(!ws){
return;
}
...
Array.from(pagePorts.values()).map(
function map_$0(port){return port&&connectPortToFlipper(port);});



ws.onclose=function(){
console.log('Disconnected from Flipper');
Array.from(pagePorts.values()).map(function map_$0(port){return(
port.postMessage({method:'onDisconnect'}));});
...
function reconnect(){
ws=null;
setTimeout(connectToFlipper,1000);
}


connectToFlipper();

window.chrome.runtime.onConnect.addListener(function runtime_onConnect_addListener_$0(port){
if(port.name.startsWith('flipper:')){var _port$name$split2=
port.name.split(':'),_prefix=_port$name$split2[0],app=_port$name$split2[1],_plugins=_port$name$split2[2];
pagePorts.set(app,port);
connectPortToFlipper(port);
}
});
}_c=CometDevToolsBackgroundScriptFlipperConnection;

module.exports=CometDevToolsBackgroundScriptFlipperConnection;var _c;$RefreshReg$(_c,"CometDevToolsBackgroundScriptFlipperConnection");}),null);
/**
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 *  strict-local
 * @format
 * @oncall ws_tools
 */__d("TwoWayMap",[],(function $module_TwoWayMap(global,require,requireDynamic,requireLazy,module,exports){

'use strict';var

TwoWayMap=function(){function TwoWayMap(){this.
$TwoWayMap_map={};this.
$TwoWayMap_inv={};}var _proto=TwoWayMap.prototype;_proto.

set=function set(key,value){
this.$TwoWayMap_map[key]=value;
this.$TwoWayMap_inv[value]=key;
};_proto.

get=function get(key){
var value=this.$TwoWayMap_map[key];
if(value==null){
return null;
}else{
return value;
...
getInverse=function getInverse(value){
var key=this.$TwoWayMap_inv[value];
if(key==null){
return null;
}else{
return key;
...
delete this.$TwoWayMap_map[key];
delete this.$TwoWayMap_inv[value];
}
};_proto.

removeInverse=function removeInverse(value){
var key=this.$TwoWayMap_inv[value];
if(key==null){
return;
}else{
...
module.exports=TwoWayMap;}),null);
/**
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 * Handles connection state for CometDevToolsBackgroundScriptImpl
 *
 *  strict-local
 * @format
 * @oncall ws_tools
 */__d("CometDevToolsBackgroundScriptRegistry",["TwoWayMap"],(function $module_CometDevToolsBackgroundScriptRegistry(global,require,requireDynamic,requireLazy,module,exports){
...
var __DEV__=window.__DEV__;var





CometDevToolsBackgroundScriptRegistry=function(){function CometDevToolsBackgroundScriptRegistry(){this.

panelFrameIdForPageTabId=new(require("TwoWayMap"))();this.



portForPageTabId={};this.
portForPanelFrameId={};this.

onNewConnection=function(){};this.


nextFrameId=1;}var _proto=CometDevToolsBackgroundScriptRegistry.prototype;_proto.



registerContentScriptPort=function registerContentScriptPort(tabId,port){
this.portForPageTabId[tabId]=port;
if(__DEV__){
console.log('registerContentScriptPort:',tabId);
}
};_proto.

registerPanelPort=function registerPanelPort(port){
var frameId=this.nextFrameId;
this.nextFrameId+=1;
this.portForPanelFrameId[frameId]=port;
if(__DEV__){
console.log('registerPanelPort:',frameId);
}
return frameId;
};_proto.

unregisterContentScriptPort=function unregisterContentScriptPort(tabId){
delete this.portForPageTabId[tabId];
};_proto.

unregisterPanelPort=function unregisterPanelPort(frameId){
delete this.portForPanelFrameId[frameId];
};_proto.

getPortForPanelFrameId=function getPortForPanelFrameId(frameId){
return this.portForPanelFrameId[frameId];
};_proto.

getPortForPageTabId=function getPortForPageTabId(tabId){
return this.portForPageTabId[tabId];
};_proto.



setPanelFrameIdForPageTabId=function setPanelFrameIdForPageTabId(tabId,frameId){
this.panelFrameIdForPageTabId.set(tabId,frameId);
if(__DEV__){
console.log('setPanelFrameIdForPageTabId:',tabId,frameId);
}




if(this.portForPageTabId[tabId]){
this.onNewConnection(tabId,frameId);
}
};_proto.

pageTabIdIsReadyToConnect=function pageTabIdIsReadyToConnect(tabId){



var maybeFrameId=this.getPanelFrameIdForPageTabId(tabId);
if(maybeFrameId!=null){

this.onNewConnection(tabId,maybeFrameId);
}
};_proto.

getPanelFrameIdForPageTabId=function getPanelFrameIdForPageTabId(tabId){
return this.panelFrameIdForPageTabId.get(tabId);
};_proto.

getPageTabIdForPanelFrameId=function getPageTabIdForPanelFrameId(frameId){
return this.panelFrameIdForPageTabId.getInverse(frameId);
};_proto.

removePanelFrameIdForPageTabId=function removePanelFrameIdForPageTabId(tabId){
this.panelFrameIdForPageTabId.remove(tabId);
};return CometDevToolsBackgroundScriptRegistry;}();


module.exports=CometDevToolsBackgroundScriptRegistry;}),null);
/**
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 *  strict
 ...
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 * then/promise version: 7.0.3
 *
 * NOTE: this module contains local modifications:
 * - Promise.all
 * - Promise.allSettled
 * - Named Fullfill function for better profiling.
 *
 * @format
 * @oncall comet_infra
 * @typechecks
 */__d("Promise",["cr:gk.use_native_promise_impl_mmZuCJXhZef"],(function $module_Promise(global,require,requireDynamic,requireLazy,module,exports){
...
require("cr:gk.use_native_promise_impl_mmZuCJXhZef"))!=null?_require:global.Promise;
 ...
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 * Captures browser tab traces as part of the FB Browser Tools.
 * Note: the Promise constructor is referred to by global.Promise in this file
 * intentionally to avoid the Promise polyfill being pulled into this bundle as
 * it depends on a requireCond which breaks everything.
 *
 *  strict-local
 * @format
 * @oncall ws_tools
 */__d("CometDevToolsBackgroundScriptTracing",[],(function $module_CometDevToolsBackgroundScriptTracing(global,require,requireDynamic,requireLazy,module,exports){
...
var msg=chrome.runtime.lastError.message||'Unknown error';
...
var DBG_PROTOCOL_VER='1.1';
var DBG_PROTOCOL_START_TRACING='Tracing.start';
var DBG_PROTOCOL_STOP_TRACING='Tracing.end';
var DBG_PROTOCOL_COMPLETED_TRACING='Tracing.tracingComplete';
var DBG_PROTOCOL_READ='IO.read';
var DBG_PROTOCOL_READ_SIZE=4*1024*1024;
var DBG_PROTOCOL_RETURN_AS_STREAM='ReturnAsStream';



var tracesOngoing=new Map();




var traceDataListeners=new Map();




var shouldProfileTab=new Set();


function setAutoProfilingForTab(tabID,enabled){
if(enabled){
shouldProfileTab.add(tabID);
}else{
shouldProfileTab["delete"](tabID);
}
}
...
interactionID)
{
if(shouldProfileTab.has(debugee.tabId)){var _tracesOngoing$get;
var tabInteractions=(_tracesOngoing$get=tracesOngoing.get(debugee.tabId))!=null?_tracesOngoing$get:new Set();
tracesOngoing.set(debugee.tabId,tabInteractions);
var shouldStartTrace=tabInteractions.size===0;
tabInteractions.add(interactionID);
if(shouldStartTrace){

return captureTraceForTab(debugee).then(function then_$0(){
return true;
});
}
}
...
interactionID)
{
var tabInteractions=tracesOngoing.get(debugee.tabId);
if(tabInteractions&&tabInteractions.has(interactionID)){
var shouldStopTrace=tabInteractions.size===1;
tabInteractions["delete"](interactionID);
if(shouldStopTrace){

return endTrace(debugee);
}
}
return global.Promise.resolve();
}

function endOfTraceCleanup(
debugee,
options)
{
if(tracesOngoing.has(debugee.tabId)){


var listeners=traceDataListeners.get(debugee.tabId);
if(listeners){
var err=new Error(
'trace cancelled without completing while logging data');

listeners.forEach(function listeners_forEach_$0(listener){return listener.reject(err);});
}
traceDataListeners["delete"](debugee.tabId);

tracesOngoing["delete"](debugee.tabId);
if(options.alsoDetachDebugger){
chrome["debugger"].detach(debugee);
}
}
}

function endTrace(debugee){
...
var listeners=traceDataListeners.get(debugee.tabId)||[];
traceDataListeners.set(debugee.tabId,listeners);
listeners.push({resolve:resolve,reject:reject});


chrome["debugger"].sendCommand(
debugee,
DBG_PROTOCOL_STOP_TRACING,
function sendCommand_$2(){
if(checkHasErrors()){



endOfTraceCleanup(debugee,{alsoDetachDebugger:true});
}
});
...
reject(
new Error(
'called endTrace but no trace ongoing for tabId: '+debugee.tabId));
...
function startTracing(debugee,categories){
return new global.Promise(function(resolve,reject){
var params={categories:categories,transferMode:DBG_PROTOCOL_RETURN_AS_STREAM};
chrome["debugger"].sendCommand(
debugee,
DBG_PROTOCOL_START_TRACING,
params,
function tracingStarted(){
if(checkHasErrors()){
reject(new Error("Can't start tracing"));
return;
}


setTimeout(function tracingTimeout(){
if(tracesOngoing.has(debugee.tabId)){
console.error('tracing timed out',debugee.tabId);

endTrace(debugee)["catch"](function $0(e){


console.error('endTrace error',e);
});
}
},120000);
...
if(checkHasErrors()){
reject(new Error('startTracing failed'));
return;
}
});
}

var TRACING_CATEGORIES=[
...
join(',');
function captureTraceForTab(debugee){

return new global.Promise(function(resolve,reject){
chrome["debugger"].attach(debugee,DBG_PROTOCOL_VER,function result(){


if(checkHasErrors()){
reject(new Error("Can't attach debugger"));
return;
}

resolve(startTracing(debugee,TRACING_CATEGORIES));
});
});
}

function readTraceLoop(
debugee,
...
data)
{if(data===void 0){data=[];}
var args={handle:stream,size:DBG_PROTOCOL_READ_SIZE};
chrome["debugger"].sendCommand(
debugee,
DBG_PROTOCOL_READ,
args,
function trace_read(params){
if(checkHasErrors()){
reject(new Error('readTrace failed'));
return;
}
...
if(params.eof){
var profile=data.join('');
resolve(profile);
}else{

setTimeout(function setTimeout_$0(){
readTraceLoop(debugee,stream,resolve,reject,data);
},0);
}
});
...
function readTrace(debugee,stream,data){if(data===void 0){data=[];}
return new global.Promise(function(resolve,reject){
readTraceLoop(debugee,stream,resolve,reject,data);
});
}
...
function debuggerOnEvent(debugee,method,params){
if(method===DBG_PROTOCOL_COMPLETED_TRACING){
readTrace(debugee,params.stream).
then(function then_$0(data){
var listeners=traceDataListeners.get(debugee.tabId);
if(listeners){
listeners.forEach(function listeners_forEach_$0(listener){return listener.resolve(data);});
}
traceDataListeners["delete"](debugee.tabId);
})["catch"](
function $0(err){
console.error('readTrace failed',debugee,err);
})["finally"](
function $0(){
endOfTraceCleanup(debugee,{alsoDetachDebugger:true});
});
...
chrome["debugger"].onDetach.addListener(function debuggerOnDetach(
debugee,
reason)
{
console.error('forcibly detached from debugger',debugee,reason);
endOfTraceCleanup(debugee,{alsoDetachDebugger:false});
});

chrome.tabs.onUpdated.addListener(function tabs_onUpdated_addListener_$0(tabId,changeInfo){
if(changeInfo.status=='loading'){
var interactionIDs=tracesOngoing.get(tabId);
if(interactionIDs&&!interactionIDs.has('manual')){
endOfTraceCleanup({tabId:tabId},{alsoDetachDebugger:false});
}
}
});exports.setAutoProfilingForTab=setAutoProfilingForTab;exports.startInteraction=startInteraction;exports.endInteraction=endInteraction;}),66);
/**
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *
 *  strict
 ...
 * @format
 * @oncall ws_tools
 * @providesLegacy CometDevToolsBackgroundScriptImpl
 */__d("legacy:CometDevToolsBackgroundScriptImpl",["CometDevToolsBackgroundScriptFlipperConnection","CometDevToolsBackgroundScriptRegistry","CometDevToolsBackgroundScriptTracing","browser-tools-common"],(function $module_legacy_CometDevToolsBackgroundScriptImpl(global,require,requireDynamic,requireLazy,__DO_NOT_USE__module,__DO_NOT_USE__exports){
...
var tabs=window.chrome?window.chrome.tabs:window.browser.tabs;
...
window.__DEV__=__DEV__;var





BrowserToolsMessageValidation=require("browser-tools-common").BrowserToolsMessageValidation;var
isValidMessage=BrowserToolsMessageValidation.isValidMessage;

var CONTENT_SCRIPT='content_script';
var PANEL='panel';

var registry=new(require("CometDevToolsBackgroundScriptRegistry"))();
registry.onNewConnection=function(tabId,frameId){
var maybeTabPort=registry.getPortForPageTabId(tabId);
var maybeFramePort=registry.getPortForPanelFrameId(frameId);

if(maybeTabPort==null){
console.error("No port for tabId "+tabId+" on new connection callback");
return;
}

if(maybeFramePort==null){
console.error("No port for frameId "+frameId+" on new connection callback");
return;
}

if(__DEV__){
console.log("Sending 'linked' messages.");
}

maybeTabPort.postMessage({action:'linked'});
maybeFramePort.postMessage({action:'linked'});
};

function onMessageFromContentScript(message,port){
if(!isValidMessage(message)){
console.error('Malformed message from contentScript',message);
return;
}

var maybeTabId=getTabId(port);
if(maybeTabId==null){
console.error("ContentScript's port has an invalid tabId:",maybeTabId);
return;
}

var maybeFrameId=registry.getPanelFrameIdForPageTabId(maybeTabId);
if(maybeFrameId==null&&message.action!=='register'){
...
'Tab',
maybeTabId,
"isn't being inspected by any registered panel");

return;
}

var tabId=maybeTabId;
var frameId=maybeFrameId;



if(__DEV__){
if(frameId==null){
console.log("("+tabId+")- bg",message);
}else{
console.log("("+tabId+")- bg ->("+frameId+")",message.namespace,message);
}
}

var destination=
frameId!=null?registry.getPortForPanelFrameId(frameId):null;

switch(message.action){
case'register':
registry.pageTabIdIsReadyToConnect(tabId);
break;

case'ready':
if(!Array.isArray(message.body)){
console.error('Malformed ready message',message);
return;
}

if(!destination){
console.error(
"consistency error in ready, we don't have a port for this panel!");

return;
}
destination.postMessage({
action:'ready',
body:message.body.concat(['background'])});

return;

case'profile':
handleProfilingMessage(message,tabId,frameId);
break;
...
if(!destination){
console.error(
"consistency error in relay, we don't have a port for this panel!");
...
function onMessageFromPanel(message,port){
if(!isValidMessage(message)){
console.error('Malformed message from panel',message);
return;
}

var maybeDest=message.dest;
if(maybeDest==null||typeof maybeDest!=='number'){
console.error('Malformed message from panel; no destination.',message);
return;
}

var maybeFrameId=getFrameId(message);
if(maybeFrameId==null){
console.error("Panel's port has an invalid frameId:",maybeFrameId);
return;
}
...
var destination=registry.getPortForPageTabId(tabId);

switch(message.action){
case'script':
tabs.executeScript(tabId,{code:content},function tabs_executeScript_$2(result){
var error=window.chrome.runtime.lastError;
if(error!==undefined){
console.error('Error on executeScript:',tabId,result,error);
}
});
return;
...
case'script_file':
tabs.executeScript(tabId,{file:content},function tabs_executeScript_$2(result){
var error=window.chrome.runtime.lastError;
if(error!==undefined){
console.error(
'Error on executeScript:',
tabId,
result,
error.message);

console.error('after message:',message);
}
});
return;
...
registry.setPanelFrameIdForPageTabId(tabId,frameId);
...
console.error(
'Got navigate message without valid args in backround script.');
...
console.log('Got navigate message without valid url');
return;
}
...
console.error(
'registry error:',
'got a message for tabId',
tabId,
"we don't have a port for this tab!",
message);
...
if(!sender){
console.error('port has no sender (need tabid)',port);
return;
}

var tab=sender.tab;
if(!tab){
console.error('port has no tab (need tabid)',sender);
return;
}

var id=tab.id;
if(id===null){
console.error('port has no tab id',tab);
return;
}
...
function getFrameId(message){
var frameId=message.src;
if(typeof frameId!=='number'){
console.error("message didn't inculde a frameid",message);
return;
}
...
function registerContentScriptPort(port){
var tabId=getTabId(port);
if(tabId==null){
console.error('No tabId => ContentScript port failed to register.');
return;
}
registry.registerContentScriptPort(tabId,port);
port.onDisconnect.addListener(function port_onDisconnect_addListener_$0(port){



registry.unregisterContentScriptPort(tabId);
unregisterContentScriptPort(port);
});
}

function registerPanelPort(port){
var frameId=registry.registerPanelPort(port);
port.onDisconnect.addListener(function port_onDisconnect_addListener_$0(port){
unregisterPanelPort(frameId);
});
return frameId;
}

function unregisterContentScriptPort(port){
var tabId=getTabId(port);
if(tabId==null){
console.error('Content Script port has no tabid to unregister');
return;
}

var frameId=registry.getPanelFrameIdForPageTabId(tabId);
if(frameId==null){
console.error('No tabId to receive unlink event');
return;
}

var destination=registry.getPortForPanelFrameId(frameId);
if(destination==null){
console.error('No port for frameId recieving unlink event');
return;
}


destination.postMessage({action:'unlinked'});
if(__DEV__){
console.log("bg ->("+frameId+")",{action:'unlinked'});
}
...
function unregisterPanelPort(frameId){
var tabId=registry.getPageTabIdForPanelFrameId(frameId);
if(tabId==null){
console.error('No tabId to receive unlink event');
return;
}
...
var destination=registry.getPortForPageTabId(tabId);
if(destination==null){
console.error('No port for tabId recieving unlink event');
return;
}
...
destination.postMessage({action:'unlinked'});
if(__DEV__){
console.log("("+tabId+")<- bg",{action:'unlinked'});
}

registry.unregisterPanelPort(frameId);
registry.removePanelFrameIdForPageTabId(tabId);
}

function handleProfilingMessage(
message,
tabId,
frameId)
{
...
typeof message.body.interactionID==='string'))

{
console.error('invalid profile.start message');
return;
}
require("CometDevToolsBackgroundScriptTracing").startInteraction(
{tabId:tabId},
message.body.interactionID).

then(function then_$0(started){
if(started&&frameId!=null){
var panel=registry.getPortForPanelFrameId(frameId);
if(panel){
panel.postMessage({
action:'relay',
body:{tracing:true},
...
function $0(err){
console.error('starting profile failed',err);
});
break;
}
case'stop':{
...
{
console.error('invalid profile.stop message');
return;
}
require("CometDevToolsBackgroundScriptTracing").endInteraction(
{tabId:tabId},
message.body.interactionID).
...
function $0(err){
console.error('stopping profile failed',err);
});
break;
}
case'auto_profile':{
...
message.body!=null&&
typeof message.body==='object'&&
typeof message.body.enabled==='boolean'))

{
console.error('invalid profile.auto_profile message');
return;
}
require("CometDevToolsBackgroundScriptTracing").setAutoProfilingForTab(
tabId,
...
default:
console.error('unknown profiling message type',message.kind);
break;}
...
runtime.onConnect.addListener(function runtime_onConnect_addListener_$0(port){




if(port.name===CONTENT_SCRIPT){
registerContentScriptPort(port);
port.postMessage({action:'tab_id',tabId:getTabId(port)});
port.onMessage.addListener(onMessageFromContentScript);
port.onDisconnect.addListener(function port_onDisconnect_addListener_$0(port){
port.onMessage.removeListener(onMessageFromContentScript);
});
}
...
if(port.name===PANEL){
var frameId=registerPanelPort(port);
port.postMessage({action:'you_are',body:frameId});
port.onMessage.addListener(onMessageFromPanel);
port.onDisconnect.addListener(function port_onDisconnect_addListener_$0(port){
port.onMessage.removeListener(onMessageFromPanel);
});
}
});

require("CometDevToolsBackgroundScriptFlipperConnection")();}),3);
/**
 * (c) Meta Platforms, Inc. and affiliates. Confidential and proprietary.
 *  strict-local
 * @format
 * @oncall ws_tools
 * @providesMeta CometDevToolsBackgroundScript
 * @requires CometDevToolsBackgroundScriptImpl
