...
describe('ReactComponentTreeDevtoolDev', () => {
  var React;
  var ReactDOM;
  var ReactDOMServer;
  ...
  beforeEach(() => {
    ...
    ReactComponentTreeDevtoolDev = require('ReactComponentTreeDevtoolDev');
  ...
  function assertTreeMatches(pairs) {
    ...
    pairs.forEach(([element, expectedTree]) => {
      ...
      // because ReactDebugToolDev purges it when the flush ends.
      ReactDOMServer.renderToString(<Wrapper />);
      expectWrapperTreeToEqual(null);

      // To test it, we tell the devtool to ignore next purge
      // so the cleanup request by ReactDebugToolDev is ignored.
      // This lets us make assertions on the actual tree.
      ReactComponentTreeDevtoolDev._preventPurging = true;
      ReactDOMServer.renderToString(<Wrapper />);
      ReactComponentTreeDevtoolDev._preventPurging = false;
      expectWrapperTreeToEqual(expectedTree);
  ...
  it('reports update counts', () => {
    ...
    var divID = ReactComponentTreeDevtoolDev.getRootIDs()[0];
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(divID)).toEqual(0);

    ReactDOM.render(<span className="a" />, node);
    var spanID = ReactComponentTreeDevtoolDev.getRootIDs()[0];
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(divID)).toEqual(0);
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(spanID)).toEqual(0);

    ReactDOM.render(<span className="b" />, node);
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(divID)).toEqual(0);
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(spanID)).toEqual(1);

    ReactDOM.render(<span className="c" />, node);
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(divID)).toEqual(0);
    expect(ReactComponentTreeDevtoolDev.getUpdateCount(spanID)).toEqual(2);
  ...
  describe('stack addenda', () => {
    it('gets created', () => {
      function getAddendum(element) {
        var addendum = ReactComponentTreeDevtoolDev.getCurrentStackAddendum(element);
    ...
    it('can be retrieved by ID', () => {
      function getAddendum(id) {
        var addendum = ReactComponentTreeDevtoolDev.getStackAddendumByID(id);
        return addendum.replace(/\(at .+?:\d+\)/g, '(at **)');
      }
