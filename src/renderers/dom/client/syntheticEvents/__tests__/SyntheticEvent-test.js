...
describe('SyntheticEvent', function() {
  ...
  it('should be nullified if the synthetic event has called destructor and log warnings', function() {
    ...
    expect(console.error.calls.count()).toBe(3); // once for each property accessed
    expect(console.error.calls.argsFor(0)[0]).toBe( // assert the first warning for accessing `type`
