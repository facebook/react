...
if (__DEV__) {
  ...
  var isWrappedNativeComponent = function(instance) {
    return (
      instance._renderedComponent &&
      instance._renderedComponent.hasOwnProperty('_renderedChildren') &&
      instance._currentElement &&
      typeof instance._currentElement.type === 'string'
    );
  };

  validateDOMNesting = function(parentStack, childTag, instance) {
    if (!isTagValidInContext(childTag, parentStack)) {
      ...
      // parentStack includes only DOM tags; traverse ._parentComponent to
      // include composite names
      var fullParentStack = [];
      for (var inst = instance; inst; inst = inst._parentComponent) {
        if (!isWrappedNativeComponent(inst)) {
          fullParentStack.push(inst.getName ? inst.getName() : '(anonymous)');
        }
      }
      fullParentStack.push(parentStack[0]);  // hack: container tag name
      fullParentStack.reverse();
