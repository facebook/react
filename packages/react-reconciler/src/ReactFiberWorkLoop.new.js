...
const renderLanesCursor: StackCursor<Lanes> = createCursor(NoLanes);
  ...
  if (root !== null) {
    ...
    if (workInProgressRoot === root) {
      ...
      if (workInProgressRootExitStatus === RootSuspendedWithDelay) {
        ...
        markRootSuspended(root, root.wipLanes);
      }
    }
  }
...
function performSyncWorkOnRoot(root) {
  ...
  if (
    ...
    includesSomeLane(root.expiredLanes, root.wipLanes)
  ) {
...
export function pushRenderLanes(fiber: Fiber, subtreeRenderLanes: Lanes) {
  pushToStack(renderLanesCursor, renderLanes, fiber);
  renderLanes = combineLanes(renderLanes, subtreeRenderLanes);
...
export function popRenderExpirationTime(fiber: Fiber) {
  renderLanes = renderLanesCursor.current;
  popFromStack(renderLanesCursor, fiber);
}
...
function prepareFreshStack(root: FiberRoot, lanes: Lanes) {
  ...
  renderLanes = workInProgressRootIncludedLanes = root.wipLanes = lanes;
...
export function renderDidSuspendDelayIfPossible(): void {
  ...
  if (
    ...
    markRootSuspended(workInProgressRoot, workInProgressRoot.wipLanes);
  }
}
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  } else if (workInProgressRoot !== root) {
    workInProgressRoot.wipLanes = NoLanes;
    prepareFreshStack(root, lanes);
    startWorkOnPendingInteractions(root, lanes);
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  if (root === workInProgressRoot) {
    ...
    workInProgressRoot = null;
    workInProgress = null;
    renderLanes = root.wipLanes = NoLanes;
  } else {
