...
describe('ReactInteractionTracing', () => {
  ...
  // @gate enableTransitionTracing
  it('warn and calls marker incomplete if name changes before transition completes', async () => {
    const transitionCallbacks = {
      onTransitionStart: (name, startTime) => {
        Scheduler.unstable_yieldValue(
          `onTransitionStart(${name}, ${startTime})`,
  ...
  it('marker incomplete for tree with parent and sibling tracing markers', async () => {
    ...
    await act(async () => {
      ...
      root.render(<App navigate={true} markerName="marker three" />);
      ReactNoop.expire(1000);
      await advanceTimers(1000);
      expect(() => {
        expect(Scheduler).toFlushAndYield([
    ...
    expect(Scheduler).toFlushAndYield([
      'Page Two',
      'onMarkerProgress(transition one, parent, 1000, 5000, [suspense sibling])',
      // Deoesn't call markerComplete but does call markerProgress for incomplete transitions
      'onMarkerProgress(transition one, marker one, 1000, 5000, [])',
      'onTransitionProgress(transition one, 1000, 5000, [suspense sibling])',
    ]);
    ...
    expect(Scheduler).toFlushAndYield([
      ...
      'onMarkerComplete(transition one, parent, 1000, 6000)',
      'onTransitionProgress(transition one, 1000, 6000, [])',
      'onTransitionComplete(transition one, 1000, 6000)',
    ]);
  });
