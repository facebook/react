...
describe('ReactDebugFiberPerf', () => {
	...
	it('supports returns', () => {
		...
		function HandleReturns(props, returns) {
			return returns.map((y, i) => <y.continuation key={i} isSame={props.foo === y.props.bar} />);
		}

		function CoParent(props) {
			return ReactCallReturn.unstable_createCall(props.children, HandleReturns, props);
		}
	...
	it('supports portals', () => {
		...
		ReactNoop.render(<Parent>{ReactDOM.createPortal(<Child />, noopContainer, null)}</Parent>);
		ReactNoop.flush();
		expect(getFlameChart()).toMatchSnapshot();
	});
