...
export function initHostComponentInputWrapperState(
  element: Element,
  props: Object,
) {
  if (__DEV__) {
    controlledValuePropTypes.checkPropTypes('input', props);
...
export function getHostComponentInputProps(element: Element, props: Object) {
  const node = ((element: any): InputWithWrapperState);
  const checked = props.checked;
...
export function applyHostComponentInputMountWrapper(
  element: Element,
  props: Object,
  isHydrating: boolean,
) {
...
export function restoreHostComponentInputControlledState(
  element: Element,
  props: Object,
) {
  const node = ((element: any): InputWithWrapperState);
...
function updateNamedCousins(rootNode, props) {
  ...
  if (props.type === 'radio' && name != null) {
    ...
    for (let i = 0; i < group.length; i++) {
      ...
      const otherProps = getFiberPropsFromDomNodeInstance(otherNode);
      invariant(
        otherProps,
