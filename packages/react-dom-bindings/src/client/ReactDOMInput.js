...
export function updateInput(
  ...
  if (
    ...
    if (__DEV__) checkAttributeStringCoercion(type, 'type');
    node.type = type;
  } else {
    node.removeAttribute('type');
  }
  ...
  if (typeof value === 'function' || typeof value === 'symbol') {
    ...
    const stringValue = toString(getToStringValue(value));
    if (type === 'number') {
      if ((value === 0 && node.value === '') || node.value !== stringValue) {
        node.value = stringValue;
      }
    } else if (node.value !== stringValue) {
      node.value = stringValue;
    }
  } else if (type === 'submit' || type === 'reset') {
    node.removeAttribute('value');
  ...
  if (
    ...
    if (__DEV__) checkAttributeStringCoercion(name, 'name');
    node.name = toString(getToStringValue(name));
  } else {
    node.removeAttribute('name');
  }
