...
let injectBackendIntervalID = null;
...
function initBridgeAndStore() {
  ...
  bridge = new Bridge({
    ...
    send(event: string, payload: any, transferable?: Array<any>) {
      try {
        // Sometimes when navigating away from a page,
        // the port will get disconnected before Bridge shutdown has finished.
        // In this case we should catch the error-
        // else the new page/tab can end up in a broken state (and not get initialized).
        port.postMessage({event, payload}, transferable);
      } catch (error) {
        console.error('[React DevTools] postMessage() error:', error);
      }
    },
  });
  bridge.addListener('prepForReloadAndProfile', () => {
  ...
  // since global values stored on window have been reset by navigation.
  ...
  let numInjectionAttempts = 0;
  function injectBackend() {
    if (numInjectionAttempts++ > 10) {
      clearInterval(injectBackendIntervalID);
    }
    bridge.send('injectBackend');
  }

  chrome.runtime.onMessage.addListener(function listenForBackendInjectedAck(
    request,
    sender,
  ) {
    if (request.backendInjectedAck) {
      chrome.runtime.onMessage.removeListener(listenForBackendInjectedAck);
      clearInterval(injectBackendIntervalID);
    }
  });
  ...
  //
  // Note that on some websites (e.g. reactjs.org) the initial "inject" message is missed,
  // so we poll and wait for an explicit ack before stopping.
  injectBackendIntervalID = setInterval(injectBackend, 100);
  injectBackend();
