...
> = new LRU({
  ...
  dispose: (runtimeSourceURL: string, metadata: CachedRuntimeCodeMetadata) => {
    if (__DEBUG__) {
      console.log(
        'runtimeURLToMetadataCache.dispose() Evicting cached metadata for "' +
          runtimeSourceURL +
          '"',
      );
    }
...
> = new LRU({
  ...
  dispose: (originalSourceURL: string, metadata: CachedSourceCodeMetadata) => {
    if (__DEBUG__) {
      console.log(
        'originalURLToMetadataCache.dispose() Evicting cached metadata for "' +
          originalSourceURL +
          '"',
      );
    }
...
export default async function parseHookNames(
  ...
  // Gather the unique set of source locations to inspect for the built-in hooks.
  ...
  for (let i = 0; i < hooksList.length; i++) {
    ...
    if (!locationKeyToHookSourceData.has(locationKey)) {
      if (runtimeSourceURL == null) {
        // Not reachable because getLocationKey will have thrown
        throw Error('Hook source code location not found.');
      }
      ...
      if (runtimeMetadata != null) {
        if (__DEBUG__) {
          console.groupCollapsed(
            'parseHookNames() Found cached runtime metadata for file "' +
              hookSourceData.runtimeSourceURL +
              '"',
          );
  ...
  locationKeyToHookSourceData.forEach(hookSourceData => {
    ...
    if (sourceMappingURLs == null) {
      ...
      for (let i = 0; i < sourceMappingURLs.length; i++) {
        ...
        if (index >= 0) {
          // TODO deduplicate parsing in this branch (similar to fetching in the
          // other branch), since there can be multiple location keys per source
          // map.
          ...
          const match = parsed.sources.find(
            source =>
              ...
              runtimeSourceURL.includes(source),
          );
          if (match) {
            setPromises.push(
...
async function parseSourceAST(
  ...
  locationKeyToHookSourceData.forEach(hookSourceData => {
    ...
    if (sourceConsumer !== null) {
      ...
      // TODO maybe canonicalize this URL somehow? It can be relative if the
      // source map specifies it that way, but we use it as a cache key across
      // different source maps and there can be collisions.
      ...
      // This mixes runtimeSourceURLs with source mapped URLs in the same cache
      // key space. TODO: namespace them?
    ...
    if (sourceMetadata != null) {
      if (__DEBUG__) {
        console.groupCollapsed(
          'parseSourceAST() Found cached source metadata for "' +
            originalSourceURL +
            '"',
        );
      ...
      if (__DEBUG__) {
        console.log(
          'parseSourceAST() Caching source metadata for "' +
            originalSourceURL +
            '"',
        );
      }
...
function updateLruCache(
  ...
  locationKeyToHookSourceData.forEach(({sourceConsumer, runtimeSourceURL}) => {
    ...
    if (!runtimeURLToMetadataCache.has(runtimeSourceURL)) {
      if (__DEBUG__) {
        console.log(
          'updateLruCache() Caching runtime metadata for "' +
            runtimeSourceURL +
            '"',
        );
      }
