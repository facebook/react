...
import {__DEBUG__, __PROFILE__} from 'react-devtools-shared/src/constants';
...
import {
  registerEventLogger,
  logEvent,
} from 'react-devtools-shared/src/EventLogger';

import type {ParseHookNamesEvents} from 'react-devtools-shared/src/EventLogger';
import type {
  HooksNode,
  HookSource,
  HooksTree,
...
if (__PROFILE__) {
  ...
  registerEventLogger(event => {
    global.__loggerEvents.push(event);
  });
  console.log('Event log is now being recorded to `globalThis.__loggerEvents`');
}
...
type HookSourceData = {|
  ...
  // Name of the hook generated by react-debug-tools.
  hookName: string,

  // Identifier for hook location
  locationKey: string,
...
function withSyncProfiling<TReturn>(
  ...
  hookSourceData: HookSourceData,
  callback: () => TReturn,
): TReturn {
  if (__PROFILE__ && logEvent != null) {
    const now = performance.now();
    const result = callback();
    const durationMs = performance.now() - now;
    logEvent({
      name,
      hookName: hookSourceData.hookName,
      locationKey: hookSourceData.locationKey,
      durationMs,
    });
    return result;
  }
...
function withCallbackProfiling<TReturn>(
  callback: (
    done: (name: ParseHookNamesEvents, hookSourceData: HookSourceData) => void,
  ) => TReturn,
): TReturn {
  if (__PROFILE__ && logEvent != null) {
...
export async function parseHookNames(
  ...
  if (__PROFILE__ && logEvent != null) {
    const now = performance.now();
    const result = await parseHookNamesImpl(hooksList);
    const durationMs = performance.now() - now;
    logEvent({
      name: 'parseHookNames',
      numberOfHooks: hooksList.length,
      durationMs,
    });
    return result;
  }
  return parseHookNamesImpl(hooksList);
}
...
async function parseHookNamesImpl(
  ...
  await loadSourceFiles(locationKeyToHookSourceData);
  await extractAndLoadSourceMaps(locationKeyToHookSourceData);
  parseSourceAST(locationKeyToHookSourceData);
  updateLruCache(locationKeyToHookSourceData);
  return findHookNames(hooksList, locationKeyToHookSourceData);
}
...
function findHookNames(
  ...
  hooksList.map(hook => {
    ...
    withSyncProfiling('findHookNames', hookSourceData, () => {
    ...
    withCallbackProfiling(done => {
      ...
      setPromises.push(
        fetchPromise.then(runtimeSourceCode => {
          ...
          done('loadSourceFiles', hookSourceData);
        }),
      );
    });
  });
...
function parseSourceAST(
  ...
  locationKeyToHookSourceData.forEach(hookSourceData => {
    withSyncProfiling('parseSourceAST', hookSourceData, () => {
