  ...
  [true, false].forEach(enableSchedulerTracing => {
    describe('onRender callback', () => {
      ...
      it('is not invoked until the commit phase', () => {
        ...
        expect(renderer).unstable_toFlushAndYieldThrough(['first']);
        expect(callback).toHaveBeenCalledTimes(0);
        expect(renderer).unstable_toFlushAndYield(['last']);
      ...
      describe('with regard to interruptions', () => {
        it('should accumulate actual time after a scheduling interruptions', () => {
          ...
          expect(renderer).unstable_toFlushAndYieldThrough(['Yield:2']);
          expect(callback).toHaveBeenCalledTimes(0);

          // Resume render for remaining children.
          expect(renderer).unstable_toFlushAndYield(['Yield:3']);
        ...
        it('should not include time between frames', () => {
          ...
          expect(renderer).unstable_toFlushAndYieldThrough(['Yield:5']);
          expect(callback).toHaveBeenCalledTimes(0);
          ...
          expect(renderer).unstable_toFlushAndYield(['Yield:10', 'Yield:17']);
          expect(callback).toHaveBeenCalledTimes(2);
        ...
        it('should report the expected times when a high-pri update replaces a mount in-progress', () => {
          ...
          expect(renderer).unstable_toFlushAndYieldThrough(['Yield:10']);
          expect(callback).toHaveBeenCalledTimes(0);
          ...
          expect(ReactTestRenderer).unstable_toHaveYielded(['Yield:5']);
          ...
          expect(renderer).unstable_toFlushWithoutYielding();
          expect(callback).toHaveBeenCalledTimes(0);
        });
        ...
        it('should report the expected times when a high-priority update replaces a low-priority update', () => {
          ...
          expect(renderer).unstable_toFlushAndYield(['Yield:6', 'Yield:15']);
          expect(callback).toHaveBeenCalledTimes(1);
          let call = callback.mock.calls[0];
          ...
          expect(renderer).unstable_toFlushAndYieldThrough(['Yield:3']);
          expect(callback).toHaveBeenCalledTimes(0);
          ...
          expect(ReactTestRenderer).unstable_toHaveYielded(['Yield:11']);
          ...
          // Verify no more unexpected callbacks from low priority work
          expect(renderer).unstable_toFlushAndYield([]);
          expect(callback).toHaveBeenCalledTimes(1);
        });
        ...
        it('should report the expected times when a high-priority update interrupts a low-priority update', () => {
          ...
          expect(renderer).unstable_toFlushAndYieldThrough([
            'FirstComponent:10',
          ]);
          expect(callback).toHaveBeenCalledTimes(0);
  ...
  describe('interaction tracing', () => {
    ...
    describe('error handling', () => {
      it('should cover errors thrown in onWorkScheduled', () => {
        ...
        expect(renderer).unstable_toFlushAndYield(['Component:succeed']);
      ...
      it('should cover errors thrown in onWorkStarted', () => {
        ...
        expect(renderer).unstable_toFlushAndThrow(
          'Expected error onWorkStarted',
        );
        expect(ReactTestRenderer).unstable_toHaveYielded(['Component:text']);
        throwInOnWorkStarted = false;
    ...
    it('should report the expected times when a high-priority update interrupts a low-priority update', () => {
      ...
      SchedulerTracing.unstable_trace(interactionLowPri.name, mockNow(), () => {
        ...
        expect(renderer).unstable_toFlushAndYieldThrough(['FirstComponent']);
        expect(onRender).not.toHaveBeenCalled();
        ...
        expect(ReactTestRenderer).unstable_toHaveYielded(['SecondComponent']);
    ...
    describe('suspense', () => {
      ...
      it('does not prematurely complete for suspended renders that have exceeded their deadline', async () => {
        ...
        expect(renderer).unstable_toFlushAndYield(['AsyncText [loaded]']);
        expect(onInteractionScheduledWorkCompleted).toHaveBeenCalledTimes(1);
        expect(
          onInteractionScheduledWorkCompleted,
        ).toHaveBeenLastNotifiedOfInteraction(interaction);
