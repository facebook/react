...
import type {ClientManifest} from './ReactFlightServerConfigFBBundler';
...
type PipeableStream = {
  ...
  pipe(destination: Destination): Destination,
};
...
function decodeReplyFromBusboy<T>(
  busboyStream: Busboy,
  staticResourcesMap: ServerManifest,
): Thenable<T> {
  const response = createResponse(staticResourcesMap, '');
  let pendingFiles = 0;
...
function decodeReply<T>(
  ...
  const response = createResponse(staticResourcesMap, '', body);
  close(response);
  return getRoot(response);
}
