...
Contudo, e uma vez que existe a clara necessidade de coordenar o esforço dos vários colaboradores, existem algumas práticas, normalmente associadas a processos *Agile*, seguidas pela *core team* do projeto e aconselhadas aos membros da comunidade que desejam contribuir para o mesmo, de forma a facilitar a integração de todos no desenvolvimento do React.

De uma forma geral, esse conjunto de práticas seguidas pareceu-nos adequado para um projeto desta dimensão. Processos tipicamente *Agile*, como *Scrum* e XP, são feitos a pensar em empresas que procuram modelos de negócio rentáveis, pelo que a sua adaptação a um projeto como o do React teria sempre de ter em conta a filosofia *open-source*, com fins não lucrativos, em que o mesmo se enquadra.
