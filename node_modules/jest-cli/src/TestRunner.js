...
TestRunner.prototype._getModuleLoaderResourceMap = function() {
  console.log('_getModuleLoaderResourceMap', this._config, this._moduleLoaderResourceMap);
  var ModuleLoader = this._loadConfigDependencies().ModuleLoader;
  ...
  console.log('_getModuleLoaderResourceMap', this._moduleLoaderResourceMap);
...
TestRunner.prototype.streamTestPathsMatching = function(pathPattern) {
  ...
  this._getModuleLoaderResourceMap().then(function(resourceMap) {
    ...
    console.log('streamTestPathsMatching promise error...', e);
  });
...
TestRunner.prototype.promiseTestPathsMatching = function(pathPattern) {
  var s = this.streamTestPathsMatching(pathPattern);
  console.log('promiseTestPathsMatching', pathPattern, s);
  var p = _pathStreamToPromise(s);
  console.log('promiseTestPathsMatching', p);
  return p;
};
...
function _pathStreamToPromise(stream) {
  ...
  stream.on('data', function(path) {
    console.log('_pathStreamToPromise data:', path);
    paths.push(path);
  });
  stream.on('error', function(err) {
    console.log('_pathStreamToPromise error:', err);
    defer.reject(err);
  });
  stream.on('end', function() {
    console.log('_pathStreamToPromise end');
    defer.resolve(paths);
  });
  console.log('_pathStreamToPromise deferred ok I guess');
  return defer.promise;
  } catch (e) {
    console.log('_pathStreamToPromise deferred ERROR', e);
  }
}
