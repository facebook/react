 ...
 */
function renderGraphic(degrees, velocity, setVelocity, setDrag) {
  /**
   * Cause the rotation to "spring".
   */
  const handleMouseUp = () => {
    const nextVelocity = Math.min(velocity + CLICK_ACCEL, MAX_VEL);
    setVelocity(nextVelocity);
    setDrag(MOUSE_UP_DRAG);
  };
  ...
  return (
    <Group onMouseDown={handleMouseDown} onMouseUp={handleMouseUp}>
      <Group x={210} y={135}>
        ...
        <Group x={55} y={29}>
          <Group rotation={degrees} originX={84} originY={89}>
            <Group x={84} y={89}>
...
function VectorWidget() {
  ...
  const [drag, setDrag] = React.useState(MOUSE_UP_DRAG);

  const onTick = () => {
    const nextDegrees = degrees + BASE_VEL + velocity;
    const nextVelocity = velocity * drag;
    setDegrees(nextDegrees);
    setVelocity(nextVelocity);
  };

  React.useEffect(() => {
    const interval = window.setInterval(onTick, 20);
    return () => window.clearInterval(interval);
  }, [degrees, velocity, drag]); // Dependencies updated for useEffect

  return (
    <Surface width={700} height={700} style={{cursor: 'pointer'}}>
      {renderGraphic(degrees, velocity, setVelocity, setDrag)}
    </Surface>
  );
}
...
//     console.log(nextDegrees, nextVelocity);
