...
const {renderToPipeableStream} = require('react-server-dom-vite/server');
async function createApp() {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    globalThis.__vite_preload__ = metadata => {
      ...
      if (existingPromise) {
        ...
        const modulePromise = viteServer.ssrLoadModule(id);
        modulePromise.then(
          value => {
        ...
        __vite_module_cache__.set(id, modulePromise);
    ...
    globalThis.__vite_require__ = metadata => {
      const module = require(
        path.join(process.cwd(), 'build', 'server', metadata.specifier + '.cjs')
      );
  ...
  loadModule = async metadata => {
    await __vite_preload__(metadata.specifier);
    const mod = await __vite_require__(metadata.specifier);
  ...
  async function renderApp(res, returnValue) {
    ...
    const {pipe} = renderToPipeableStream(payload, manifest);
    pipe(res);
  }
  ...
  app.post('/', bodyParser.text(), async function (req, res) {
    ...
    if (serverReference) {
      ...
      if (req.is('multipart/form-data')) {
        ...
        const reply = decodeReplyFromBusboy(bb, manifest);
        req.pipe(bb);
        args = await reply;
      } else {
        args = await decodeReply(req.body, manifest);
      }
      ...
      const action = await decodeAction(formData, manifest);
      try {
        // Wait for any mutations
        await action();
      } catch (x) {
